---
title: "SB 36 March 2023 Output Tables"
...
XXXXXXX Pull snowflake data -- cjs_outcome_std -- removed 
...
df <- df %>%
  mutate(tool_name_orig = tool_name,
         ...
         tool_name = case_when(county_name == "Kings" ~ "VPRAI-O", 
                               county_name == "Santa Barbara" & assessment_date < as.Date("2020-4-29") ~ "VPRAI",
                               county_name == "Santa Barbara" & assessment_date >= as.Date("2020-4-29") ~ "VPRAI-R",
                               county_name == "Alameda" & assessment_date < as.Date("2022-10-24") ~ "VPRAI-R",
                               county_name == "Alameda" & assessment_date >= as.Date("2022-10-24") ~ "PSA",
                               county_name %in% c("Sierra", "Nevada") & assessment_date < as.Date("2022-07-11") ~ "ORAS",
                               county_name %in% c("Sierra", "Nevada") & assessment_date >= as.Date("2022-07-11") ~ "PSA",
         ...
         County = fct_relevel(County, c("Small Counties", "Small/Medium Counties")),
         ...
         oras_risk_score = if_else(tool_name == "ORAS", risk_score_raw, NA_real_),
         vprai_risk_score = if_else(tool_name == "VPRAI", risk_score_raw, NA_real_),
         vprair_risk_score = if_else(tool_name == "VPRAI-R", risk_score_raw, NA_real_),
         vpraio_risk_score = if_else(tool_name == "VPRAI-O", generic_tool_total_score_raw, NA_real_),
  ...
  filter(assessment_date >= as.Date("2019-10-01"),
         assessment_date <= as.Date("2022-12-31")) %>%
  filter(scored_assessment == 1)
```

\newpage

Format join_df, and eval_df
...
join_df <- join_df %>%
  mutate(book_date = booking_date,
         ...
         County = ordered(County),
         County = fct_relevel(County, c("Small Counties", "Small/Medium Counties")),
         release_type = if_else(release_type == "CiteR elease", "Cite Release", release_type), # fix LA's typo
         convicted_flag = if_else(disposition_type == "Convicted", 1, 0),
         ...
         tool_name = case_when(county == "Kings" ~ "VPRAI-O", 
                               ...
                               county == "Alameda" & assessment_date < as.Date("2022-10-24") ~ "VPRAI-R",
                               county == "Alameda" & assessment_date >= as.Date("2022-10-24") ~ "PSA",
                               county %in% c("Sierra", "Nevada") & assessment_date < as.Date("2022-07-11") ~ "ORAS",
                               county %in% c("Sierra", "Nevada") & assessment_date >= as.Date("2022-07-11") ~ "PSA",
                               tool_name == "VPRAIR" ~ "VPRAI-R",
                               T ~ tool_name),
         oras_risk_score = if_else(tool_name == "ORAS", risk_score_raw, NA_real_),
         vprai_risk_score = if_else(tool_name == "VPRAI", risk_score_raw, NA_real_),
...
join_df <- join_df  %>%
  filter(booking_date >= as.Date("2019-10-01"),
         booking_date <= as.Date("2022-12-31"))
...
write.xlsx(date_range, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/All bookings date ranges by county.xlsx", asTable = F)
...
write.xlsx(date_range, file = "All bookings date ranges by county small disaggregated.xlsx", asTable = F)
...
write.xlsx(date_range, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/Assessments with outcomes, date ranges by county.xlsx", asTable = F)
...
gender_df <- join_df %>%
  mutate(release_cat = case_when(release_type %in% c("Cite Release", "Bail Bond", "Own Recognizance", "Pretrial Monitor",  "Zero Bail") ~ "Pretrial release type",
  ...
  mutate(quick_pretrial_type_flag = if_else(two_day_release == 1 & release_cat == "Pretrial release type", 1, 0),
         ...
         quick_or_type_flag = if_else(two_day_release == 1 & release_type %in% c("Own Recognizance", "Pretrial Monitor"), 1, 0),
         ...
         later_or_type_flag = if_else(two_day_release == 0 & release_type %in% c("Own Recognizance", "Pretrial Monitor"), 1, 0)) %>%
  group_by(County, sex) %>%
  summarise(Total = n(),
...
write.xlsx(gender_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Gender fresh.xlsx", asTable = F)
...
write.xlsx(race_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Race fresh.xlsx", asTable = F)
...
write.xlsx(arrest_charge_level_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Offense Type fresh.xlsx", asTable = F)
...
write.xlsx(gender_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Gender LA.xlsx", asTable = F)
...
write.xlsx(race_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Race LA.xlsx", asTable = F)
...
write.xlsx(arrest_charge_level_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2A Offense Type LA.xlsx", asTable = F)
...
write.xlsx(fta_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2B.xlsx", asTable = T)
...
write.xlsx(fta_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2B Aggregated.xlsx", asTable = T)
...
write.xlsx(new_charge_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2C.xlsx", asTable = T)
...
write.xlsx(new_charge_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2C Aggregated.xlsx", asTable = T)
...
write.xlsx(age_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Age.xlsx", asTable = T)
...
write.xlsx(age_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Age Aggregated.xlsx", asTable = T)
...
write.xlsx(zip_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Zipcode.xlsx", asTable = T)
```
\newpage
...
write.xlsx(gender_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Gender.xlsx", asTable = T)
...
write.xlsx(gender_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Gender Aggregated.xlsx", asTable = T)
...
write.xlsx(race_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Race.xlsx", asTable = T)
...
write.xlsx(race_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2D Race Aggregated.xlsx", asTable = T)
...
write.xlsx(assess_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Risk Level.xlsx", asTable = T)
...
write.xlsx(assess_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Risk Level Aggregated.xlsx", asTable = T)
...
write.xlsx(zip_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Zipcode.xlsx", asTable = T)
```
...
write.xlsx(charge_level_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Charge Level.xlsx", asTable = T)
...
write.xlsx(charge_level_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Charge Level Aggregated.xlsx", asTable = T)
...
release_df <- df %>%
  # filter(la_flag == 0) %>%
  mutate(release_decision_prearraignment = case_when(release_decision_prearraignment %in% c("Monitor", "Own Recognizance") ~ "Granted Program Release",
                                                     release_decision_prearraignment %in% c("Deny Release") ~ "Denied Program Release",
                                                     release_decision_prearraignment %in% c("Other Unknown", "Unknown", "Other", "Undefined") ~ "Other/Unknown",
                                                     release_decision_prearraignment %in% c("Ineligible") ~ "Ineligible")) %>%
  count(County, release_decision_prearraignment)
...
write.xlsx(release_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Release Decision Pre-Arraignment.xlsx", asTable = T)
...
#   count(release_decision) 
# 
# write.xlsx(release_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Release Decision Aggregated.xlsx", asTable = T)  
# 
...
#   count(release_decision) 
# 
# write.xlsx(release_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Release Decision Aggregated without LA.xlsx", asTable = T)  
...
release_df <- df %>%
  # filter(la_flag == 0) %>%
  mutate(release_decision_arraignment = case_when(release_decision_arraignment %in% c("Monitor", "Own Recognizance") ~ "Granted Program Release",
                                                     release_decision_arraignment %in% c("Deny Release") ~ "Denied Program Release",
                                                     release_decision_arraignment %in% c("Other Unknown", "Unknown", "Undefined", "Other") ~ "Other/Unknown",
                                                     release_decision_arraignment %in% c("Ineligible") ~ "Ineligible")) %>%
  count(County, release_decision_arraignment)# %>%
  #filter(!County %in% c("San Joaquin", "Sonoma", "Los Angeles"))
  # group_by(County) %>%
  # mutate(Total = sum(n)) %>%
  # pivot_wider(names_from =  release_decision, values_from =  n, values_fill = 0) %>%
...
write.xlsx(release_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Release Decision Arraignment.xlsx", asTable = T)
# San Joaquin and Sonoma did not provide release decision information. 
...
write.xlsx(release_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2E Release Decision LA only.xlsx", asTable = T)
# San Joaquin and Sonoma did not provide release decision information. 
...
write.xlsx(monitor_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2F Supervision Level.xlsx", asTable = T)
...
write.xlsx(monitor_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2F Supervision Level Aggregated.xlsx", asTable = T)
...
write.xlsx(outcome_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2G Supervision Level.xlsx", asTable = T) 
...
write.xlsx(outcome_per_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2G Supervision Level Percent.xlsx", asTable = T)
...
write.xlsx(outcome_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2G Supervision Level Aggregated.xlsx", asTable = T) 

outcome_per_df <- outcome_df %>%
  mutate(FTA_percent = FTA / Total,
         New_Arrest_Percent = New_Arrest / Total)

write.xlsx(outcome_per_df, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/2G Supervision Level Aggregated Percent.xlsx", asTable = T)
...
write.xlsx(risk_df_oras, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A Oras.xlsx", asTable = T)  
...
write.xlsx(risk_df_oras, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A Oras categories.xlsx", asTable = T)  
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Modoc, Napa, Nevada, Ventura, and Yuba.
\newpage
...
write.xlsx(risk_df_vprai, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A Vprai.xlsx", asTable = T)  
...
write.xlsx(risk_df_vprai, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A Vprai categories.xlsx", asTable = T)  
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from San Joaquin and Santa Barbara.
\newpage
...
write.xlsx(risk_df_vprair, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A VpraiR.xlsx", asTable = T) 
...
write.xlsx(risk_df_vprair, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A VpraiR categories.xlsx", asTable = T) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Alameda, San Mateo and Santa Barbara.
\newpage
...
write.xlsx(risk_df_vpraio, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A VpraiO.xlsx", asTable = T)
...
write.xlsx(risk_df_vpraio, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A VpraiO categories.xlsx", asTable = T)
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Kings.
\newpage
...
write.xlsx(risk_df_psa_fta, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A PSA FTA.xlsx", asTable = T)

write.xlsx(risk_df_psa_nca, file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3A PSA NCA.xlsx", asTable = T)
...
release_arr_oras <- join_df %>%
  filter(tool_name == "ORAS",
         !is.na(oras_risk_score)) %>%
  mutate(release_decision = case_when(release_decision_arraignment %in% c("Monitor", "Own Recognizance")  ~ "Granted Program Release", 
                                      release_decision_arraignment %in% c("Deny Release") ~ "Denied Program Release", 
                                      release_decision_arraignment %in% c("Ineligible") ~ "Ineligible",
                                      T ~ "Other/Unknown"),
         risk_category = case_when(
...
release_pre_oras <- join_df %>%
  filter(tool_name == "ORAS",
         !is.na(oras_risk_score)) %>%
  mutate(release_decision = case_when(release_decision_prearraignment %in% c("Monitor", "Own Recognizance")  ~ "Granted Program Release", 
                                      release_decision_prearraignment %in% c("Deny Release") ~ "Denied Program Release", 
                                      release_decision_prearraignment %in% c("Ineligible") ~ "Ineligible",
                                      T ~ "Other/Unknown"),
         risk_category = case_when(
...
write.xlsx(list(race_oras, gender_oras, offense_type_oras, release_arr_oras, release_pre_oras), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B ORAS.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
race_vprai <- df %>%
  ...
  mutate(race = if_else(race == "Other", "Other_Unknown", race),
         risk_category = case_when(
           vprai_risk_score %in% 0:1 ~ "Scores 0-1",
           vprai_risk_score == 2 ~ "Score 2",
           vprai_risk_score == 3 ~ "Score 3",
           vprai_risk_score == 4 ~ "Score 4",
           vprai_risk_score %in% 5:9 ~ "Scores 5-9"
         ),
         risk_score_sort = case_when(
           risk_category == "Scores 0-1" ~ 1,
           risk_category == "Score 2" ~ 2,
           risk_category == "Score 3" ~ 3,
           risk_category == "Score 4" ~ 4,
           risk_category == "Scores 5-9" ~ 5,
         )) %>%
  count(vprai_risk_score, race, risk_category, risk_score_sort) 
...
gender_vprai <- df %>%
  ...
  count(vprai_risk_score, sex, risk_category, risk_score_sort) 
...
offense_type_vprai <- join_df %>%
  ...
  count(vprai_risk_score, arrest_charge_level, risk_category, risk_score_sort) 
...
release_vprai <- join_df %>%
 ...
 mutate(release_decision = case_when(release_decision_arraignment %in% c("Monitor", "Own Recognizance")  ~ "Granted Program Release", 
  ...
  count(vprai_risk_score, release_decision, risk_category, risk_score_sort)

release_pre_vprai <- join_df %>%
  filter(tool_name == "VPRAI",
         !is.na(vprai_risk_score)) %>%
 mutate(release_decision = case_when(release_decision_prearraignment %in% c("Monitor", "Own Recognizance")  ~ "Granted Program Release", 
                                      release_decision_prearraignment %in% c("Deny Release") ~ "Denied Program Release", 
...
write.xlsx(list(race_vprai, gender_vprai, offense_type_vprai,  release_vprai, release_pre_vprai), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B VPRAI.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type", "Release Arraignment", "Release Prearraignment"))
...
write.xlsx(list(race_vprair, gender_vprair, offense_type_vprair, release_vprair, release_pre_vprair), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B VPRAIR.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
race_vpraio <- df %>%
  ...
  mutate(race = if_else(race == "Other", "Other_Unknown", race),
         risk_category = case_when(
           vpraio_risk_score %in% 0:1 ~ "Scores 0-1",
           vpraio_risk_score == 2 ~ "Score 2",
           vpraio_risk_score == 3 ~ "Score 3",
           vpraio_risk_score == 4 ~ "Score 4",
           vpraio_risk_score %in% 5:10 ~ "Scores 5-10"
         )) %>%
  count(vpraio_risk_score, race, risk_category) 
...
gender_vpraio <- df %>%
  ...
  count(vpraio_risk_score, sex, risk_category) 
...
write.xlsx(list(race_vpraio, gender_vpraio, offense_type_vpraio,  release_vpraio, release_pre_vpraio), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B VPRAI-O.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
write.xlsx(list(race_psa, gender_psa, offense_type_psa,  release_psa, release_pre_psa), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B PSA FTA.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
write.xlsx(list(race_psa, gender_psa, offense_type_psa,  release_psa, release_pre_psa), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B PSA NCA.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
write.xlsx(list(race_psa, gender_psa, offense_type_psa,  release_psa, release_pre_psa), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B PSA NVCA.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release Arraignment", "Release Prearraignment"))
...
write.xlsx(list(risk_df_oras_gender, risk_df_oras_race, risk_df_oras_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C Oras.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type"))  
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Modoc, Napa, Nevada, Ventura, and Yuba.
\newpage
...
write.xlsx(list(risk_df_oras_gender, risk_df_oras_race, risk_df_oras_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C Oras categories.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type"))  
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Modoc, Napa, Nevada, Ventura, and Yuba.
\newpage
...
write.xlsx(list(risk_df_vprai_gender, risk_df_vprai_race, risk_df_vprai_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAI.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from San Joaquin and Santa Barbara.
\newpage
...
write.xlsx(list(risk_df_vprai_gender, risk_df_vprai_race, risk_df_vprai_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAI categories.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from San Joaquin and Santa Barbara.
\newpage
...
write.xlsx(list(risk_df_vprair_gender, risk_df_vprair_race, risk_df_vprair_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAIR.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Alameda, San Mateo and Santa Barbara.
\newpage
...
write.xlsx(list(risk_df_vprair_gender, risk_df_vprair_race, risk_df_vprair_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAIR categories.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Alameda, San Mateo and Santa Barbara.
\newpage
...
write.xlsx(list(risk_df_vpraio_gender, risk_df_vpraio_race, risk_df_vpraio_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAI-O.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Kings.
\newpage
...
write.xlsx(list(risk_df_vpraio_gender, risk_df_vpraio_race, risk_df_vpraio_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAI-O categories.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Kings.
\newpage
...
write.xlsx(list(risk_df_psa_gender, risk_df_psa_race, risk_df_psa_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C PSA FTA.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
...
write.xlsx(list(risk_df_psa_gender, risk_df_psa_race, risk_df_psa_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C PSA NCA.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
...
write.xlsx(list(risk_df_psa_gender, risk_df_psa_race, risk_df_psa_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C PSA NVCA.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Calaveras, Los Angeles, Sacramento, Tulare and Tuolumne.
\newpage
# SB 36: 3D
...
write.xlsx(list(override_df, override_df2), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3D.xlsx", asTable = T, sheetName = c("Overrides", "Supplementary")) 
...
override_df_arr <- df %>%
  ...
  mutate(nperc = str_c(perc, "%", "  (", formatC(n, big.mark = ","), ")"))

oras_arr <- override_df_arr %>%
  ungroup() %>%
  filter(tool_name == "ORAS") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vprai_arr <- override_df_arr %>%
  ungroup() %>%
  filter(tool_name == "VPRAI") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vprair_arr <- override_df_arr %>%
  ungroup() %>%
  filter(tool_name == "VPRAI-R") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vpraio_arr <- override_df_arr %>%
  ungroup() %>%
  filter(tool_name == "VPRAI-O") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

psa_arr <- override_df_arr %>%
  ungroup() %>%
  filter(tool_name == "PSA") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)
...
override_df_pre <- df %>%
  mutate(release_recommendation = if_else(is.na(release_recommendation) |release_recommendation %in% c("Other UnKnown", "Other Unknown", "Other/Unknown", "Undefined"), "Other/Unknown", release_recommendation)) %>%
  mutate(release_recommendation = if_else(release_recommendation %in% c( "Monitor", "Own Recognizance"), "Release", release_recommendation),
         release_decision = case_when(release_decision_prearraignment %in% c("Monitor", "Own Recognizance") ~ "Release",
                                      release_decision_prearraignment %in% c("Deny Release") ~ "Deny Release",
                                      release_decision_prearraignment %in% c("Ineligible") ~ "Ineligible",
                                      T ~ "Other/Unknown")) %>%
  group_by(tool_name, release_recommendation, release_decision) %>%
  count() %>%
...
oras_pre <- override_df_pre %>%
  ungroup() %>%
  filter(tool_name == "ORAS") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vprai_pre <- override_df_pre %>%
  ungroup() %>%
  filter(tool_name == "VPRAI") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vprair_pre <- override_df_pre %>%
  ungroup() %>%
  filter(tool_name == "VPRAI-R") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

vpraio_pre <- override_df_pre %>%
  ungroup() %>%
  filter(tool_name == "VPRAI-O") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

psa_pre <- override_df_pre %>%
  ungroup() %>%
  filter(tool_name == "PSA") %>%
  select(-n, -perc, -tool_name) %>%
  pivot_wider(names_from = release_decision, values_from = nperc)

write.xlsx(list(oras_arr, vprai_arr, vprair_arr, vpraio_arr, psa_arr), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3D Arraignment.xlsx", asTable = T, sheetName = c("ORAS Arraignment", "VPRAI Arraignment", "VPRAI-R Arraignment", "VPRAI-O Arraignment", "PSA Arraignment")) 

write.xlsx(list(oras_pre, vprai_pre, vprair_pre, vpraio_pre, psa_pre), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3D Prearraignment.xlsx", asTable = T, sheetName = c("ORAS Prearraignment", "VPRAI Prearraignment", "VPRAI-R Prearraignment", "VPRAI-O Prearraignment", "PSA Prearraignment")) 
