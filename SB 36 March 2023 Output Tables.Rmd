...
df <- df %>%
  mutate(tool_name_orig = tool_name,
         ...
         tool_name = case_when(county_name == "Kings" ~ "VPRAIO", 
...
join_df <- join_df %>%
  mutate(book_date = booking_date,
         ...
         age = individual_age,
         tool_name = case_when(county == "Kings" ~ "VPRAIO", 
         ...
         vprai_risk_score = if_else(tool_name == "VPRAI", risk_score_raw, NA_real_),
         vprair_risk_score = if_else(tool_name == "VPRAI-R", risk_score_raw, NA_real_),
         vpraio_risk_score = if_else(tool_name == "VPRAIO", generic_tool_total_score_raw, NA_real_),
         psa_fta_risk_score = score_failure_to_appear,
         psa_nca_risk_score = score_new_criminal_activity,
         psa_nvca_risk_score = score_new_criminal_violent_activity,
         scored_assessment = if_else(tool_name %in% c("ORAS", "VPRAI", "VPRAI-R") & !is.na(risk_score_raw) |
...
release_df <- df %>%
  # filter(la_flag == 0) %>%
  mutate(release_decision_prearraignment = if_else(release_decision_prearraignment %in% c("Monitor", "Own Recognizance"), "Granted Program Release", release_decision_prearraignment),
         release_decision_prearraignment = if_else(release_decision_prearraignment %in% c("Deny Release"), "Denied Program Release", release_decision_prearraignment)) %>%
...
write.xlsx(list(race_vpraio, gender_vpraio, offense_type_vpraio,  release_vpraio), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3B VPRAIO.xlsx", asTable = T, sheetName = c("Race", "Gender", "Offense Type",  "Release"))
...
write.xlsx(list(risk_df_vpraio_gender, risk_df_vpraio_race, risk_df_vpraio_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAIO.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Kings.
\newpage
...
write.xlsx(list(risk_df_vpraio_gender, risk_df_vpraio_race, risk_df_vpraio_offense), file = "G:/CrimJustice/Pretrial/000-Pretrial Pilot Program/SB 36 and AB 74  Reporting/SB 36 tables 2023/3C VPRAIO categories.xlsx", asTable = T, sheetName = c("Gender", "Race", "Offense Type")) 
```
Source: Pretrial Pilot Program Probation Data (10-01-2019 through 03-31-2021) from Kings.
\newpage
