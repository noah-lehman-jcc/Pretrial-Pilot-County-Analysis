...
book_df %>%
  #select(court_docket_id)
  count(court_docket_id %in% c("")) %>%
  mutate(percent = round(n/sum(n)*100))
  
book_df %>%
  mutate(missing = if_else(court_docket_id < 1, 1, 0)) %>%
  count(missing) %>%
  mutate(percent = n/sum(n))

book_df %>%
  filter(year(book_date) >=2017) %>%
  anti_join(court_df, by = "court_docket_id") %>%
  count(court_docket_id, release_type) %>%
  arrange(desc(n))
...
nrow(book_df %>%
       filter(year(book_date) >=2017) %>%
semi_join(court_df, by = c("court_docket_id"), na_matches = "never"))/nrow(book_df %>% filter(year(book_date) >=2017))
