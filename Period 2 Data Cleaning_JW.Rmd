...
```
  ...
  Assessment Tables
```{r}
# risk_level and release_decision are missing. I am unsure about fta_score, nca_score, and nvca_score
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, contains("score"), tool_name)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Calaveras Assessment Output 08-2020.csv")
```
...
# Date Reformatting--See pg. xxx for more details.
assess_df <- assess_df %>%
  mutate_at(vars(contains("date"), dob), funs(if_else(grepl("/", .), mdy(gsub(" @.*", "", .)), convertToDate(.))))
...
```
  ...
  Assessment Tables
```{r}
# risk_level and release_decision are missing.
assess_table <- assess_df %>%
  # Collapse to each assessment.
  ...
  mutate(county = "Modoc") %>%
  # Select relevants variables.
  select(county, release_recommendation, release_decision, assessment_date, oras_score, tool_name)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Modoc Assessment Output 08-2020.csv")
```
...
# Variable name standardization--See pg. xxx for more details.
names(book_df) <- c("book_num", "local_id", "court_case_id", "bail_amt", "arrest_date", "book_date", "book_type_original", "release_date", "release_type_original", "fbi")
...
assess_df %>%
  count(release_decision_prearraignment, release_decision_postarraignment, release_decision) %>%
  arrange(desc(n))
```
...
```
  ...
  Assessment Tables
```{r}
# risk_level, release_recommendation, and release_decision are missing.
assess_table <- assess_df %>%
  # Collapse to each assessment.
  ...
  # Select relevants variables.
  select(county, release_decision, assessment_date, vprai_risk_score, tool_name)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/San Joaquin Assessment Output 08-2020.csv")
```
...
```{r}
# Ventura Assessment Data Import
...
assess_df %>%
  count(pretrial_termination_reason_original) %>%
  arrange(desc(n))
  select(pretrial_termination_date) %>%
  filter(!is.na(pretrial_termination_date))

summary(assess_df$pretrial_termination_date)
...
assess_condition <- assess_condition %>%
  mutate(condition_em_flag = if_else(grepl("ELECTRONIC MONITOR|GPS|SCRAM", toupper(release_conditions_original)), 1, 0),
         ...
         condition_no_contact = if_else(grepl("NO CONTACT|RESTRAINING|NOT CONTACT", toupper(release_conditions_original)), 1, 0))
...
assess_table <- assess_df %>%
  ...
  # Select relevants variables.
  select(county, risk_level, release_recommendation, release_decision, assessment_date, oras_risk_score, tool_name)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Ventura Assessment Output 08-2020.csv")
```
