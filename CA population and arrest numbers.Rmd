...
snow_df <- join_df
#join_df <- snow_df
names(snow_df) <- tolower(names(snow_df))
...
join_df <- snow_df %>%
  ...
  mutate(across(contains("condition"), ~if_else(grepl("^Y", toupper(.x)) | .x == 1, 1,0))) %>%
  mutate(court_date_reminder_flag = if_else(toupper(court_date_reminder_flag) %in% c("YES", "Y", "1", "MESSAGE", "RECEIVES COURT DATE REMINDERS", "TEXT MESSAGE") | court_date_reminder_flag == 1, 1, 0)) %>%
  mutate(across(contains("_flag"), ~if_else(is.na(.x) | .x == "No" | .x == "Unknown" | .x == "0" | .x == 0, 0, as.numeric(1))))
```

Filter dates for full_df
```{r}
full_df <- join_df %>%
...
### Figures for mitigating bias report, all pilots
...
full_df <- join_df %>%
  filter(booking_date < as.Date("2022-01-01")) %>%
  filter(booking_date >= as.Date("2020-07-01"))

```

# Figure 1: Montly bookings to show Covid decline, by misdo/felony
```{r}
join_df %>%
  filter(booking_date < as.Date("2022-01-01")) %>%
  filter(booking_date >= as.Date("2018-07-01")) %>%
...
```
# Figure 2 & 3: Race distribution from population, arrests, booked, filed, filed detained >2 days for MISDEMEANORS and FELONIES
...
collapsed_snowpercs_Misdo <- full_df %>%
  #filter(booking_date >= "2020-03-01") %>%
  mutate(race_standard = if_else(race %in% c("Black", "White", "Hispanic"), race_standard, "Other"),
         booking_charge_level = case_when(booking_charge_level %in% c("F", "Felony") ~ "Felony",
                                          booking_charge_level %in% c("M", "Misdemeanor") ~ "Misdemeanor",
                                          T ~ "Other")) %>%
...
out
#saveWorkbook(out, "collapsed demographics with filings by charge level all pilots 7.2020 thru 12.2021.xlsx")


```

# Figure 4: Pretrial release rates for bookings with court filings by misdo/felony and race
```{r}
pret <- full_df %>%
  #filter(booking_date >= "2020-03-01") %>%
  mutate(county = str_c(county, " County")) %>%
...
#write.xlsx(pret, "Pretrial Release Rates all pilots from 7.2020 thru 12.2021.xlsx")

```

# Figure 5: Pretrial release types for bookings with court filings by misdo/felony and race
```{r}
...
join_df %>%
  ...
  mutate(days_to_disp = pretrial_period_end_date - booking_date) %>%
  group_by(is_released_pretrial, charge_level_standard) %>%
  summarise(avg_days_to_disp = mean(days_to_disp, na.rm = T)) %>%
  pivot_wider(names_from = charge_level_standard, values_from = avg_days_to_disp)
```
...
join_df %>%
  ...
  filter(charge_level_standard %in% c("M", "F")) %>%
  filter(is_released_pretrial == "Yes",
         recid_arrested_flag_doj == 1,
         release_date >= booking_date)  %>%
  mutate(days_to_recid = as.Date(recid_date_doj) - release_date) %>%
  group_by(charge_level_standard) %>%
  summarise(avg_days_to_recid = mean(days_to_recid, na.rm = T),
            med_days_to_recid = median(days_to_recid, na.rm = T),
            max_days_to_recid = max(days_to_recid, na.rm = T))
```
...
join_df %>%
  ...
  filter(is_released_pretrial == "Yes",
         ...
         release_date >= booking_date)  %>%
  mutate(days_to_recid = as.Date(recid_date_doj) - release_date) %>%
  ggplot(aes(x = days_to_recid)) +
    geom_boxplot() +
    facet_grid(rows = "charge_level_standard") +
    xlim(0,500)
```

Average score for detained vs not
```{r}
join_df %>%
  filter(is_booking_assessed == "Yes") %>%
  mutate(tool_name = if_else(county == "Nevada", "ORAS", tool_name)) %>%
  group_by(county, is_released_pretrial) %>%
  summarise(tool_name = max(tool_name, na.rm = T),
            avg_score = if_else(tool_name != "PSA", mean(risk_score_raw, na.rm = T), mean(psa_nca_risk_score, na.rm = T))) %>%
  pivot_wider(names_from = is_released_pretrial, names_prefix = "is_released_pretrial_", values_from = avg_score) %>%
  mutate(dif = is_released_pretrial_No - is_released_pretrial_Yes)
...
```{r}
join_df %>%
  filter(monitoring_level_grouped %in% c("Lowest Levels", "Medium Levels", "Highest Levels")) %>%
  mutate(monitoring_level_grouped = ordered(monitoring_level_grouped, c("Highest Levels", "Medium Levels", "Lowest Levels"))) %>%
  count(county, monitoring_level_grouped) %>%
  pivot_wider(names_from = monitoring_level_grouped, values_from = n)
...
join_df %>%
  filter(is_released_pretrial == "Yes",
         release_type %in% c("Own Recognizance", "Pretrial Monitor")) %>%
  select(county, court_date_reminder_flag, contains("condition")) %>%
  group_by(county) %>%
  summarise(across(everything(), ~round(100*mean(.x, na.rm = T),1)))



```
Look at FTA rates with and without court date reminders
```{r}
join_df %>%
  filter(is_released_pretrial == "Yes",
         release_type %in% c("Own Recognizance", "Pretrial Monitor")) %>%
  group_by(county, court_date_reminder_flag, risk_score) %>%
  summarise(fta_rate = sum(fta_county_or_doj)/sum(booking_assessed_count))
```
...
join_df %>%
  filter(is_released_pretrial == "Yes") %>%
  select(county, contains("condition"), court_date_reminder_flag) %>%
  group_by(county, condition_phone_flag) %>%
  count()

snow_df %>%
  mutate(condition_phone_flag_mod = if_else(grepl("^Y", toupper(condition_phone_flag)) | condition_phone_flag == 1, 1,0)) %>%
  count(condition_phone_flag_mod, condition_phone_flag)
```
...
join_df %>%
  ...
  filter(is_released_pretrial == "Yes",
         recid_arrested_flag_doj == 1)  %>%
  mutate(days_to_recid = as.Date(recid_date_doj) - release_date) %>%
  filter(days_to_recid > 100000) %>%
  select(booking_date, release_date, recid_date_doj)

join_df %>%
  filter(release_date < as.Date("2000-01-01")) %>%
  select(county, release_date)

join_df %>%
  filter(release_date < booking_date) %>%
  select(county, booking_date, release_date)
```

```{r}
join_df %>%
  mutate(booking_charge_std = gsub("(|)|-|\\.|", "", booking_charge)) %>%
  filter(booking_charge != booking_charge_std) %>%
  count(booking_charge, booking_charge_std) %>%
  arrange(desc(n))

join_df %>%
  mutate(booking_charge_std = gsub("\\(|\\)|-|\\.| ", "", booking_charge)) %>%
  count(booking_charge_code, booking_charge_level, booking_charge_std) %>%
  arrange(desc(n))
```
