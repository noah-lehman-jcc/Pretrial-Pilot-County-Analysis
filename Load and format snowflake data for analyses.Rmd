...
```
Golive is significant (p<.0001) and negative (-0.096)
10,032 observations
...
```
Golive significant (p < .001) and negative (-0.091) 
8084 observations
...
nobs(lm(data = full_bts %>%
           ...
           fta_flag ~ days_released*avg_bts_mob + golive_flag + covid_flag + eb_flag + emerg_flag + charge_hier + county))
```
Golive not significant (p = 0.9) and positive (0.0016)
8084 observations
...
full_bts %>%
  ...
  group_by(county) %>%
  mutate(tot = sum(n), perc = n/tot) %>%
  filter(charge_unknown == 1) %>%
  select(county, perc) %>%
  mutate(perc = scales::percent(perc, accuracy = 1)) %>%
  rename(percent_unknown_charge = perc))
```
