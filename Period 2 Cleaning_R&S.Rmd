...
assess_df_case <- assess_df_case %>%
    mutate (release_recommendation = case_when(release_recommendation_original %in% c("Release on Own Recognizance") ~ "OR", 
                                             release_recommendation_original %in% c("Pre-Arraignment Release", "Release at Arraignment") ~ "Monitor",
                                             release_recommendation_original %in% c("Detain(PT denied)", "If released, maximum conditions apply") ~ "Detain", 
                                             TRUE ~ "Other_Unknown"),
        ...
        # pretrail_release_type will come from jail data
...
assess_df_case %>%
  count(pretrial_release_type_original, pretrial_termination_outcome_original) %>%
  view
...
test<-assess_df_score %>%
   count(pretrial_id2) %>%
   filter (n>1)

assess_df_score %>%
  filter (grepl("4000011", pretrial_id2))

test2 <- assess_df_case %>%
  distinct()

#   
#   semi_join(assess_df_score, by= c("pretrial_id2")) %>%
#   distinct(pretrial_id2)
...
#Standardize Factors
  ##ask if violation reasons (arrest, FTR FTA are only for cases that resulted in termination of supervision)
#assess_df_viol <- assess_df_viol %>%
  # mutate(pretrial_termination_reason = case_when(pretrial_violation_original == "Arrest" ~ "New_Crime",
  #                                                pretrial_violation_original == "FTA" ~ "FTA",
  #                                                pretrial_violation_original == "FTR" ~ "Violation",
  #                                                TRUE ~ "Other_Unknown"))
...
#Standarize Flags
assess_df_viol <- assess_df_viol %>%
  mutate(pretrial_violation_flag = if_else(pretrial_violation_original !="NULL", 1,0))
...
```{r}
##this is a first run at joining tables we need to ask probation what IDs mean and do it better
assess_df_score <- assess_df_score %>%
...
assess_df_test <- assess_df %>%
  mutate(monitor_level = ifelse(release_decision == "Monitor", monitor_level, NA  ))


assess_df_test %>%
  count( pretrial_termination_outcome_original, pretrial_termination_reason, pretrial_violation_flag)
...
factor_df <- lapply(c("assess_df_case", "assess_df_demo", "assess_df_score"), function(i){
  df <- get(i) %>%
...
flag_df <- lapply(c("assess_df_case", "assess_df_terms", "assess_df_viol"), function(i){
  df <- get(i)
...
write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Nevada Assessment Code Book 10-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
```
