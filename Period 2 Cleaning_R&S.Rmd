...
#Jail Bookings Data Import--Nevada Jail Data 1/1/2015 to 7/27/2020
  ...
  ## ??? this has 20 vriables this is the new data it came through, but the dates for arrest, book and rls came in as times only, also    ##there were 39 variables in   the extract Cory sent, rather than 19 as before.  I asked cory to give us all the data back to 1/1/2015  ##with the dates corrected in     the 39 variable format.
...
```{r}
##this is a first run at joing tables we need to ask probation what IDs mean and do it better
assess_df_score <- assess_df_score %>%
...
# Variable name standardization--See pg. xxx for more details.
names(assess_df_new) <- c("local_id", "book_num", "arrest_date", "book_date_time", "last_name", "first_name", "middle_name", "dob", "sex_original", "cii", "fbi", "ipid", "cdl_id", "hispanic_flag", "race_original", "zip_code", "assessment_date_time",  "tool_name",  "vprair_tool_response_1", "vprair_tool_response_2", "vprair_tool_response_3", "vprair_tool_response_4", "vprair_tool_response_5", "vprair_tool_response_6", "vprair_tool_response_7", "vprair_tool_response_8",   "vprair_risk_score", "pretrial_referral_disposition",  "release_recommendation_original", "release_type_at_arraign", 
                      "condition_drug_test_original", "conditions_cam_monitoring_original", "condition_gps_original", "condition_home_detention_original", "condition_curfew_original", "condition_search_original", "condition_stay_away_original", "condition_other_original", "pretrial_signature_date_time", "num_fta" , "num_new_arrests", "office_ftas", "scram", "gps", "total_tc_violations", "num_court_reminders", "pretrial_termination_date",  "pretrial_termination_outcome_original")
