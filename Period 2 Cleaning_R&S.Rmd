...
##there is a supervion release type--Basic=phone 1 time/wk Regualar=inperson 1 time/wk, Enhanced=come in 1 time wk and additional conditions 6/7/2020 notes
...
assess_df <- assess_df %>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         release_decision = case_when(pretrial_release_type_original %in% c("OR") ~ "OR",
          ...
          risk_level = case_when(vprair_risk_score %in% c("0", "1", "2", "3", "4") ~ "Low",
                                  ...
                                  vprair_risk_score %in% c( "9", "10" , "11", 
                                                           "12", "13", "14") ~ "High"),
          pretrial_release_type = case_when(pretrial_release_type_original %in% c("OR") ~ "OR",
                                      pretrial_release_type_original %in% c("MOR Basic", "MOR BASIC", "Mor Enhanced", 
                                                                   "MOR Enhanced", "MOR Regular") ~ "Monitor",
                                      pretrial_release_type_original %in% c("Denied") ~ "Detain",
...
# Flag Standardization---See pg. xxx for more details.
  ...
  ## all pretrail termination dates are missing
...
assess_df %>%
  ...
  arrange(desc(n))
assess_df %>%
  count(release_conditions_original, pretrial_release_type_original) %>%
  arrange(desc(n))
```
...
flag_df <- flag_df %>%
  filter(modified_value == 1) %>%
  mutate(value = case_when(
  modified_field == "condition_drug_test_flag" & modified_value == "1" ~ "MATCHES (TEST)",
...
write.xlsx(list(factor_df, flag_df, termination_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/San Mateo Assessment Code Book 9-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Terminations", "Dates"))
```
