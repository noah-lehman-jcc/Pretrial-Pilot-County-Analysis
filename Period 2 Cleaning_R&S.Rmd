...
assess_df_terms <- assess_df_terms %>%
         ...
         ## Are all violations violations that result in termination? 
...
flag_df <- flag_df %>%
  ...
  modified_field == "pretrial_violation_flag" &
...
assess_df_new <- assess_df_new%>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         release_recommendation = case_when(release_recommendation_original %in% c("MOR Basic", "MOR BASIC", 
                                             ...
                                             T  ~ "Other_Unknown"),
         monitor_level = case_when(release_recommendation_original %in% c("MOR Basic", "MOR BASIC") ~ "Low",
                                   release_recommendation_original %in% c("MOR Regular") ~ "Medium",
                                   release_recommendation_original %in% c("Mor Enhanced", "MOR Enhanced") ~ "High",
                                   T  ~ "Other_Unknown"),                                         
...
# San Mateo Assessment Data Import
  ...
  ## SM said local_id is the number their jail uses, but their original jail extract does not have it. On         ##10/6/2020 their jail submitted data with the local_id (still need to check for match).  Probation currently ##doesnt have court_case_id, booking_num, or arrest_date 
