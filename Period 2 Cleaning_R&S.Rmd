...
assess_table <- assess_df %>%
  ...
  mutate(county = "Nevada", risk_score = oras_risk_score) %>%
  # Select relevants variables.
  select(county, release_recommendation, release_decision, assessment_date, risk_score, tool_name, assessment_date, dob, sex, race, risk_level,  pretrial_termination_reason)
...
# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Nevada Assessment Output 10-2020.csv")
...
write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Nevada Assessment Code Book 9-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
```
...
#we collapsed to a person date in the assessment data using local_id and assesmnet date.
...
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, risk_score, tool_name, assessment_date, dob, sex, race, risk_level,  pretrial_termination_reason, charge_level)
...
# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/San Mateo Assessment Output 10-2020.csv")
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      ##field == "release_authorization_original" ~ "release_authorization",
      field == "pretrial_termination_outcome_original" ~ "pretrial_termination_reason,  pretrial_violation_flag",
      ##field == "court_date_reminder_original" ~ "court_date_reminder_flag",
      ##field =="other_pretrial_service_original" ~ "transit_service_flag",
      field =="vprair_risk_score" ~ "risk_level",
      field =="pretrial_violation_type_original" ~ "pretrial_termination_reason")) %>%
  filter(!is.na(modified_field)) %>%
  separate_rows(modified_field, sep = ",") %>%
  mutate_all(trimws)
...
flag_df <- flag_df %>%
  filter(modified_value == 1) %>%
  mutate(value = case_when(
  modified_field == "condition_drug_test_flag" & modified_value == "1" ~ "MATCHES (TEST|TST)",
  # modified_field == "condition_em_flag" & modified_value == "1" ~ "MATCHES (ELECTRONIC MONITOR|GPS|SCRAM)",
  # modified_field == "court_date_reminder_flag" & modified_value == "1" ~ "CourtAppearance / (PTS with court reminders, OR-with court reminders)",
...
termination_df <- assess_df %>%
  count(pretrial_termination_outcome_original, pretrial_violation_type_original, pretrial_termination_reason) %>%
  arrange(desc(n))  
...
write.xlsx(list(factor_df, flag_df, termination_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/San Mateo Assessment Code Book v2 9-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Terminations", "Dates"))
```
