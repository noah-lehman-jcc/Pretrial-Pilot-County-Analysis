...
book_df<- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/P2 Submission-June 30/Jail Data Extract2.xlsx", col_types = "text") %>%
  distinct () 
...
assess_df_case <- assess_df_case %>%
    mutate (release_recommendation = case_when(release_recommendation_original %in% c("Release on Own Recognizance") ~ "OR", 
        ...
        pretrial_termination_outcome = case_when(pretrial_termination_outcome_original %in% c("Charges Dismissed", "No Charges Filed", "Sentenced") ~ "Successful",
                                                 pretrial_termination_outcome_original %in% c("Failure to Appear", "Felony Arrest", "Technical Violation") ~ "Unsuccessful",
                                                 pretrial_termination_outcome_original %in% c("Bond Posted") | pretrial_release_type_original %in% c("Release on Own Recognizance") ~ "Not_Tracked",
                                                 TRUE ~ "Other_Unknown"),
...
assess_df_case <- assess_df_case %>%
 mutate(court_date_reminder_flag = case_when(court_date_reminder=="Yes" ~ 1,
                                              TRUE ~ 0),
...
## attempted to join these data sets based pretrial_id1 and pretrial_id2 need to consult with probation
# assess_df_case %>%
#   add_count(pretrial_id2) %>%
#   filter (n>1)
#   
...
##need to fix this not sure what is wrong
