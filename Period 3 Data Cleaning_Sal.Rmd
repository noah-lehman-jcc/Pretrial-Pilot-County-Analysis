...
assess_df <- assess_df %>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         vpraio_tool_response_1 = if_else(trimws(vpraio_tool_response_1) == "Felony", 1, 0),
         vpraio_tool_response_2 = if_else(trimws(vpraio_tool_response_2) == "Yes", 1, 0),
         vpraio_tool_response_3 = if_else(trimws(vpraio_tool_response_3) == "Yes", 1, 0),
         vpraio_tool_response_4 = if_else(trimws(vpraio_tool_response_4) == "Yes", 2, 0),
         vpraio_tool_response_5 = if_else(trimws(vpraio_tool_response_5) == "Yes", 1, 0),
         vpraio_tool_response_6 = if_else(trimws(vpraio_tool_response_6) == "Yes", 1, 0),
         vpraio_tool_response_7 = if_else(trimws(vpraio_tool_response_7) == "Less than 1 Year", 1, 0),
         vpraio_tool_response_8 = if_else(trimws(vpraio_tool_response_8) == "Yes", 1, 0),
         vpraio_tool_response_9 = if_else(trimws(vpraio_tool_response_9) == "Yes", 1, 0))
...
#write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Kings Assessment Code Book 3-21.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))

```

Join across agencies (Updated 3.9.21)
```{r}
...
# Variable name standardization--See pg. xxx for more details. 
## Their id variables appear to be mislabeled, the first column is labeled cii but is not appear to be a cii, while the second column is labeled fbi but appears to be cii
names(person_df) <- c("unknown_id", "cii", "local_id", "cdl_id", "cdl_id_state", "name", "dob", "race_original", "sex_original") 
...
book_df <- book_df %>%
  mutate(release_date = as_date(convertToDateTime(release_date))) %>%
...
book_df %>%
  ...
  ungroup() %>%
  count(book_num, book_date, release_date)

book_df %>%
  mutate(release_date = as_date(convertToDateTime(release_date))) %>%
  filter(book_num == "135398") %>%
  count(book_num, release_date)
```
...
book_df <- left_join(book_df, person_df %>% mutate(local_id = as.numeric(local_id)))
...
# Since Sac uses a unified system, the person df will be standardized and joined after the jail-court join
...
# unified system, so court_person will be joined after the jail-court join
...
prob_df <- prob_df %>%
  mutate(tool_name = "psa",
         ...
         psa_tool_response_2a = if_else(psa_tool_response_1 <= 20 & psa_tool_response_2 == "Yes", 1, 0),
         psa_tool_response_1 = if_else(psa_tool_response_1 >= 23, 0, 2),
         ...
         psa_tool_response_5a = if_else(psa_tool_response_4 == 1 | psa_tool_response_5 == 1, 1, 0),
...
Probation Data
```{r}
# Probation Data Import--- Sonoma, 2015-01-01 to 2021-02-27
...
prob_person <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset4_revised_Individuals.csv.filepart", stringsAsFactors = FALSE) #this appears to be a person table
...
prob_book <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset1_revised_AssessmentsAndGrants.csv.filepart", stringsAsFactors = FALSE)
...
names(prob_book) <- c("record_type", "assessment_id", "lent_key", "pretrial_grant_id", "tool_name", "assessment_date_time", "assessment_custody_status",
                      "book_num", "book_release_date_time", "book_release_type_original", "city", "zip_code", "pretrial_start_date_time",
                      "pretrial_termination_date_time", "pretrial_release_level_original", "other_pretrial_service", "ptr_grant_prearraignment",
                      "release_authorization", "outcome_fta_signup", "outcome_fta_court", "outcome_new_arrest")

prob_book <- prob_book %>%
  mutate(across(c(assessment_date_time, book_release_date_time, pretrial_start_date_time, pretrial_termination_date_time), mdy_hm),
         assessment_date = as_date(assessment_date_time),
         book_release_date = as_date(book_release_date_time),
         pretrial_start_date = as_date(pretrial_start_date_time),
...
prob_sprat <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset2_revised_SPRATassessments.csv.filepart", stringsAsFactors = FALSE)
...
prob_psa <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset3_revised_PSAassessments.csv.filepart", stringsAsFactors = FALSE)
...
prob_psa <- prob_psa %>%
  mutate(risk_level = case_when(presumptive_release_level == "Release Level 1" ~ "Low",
         ...
         psa_tool_response_6 = case_when(psa_tool_response_6 == "None" ~ 0,
                                         psa_tool_response_6 %in% c("1 or 2") ~ 1,
                                         psa_tool_response_6 == "3 or more" ~ 2),
...
prob_violations <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset5_revised_PretrialViolations.csv.filepart", stringsAsFactors = FALSE)
...
prob_conditions <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset6_revised_TermsandConditions.csv.filepart", stringsAsFactors = FALSE)

names(prob_conditions) <- c("lent_key", "pretrial_grant_id", "release_conditions_original", "release_condition_type")
...
prob_reminders <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset7_revised_CourtDateReminders.csv.filepart", stringsAsFactors = FALSE)
...
prob_reminders <- prob_reminders %>%
  mutate(across(c(court_date_time, notification_attempt_date_time), mdy_hms),
...
assess_table <- assess_df %>%
  ...
  rename(sprat_risk_score = risk_score) %>%
  # Select relevants variables.
  select(county, monitor_level, release_recommendation, release_decision, assessment_date,
matches("risk|score"), tool_name, assessment_date, dob, sex, race, pretrial_termination_reason, pretrial_ineligible_reason)
