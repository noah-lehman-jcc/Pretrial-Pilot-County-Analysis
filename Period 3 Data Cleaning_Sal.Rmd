...
# Jail Bookings Data Import --- Kings, 2015-01-01 to 2020-6-30
  ...
  ## Extracts are based solely on book_date, so any resolution of earlier bookings would not be included in newer extracts, ask them to rectify this in the future

###

# Jail Bookings
  ## book_df contains one row for each combination of book_num, arrest_charge_original, arrest_charge_count ,court_case_id, disposition_date, disposition_type_original. It contains information on the booking
# In P3 Kings provided cumulative booking data from 2015 through P3

book_df <- fread("Historical Data Submissions/Kings/P3 Submission-Jan 15/Pretrial Pilot Program 2015-2019.csv")
...
save(book_df, file = "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Kings Booking Collapse.RData")
```
...
save(court_df, file = "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Kings Court Collapse.RData")
...
Probation Data
```{r}
assess_df <- read_excel("Historical Data Submissions/Kings/P3 Submission-Jan 15/KINGS COUNTY - PRETRIAL DATA - 01.01.19 - 12.31.20.xlsx", skip = 1)
...
save(assess_df, file = "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Kings Assessment Collapse.RData")
...
# Save file  
save(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Kings Assessment Output 02-2021.RData")
...
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/February 2021/Kings P3 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
```
...
# person_df2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020_2.xlsx", sheet = 1)
person_df <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 1, col_types = "text") %>%
  select(-SupervisionLevel)

#de-duplicating data from multiple extracts
# person_df <- bind_rows(person_df, person_df2, person_df3) %>%
#   distinct()
# 
# rm(person_df2, person_df3)

# Variable name standardization--See pg. xxx for more details. 
names(person_df) <- c("cii", "fbi", "local_id", "cdl_id", "cdl_id_state", "name", "dob", "race_original", "sex_original") 
...
#   select(1:2, 4:8)
# book_df2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020_2.xlsx", sheet = 2, col_types = "text")
book_df <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 2, col_types = "text")


#remove duplicates across data extracts, selectively keeping ones from second extract which includes arrest dates
# book_df <- anti_join(book_df, book_df2, by = "Booking_Number")
# 
...
# book_charge2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020_2.xlsx", sheet = 3, col_types = "text")
book_charge <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 3, col_types = "text")

#remove duplicates across data extracts, selectively keeping ones from later extract
# book_charge <- anti_join(book_charge, book_charge2)
# 
# book_charge <- bind_rows(book_charge, book_charge2)
# rm(book_charge2)
...
save(book_df, file = "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Booking Collapse.RData")
```
...
# da_df2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 4)
da_df <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 4, col_types = "text")

#Removing duplicates between submission 1 and 2 with releases during the P2 time period
# da_df <- da_df %>%
# anti_join(da_df2, by = "Court_Case_ID")
# 
# da_df <- bind_rows(da_df, da_df2) %>% 
#   distinct()
# 
# rm(da_df2)
...
# Date Reformatting--See pg. xxx for more details.
da_df <- da_df %>%
  mutate(file_date_time = convertToDateTime(file_date),
         case_status_date_time = convertToDateTime(case_status_date)) %>%
  mutate_at(vars(file_date, case_status_date), convertToDate)
...
# court_charge2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 8)
court_charge <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 8, col_types = "text")


#Removing duplicates between submission 1 and 2 with releases during the P2 time period
# court_charge <- court_charge %>%
# anti_join(court_charge2, by = "Court_Case_ID")
# 
# court_charge <- bind_rows(court_charge, court_charge2) %>% 
#   distinct()
# 
# rm(court_charge2)
...
# court_war2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 10)
court_war <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 10, col_types = "text")

#Removing duplicates between submission 1 and 2 with releases during the P2 time period
# court_war <- court_war %>%
# anti_join(court_war2, by = "Court_Case_ID")
# 
# court_war <- bind_rows(court_war, court_war2) %>% 
#   distinct()
# 
# rm(court_war2)
...
#date conversion
court_war <- court_war %>%
  mutate(war_date = convertToDate(war_date))
...
#   distinct()
# court_fta2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 11) %>%
#   distinct()
court_fta <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 11, col_types = "text") %>%
  distinct()

#Removing duplicates between submission 1 and 2 with releases during the P2 time period
# court_fta <- court_fta %>%
# anti_join(court_fta2, by = "Court_Case_ID")
# 
# court_fta <- bind_rows(court_fta, court_fta2) %>% 
#   distinct()
# 
# rm(court_fta2)
...
# court_hearing2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 9)
court_hearing <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_1.xlsx", sheet = 9, col_types = "text")

#Removing duplicates between submission 1 and 2 with releases during the P2 time period
# court_hearing <- court_hearing %>%
# anti_join(court_hearing2, by = "Court_Case_ID")
# 
...
# Date Reformatting--See pg. xxx for more details.
court_hearing <- court_hearing %>%
  mutate(hearing_date_time = convertToDateTime(hearing_date_time),
...
court_df2 <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 7)

#Removing duplicates between submission 1 and 2 with releases during the P2 time period
court_df <- court_df %>%
anti_join(court_df2, by = "Court_Case_ID")

court_df <- bind_rows(court_df, court_df2) %>% 
  distinct()
...
save(court_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Court Collapse.RData")
```
...
prob_df <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_ProbData_3.xlsx", sheet = 5, col_types = "text")


# Variable name standardization--See pg. xxx for more details. 
names(prob_df) <- c("pretrial_id", "local_id", "zip_code", "assessment_id", "assessment_date", "item_label", "assessment_item", "assessment_response", "risk_score", "risk_level_original")
...
prob_df <- prob_df %>%
  ...
  mutate(item_label = round(as.numeric(item_label), digits = 1)) %>%
...
#   distinct()
prob_bookings <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_ProbData_3.xlsx", sheet = 1, col_types = "text") %>%
  distinct()
...
prob_reports <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_ProbData_3.xlsx", sheet = 2, col_types = "text")
...
prob_reports <- prob_reports %>%
  mutate(report_date_time = convertToDateTime(report_date_time),
         report_date = as_date(report_date_time))
...
prob_case <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_ProbData_3.xlsx", sheet = 3, col_types = "text")
...
prob_case <- prob_case %>%
  mutate(case_status_date_time = convertToDateTime(case_status_date_time),
         case_status_date = as_date(case_status_date_time))
...
prob_cond <- read_excel("Historical Data Submissions/Napa/P3 Submission-Jan 15/Historical_12-31-2020_ProbData_3.xlsx", sheet = 4, col_types = "text")
...
```{r}
prob_cond <- prob_cond %>%
  group_by(local_id, pretrial_id, court_case_id) %>%
  summarise_all(max)

prob_case <- prob_case %>%
  group_by(local_id, pretrial_id, court_case_id) %>%
  filter(case_status_date == max(case_status_date))
...
save(assess_df, file = "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Assessment Collapse.RData")
# assess_df %>%
#   filter(!is.na(assessment_date))
#   distinct(pretrial_id)
#   
...
# assess_df %>%
#   distinct(local_id, assessment_id, court_case_id)
```
...
assess_table <- assess_df %>%
  ...
  # Filter for only those actually assessed
  filter(!is.na(assessment_date)) %>%
  left_join(book_join %>% mutate(book_num = as.character(book_num))) %>%
  # Create county name.
  mutate(county = "Napa",
         pretrial_termination_outcome = NA_character_,
         pretrial_termination_reason = NA_character_,
...
# Save file  
#
save(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Napa Assessment Output 02-2021.RData")
...
Join
  Jail Court Join
```{r}
assess_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Assessment Collapse.csv") %>%
  mutate(across(contains("time"), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date))
book_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Booking Collapse.csv")%>%
  mutate(across(contains("time"), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date))
court_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Court Collapse.csv")%>%
  mutate(across(contains("time"), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date))


#joining court df to book df on court case id and local id, match rate = 99.5%
...
#joining assess_df to joined booking and court data on local id and assessment date within 2 days of booking, match rate = 78.8%
...
nrow(assess_df %>%
  ...
  distinct(local_id, pretrial_id, court_case_id, book_num, assessment_id, .keep_all = T) )/nrow(assess_df %>% distinct(local_id, pretrial_id, court_case_id, book_num, assessment_id, .keep_all = T))
...
join_df2 <- join_df %>%
  ...
  fuzzy_left_join(assess_df, 
                by=c("local_id", "book_date"="assessment_date", "bookplus_date"="assessment_date"),
                match_fun=list(`==`, `<=`, `>=`))%>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .)))
...
join_collapse <- join_df2 %>%
  ...
  ungroup() %>%
  filter(final_disposition_date >= book_date, book_type %in% c("On_View", "New_Warrant")) %>%
  group_by(local_id, book_date) %>%
  mutate(across(contains("flag"), max)) %>%
  ungroup() %>%
  distinct(local_id, book_date, .keep_all = T)
  
#fwrite(join_collapse, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Napa Join Collapse.csv")
```

Old and diagnostic code
```{r}

#old code for cycle stuff
  #mutate(cycle = if_else(book_type %in% c("New_Warrant", "On_View"), 1, 0)) %>%
  #arrange(book_date, desc(cycle)) %>%
  #group_by(local_id) %>%
  #mutate(cycle = cumsum(cycle)) %>%
  #group_by(local_id, cycle) %>%
  #mutate(across(contains("flag"), max))

  ungroup() %>%
  select(cycle, local_id, book_type, book_date) %>%
  arrange(local_id, book_date)

join_collapse %>%
  summarise(across(contains("flag"), sum))
...
join_df2 %>%
  mutate(across(contains("flag"), ~if_else(is.na(.x), as.integer(0), .x))) %>%
  group_by(local_id, book_date) %>%
  mutate(num_ass = sum(assess_flag)) %>%
  count(num_ass) %>%
  arrange(desc(num_ass))

join_df2 %>%
   filter(final_disposition_date >= book_date, book_type %in% c("On_View", "New_Warrant")) %>%
  count(assessment_id)
  anti_join(join_collapse, by = "assessment_id", na_matches = "never") %>%
  filter(assess_flag == 1) %>%
  filter(assessment_id == 58134)
  

```


Old crap and various diagnostics for join
```{r} 
 
test <- fuzzy_semi_join(assess_df, join_df, 
                by=c("local_id", "assessment_date"="book_date", "assessment_date"="book_date"+2),
                match_fun=list(`==`, `>=`, `<=`))
# join_df2 <- join_df %>%
#  left_join(assess_df, by = c("court_case_id", "local_id"), na_matches = "never") %>%
#   select(-contains(".y")) %>%
#   rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .)))
...
assess_df %>%
  select(-book_date) %>%
  left_join(join_df, by = c("local_id", 'book_num'), na_matches = "never") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%
  mutate(time_dif = assessment_date - book_date) %>%
  count(time_dif) %>%
  arrange(time_dif)
...
summary(as_date(assess_df$assessment_date))

assess_df %>%
  add_count(local_id, pretrial_id, court_case_id, book_num, assessment_id) %>%
  filter(n>1) %>%
  arrange(local_id)
...
#Create an excel workbook with all data table. Make sure to include the cii table first.
write.xlsx(list(doj_cii, doj_person), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/February 2021/Napa P3 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "integrated"))
```
...
# Jail Bookings
  ## no overlap in booking numbers between historical and P2 data, will need updated data from older bookings
book_charge <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacBookingDetail.csv")


names(book_charge) <- c("case_id", "book_date_wrong", "release_date_wrong", "book_num", "registry_sub_num", "book_type_code", "book_type_original", "arrest_charge_level_original", "arrest_charge_original", "arrest_charge_description", "release_type_code", "release_type_original", "release_comment", "charge_date", "charge_release_date", "court_combined_id", "court_case_id", "court_docket_id", "release_line")
...
#Booking Dates
book_df <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacBookingMain.csv")


names(book_df) <- c("jail_court_id", "book_num", "book_date", "release_date", "hours_detained", "days_detained")
...
#Jail Bail Amounts
book_bail <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacBail.csv")


names(book_bail) <- c("jail_court_id", "book_num", "registry_sub_num", "book_type_code", "bail_amt")

#need to ask Sac about P3 book charge data, looks like it is only a partial file
...
save(book_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Booking Collapse.RData")
...
# Court Case Data Import--- Sacramento   min "1901-01-01" 1st qu "2015-09-16" to max 2020-08-07

#High level casees
court_df <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacCases.csv")


# Variable name standardization--See pg. xxx for more details. 
names(court_df) <- c("jail_court_id", "court_combined_id", "court_id", "court_case_id", "file_date", "case_status", "case_status_desc", "case_status_date")
...
#Court Hearing Data
court_hearing <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacAppear.csv")

# court_hearing_mult <- lapply(1:4, function(i){
#   fread(paste0("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacAppear0", i, ".csv"))
# }) %>% bind_rows() 
#they also gave us the same file but broken into 4 parts, but we checked and combined they are identical to the one large file

# Variable name standardization--See pg. xxx for more details. 
names(court_hearing) <- c("jail_court_id", "court_combined_id", "court_id", "court_case_id", "hearing_date", "hearing_type_original", "hearing_strange", "hearing_comment", "hearing_result_code", "hearing_result", "hearing_result_comment")
...
court_dispos <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacDispos.csv")

# Variable name standardization--See pg. xxx for more details. 
names(court_dispos) <- c("jail_court_id", "court_combined_id", "court_id", "court_case_id", "plea_type_original", "disposition_type_original", "case_charge_code", "court_charge_level_original", "court_charge_original", "disposition_date")
...
#Court Warrants
court_warrants <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacWarrants.csv")

# Variable name standardization--See pg. xxx for more details. 
names(court_warrants) <- c("jail_court_id", "court", "warrant_num", "warrant_id", "warrant_status", "charge_level", "warrant_type", "judge", "judge_comment", "complaint_lea", "lea_loc", "lea", "lea_foreign", "lea_hold", "charges_comments", "warrant_date", "warrant_clear_date", "warrant_duration", "bail_amt", "no_bail_flag", "court_case_id", "mandatory_appear_flag")
...
# court_df %>%
#   #distinct(court_case_id, .keep_all = T) %>%
#   anti_join(court_dispos, by = "court_case_id")
# 
# court_dispos %>%
#   #distinct(court_case_id) %>%
...
#come back to this, needs to be standardized but format is unusual and needs some work to parse (type of sentence divided into columns for each possibility and sub-type)

court_sentence <- fread("Historical Data Submissions/Sacramento/P3 Submission-Jan 15/SacSentence.csv")

names(court_sentence) <- c("jail_court_id", "court_combined_id", "court_id", "court_case_id", "charge_code_num", "sentence_ver_num", "sentence_type", "sentence_date_time", "sentence_correction_date", "sentence_reverse_delete", "prob_status", "prob_status_desc", "prob_type", "prob_supervision", "local_time_type", "local_time_type_desc", "local_time_term", "sentence_suspended", "non_local_type", "non_local_term", "non_local_suspended", "local_time_status", "non_local_time_status")

court_sentence <- court_sentence %>%
  mutate(sentence_date_time = parse_date_time(sentence_date_time, c('%m/%d/%Y %H:%M:%S %p')),
         sentence_date = as_date(sentence_date_time))



#ask Sac about court hearings. There is a huge drop off in matched hearings as the dates get closer to 2020. Also the hearing dates are mostly in 2014 and 2016, with weird patterns after that (barely any from 2018, but then a bunch in 2020)
...
court_fta_war <- court_warrants %>%
  mutate(court_id = case_when(court == "SACRAMENTO MUNICIPAL" ~ "34470", 
                              ...
                              T ~ ""),
         #court_case_id = str_c(court_id, court_case_id),
...
court_hearings <- court_hearing %>%
  ...
  arrange(arraignment_date_time) %>%
  distinct(court_case_id, jail_court_id, .keep_all = T) %>%
  select(court_case_id, jail_court_id, court_id, court_case_id, arraignment_date_time, arraignment_date, court_fta_nowar_flag, court_fta_nowar_flag_count)
#

court_sentence <- court_sentence %>%
  distinct(court_case_id, jail_court_id, .keep_all = T)

court_df <- reduce(list(court_dispo, court_fta_war, court_hearings, court_sentence), left_join, by = c("court_case_id", "jail_court_id")) %>%
...
save(court_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Court Collapse.RData")
...
# prob_df %>%
#   count(pretrial_termination_outcome_original, release_decision_original)
...
prob_df <- prob_df %>%
  mutate(tool_name = "PSA",
         ...
         pretrial_ineligible_reason = case_when(pretrial_termination_outcome_original %in% c("Bailed", "Zero Bail") ~ "Bail_Bond",
                                                release_decision_original %in% c("Dropped") ~ "Case_Disposed",
                                                T ~ "Other_Unknown"),
         
         ## we assigned monitoring levels based on our own logic looking at their RCM and commonalities in conditions
         monitor_level = case_when(monitor_level_original %in% c("1") ~ "Low",
                                   monitor_level_original %in% c("2", "3", "4") ~ "Medium",
                                   monitor_level_original %in% c("5", "6") ~ "High"),
...
prob_df <- prob_df %>%
  mutate(assess_flag = if_else(is.na(assessment_date), 0, 1),
         ...
         # condition_inperson_flag = if_else(),
         condition_drug_test_flag = if_else(grepl("SCRAM|TEST", toupper(release_conditions_original)), 1, 0),
         condition_no_contact_flag = if_else(grepl("NO CONTACT|NO DIRECT OR INDIRECT CONTACT|PROTECTED PERSON", toupper(release_conditions_original)), 1, 0),
         court_date_reminder_flag = if_else(court_date_reminder_original == "Y", 1, 0))
         # Transit service doesn't actually exist here.
         #transit_service_flag = if_else(grepl("BUS", toupper(other_pretrial_service_original)), 1, 0),
         ## Is there any violation data?
         #pretrial_violation_flag = if_else(!is.na(pretrial_violation_original), 1, 0))
...
#   separate_rows(release_conditions_original, sep = "\\|\\|") %>%
#   count(release_conditions_original) %>%
#   arrange(desc(n))

# prob_df %>%
#   separate_rows(release_conditions_original, sep = "\\|\\|") %>%
#   count(release_conditions_original) %>%
#   filter(grepl("MONITOR|GPS|ELECTRONIC", toupper(release_conditions_original)))
...
save(assess_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Assessment Collapse.RData")
```


Assessment code book (10/2020) 
```{r}
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      field == "release_decision_original" ~ "release_decision, pretrial_ineligible_reason",
      field == "pretrial_termination_outcome_original" ~ "pretrial_termination_outcome, pretrial_termination_reason, pretrial_ineligible_reason",
...
factor_df <- lapply(c('prob_df'), function(i){
  df <- get(i)
...
factor_df <- factor_df %>%
  mutate(value = case_when(
    modified_field == "pretrial_ineligible_reason" & modified_value == "Bail_Bond" ~ "pretrial_termination_outcome_original = 'Bailed' OR 'Zero Bail'",
    modified_field == "pretrial_ineligible_reason" & modified_value == "Case_Disposed" ~ "pretrial_termination_outcome_original != 'Bailed' OR 'Zero Bail' AND release_decision_original = 'Dropped'",
    modified_field == "pretrial_ineligible_reason" & modified_value == "Other_Unknown" ~ "pretrial_termination_outcome_original != 'Bailed' OR 'Zero Bail' AND release_decision_original != 'Dropped'",
    T ~ value)) %>%
  group_by(modified_field, value) %>%
  mutate_at(vars(original_field, field), funs(paste(., collapse = ", "))) %>%
  distinct(modified_field, value, .keep_all = T) %>%
...
flag_df <- factor_df
factor_df <- factor_df %>%
  filter(!grepl("flag", modified_field))

# flag_df <- lapply(c('prob_df'), function(i){
#   df <- get(i)
#   
#   df <- lapply(c(1:nrow(names_df))[names_df$field %in% names(df)], function(j){
#     df <- df %>%
...
flag_df <- flag_df %>%
  filter(modified_value == 1) %>%
  mutate(value = case_when(
  modified_field == "condition_no_contact_flag" & modified_value == "1" ~ "MATCHES (NO DIRECT OR INDIRECT CONTACT, NO CONTACT, PROTECTED PERSON)",
  modified_field == "condition_em_flag" & modified_value == "1" ~ "MATCHES (SCRAM, ELECTRONIC MONITOR, GPS)",
  modified_field == "condition_drug_test_flag" & modified_value == "1" ~ "MATCHES (TEST, SCRAM)",
  #modified_field == "pretrial_violation_flag" & modified_value == "1" ~ "IF pretrial_violation_original EXISTS",
  #modified_field == "court_date_reminder_flag" & modified_value == "1" ~ "court_date_reminder_original IS (Y)",
  T ~ value)) %>%
  group_by(modified_field, value) %>%
  mutate_at(vars(original_field, field), funs(paste(., collapse = ", "))) %>%
  distinct(modified_field, value, .keep_all = T) %>%
...
#write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Sacramento Assessment Code Book 11-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
...
assess_table <- prob_df %>%
  ...
  # Filter for only those actually assessed
  filter(!is.na(assessment_date)) %>%
  left_join(book_df %>% select(book_num, charge_level), by = "book_num")  %>%
  # Create county name.
...
# Save file  
save(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Sacramento Assessment Output 02-2021.RData")
#shell.exec("G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables")
...
assess_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Assessment Collapse.csv") %>%
  mutate(across(contains("time"), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date))%>%
  as_tibble()

book_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Booking Collapse.csv")%>%
  mutate(across(c(contains("time"), -contains("time_first_release")), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date)) %>%
  as_tibble() %>%
...
court_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Court Collapse.csv")%>%
  mutate(across(c(contains("time"), -contains("local")), as_datetime)) %>%
  mutate(across(contains("date") & is.character, as_date))%>%
  as_tibble()%>%
  rename("local_id" = "jail_court_id")


#joining court df to book df on jail_court_id, match rate = 56.9% of bookings, 82.8% of court cases
...
#joining assess_df to joined booking and court data on local id and book_num, match rate = 54.6%
## assessment data has longer date range than booking data by a few months

nrow(assess_df %>%
 semi_join(join_df, by = c("local_id", "book_num"), na_matches = "never"))/nrow(assess_df)
...
book_df %>%
  filter(book_num %in% book_num[book_type_original == "FILED SUPERIOR COURT CASE"]) %>%
  arrange(book_num, registry_sub_num)
...
join_collapse <- join_df2 %>%
  ...
  mutate(across(contains("sup_vio"), max)) %>%
  ungroup() %>%
  filter(final_disposition_date >= book_date, book_type %in% c("On_View", "Warrant")) %>%
  group_by(book_num) %>%
  mutate(across(contains("flag"), max)) %>%
  ungroup() %>%
  distinct(book_num, .keep_all = T)
  
#fwrite(join_collapse, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sacramento Join Collapse.csv")
join_collapse
```
Old code
```{r}
...
#Create an excel workbook with all data table. Make sure to include the cii table first.
write.xlsx(list(doj_cii, doj_court, doj_assess), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/February 2021/Sacramento P3 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "court", "assessment"))
```
...
#book_df <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Jail Data Set - File 1 of 3 - 20200713.csv") 
book_df <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Jail Dataset3-Bookings and Offenses-20210111.csv") 


# Variable name standardization--See pg. xxx for more details. 
names(book_df) <- c("lent_key", "book_num", "book_date_time", "arrest_date",  "book_type_original", "booked_or_pending_charge", "ccas_key", "court_case_id", "court_docket_id", "arrest_charge_code_original", "arrest_charge_original", "arrest_charge_level_original", "arrest_charge_description", "arrest_charge_counts",  "bail_amt")


# Date Reformatting --See pg. xxx for more details.
  ## note that the dates are formatted differently than in the last extract (mdy_hm and mdy instead of mdy_hms)
book_df <- book_df %>%
  mutate_at(vars(book_date_time), mdy_hm) %>%
...
####
#Jail book more variables
book_case <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Jail Dataset2-Bookings-20210107.csv") 

names(book_case) <- c("book_num", "lent_key", "book_date_time",  "book_type_original", "release_date_time", "release_type_original", "release_info", "employment_status_employer", "employment_type")

# Date Reformatting --See pg. xxx for more details.
book_case <- book_case %>%
  mutate_at(vars(book_date_time, release_date_time), mdy_hm) %>%
  mutate(release_date = as_date(release_date_time),
         book_date = as_date(book_date_time))

# Standardize Factors--See pg. xxx for more details.
book_case <- book_case %>%
         mutate(book_type = case_when(book_type_original %in% c("ONVW") ~ "On_View",
                               book_type_original %in% c("WRNT") ~ "Warrant",
                               book_type_original %in% c("SENT") ~ "Commitment",
                               book_type_original %in% c("RMND") ~ "Court_Order",
                               book_type_original %in% c("HOLD") ~ "Hold",
                               book_type_original %in% c("VIO/PROBATION") ~ "Sup_Vio_Post",
                               book_type_original %in% c("VIO/PRETRIAL", "VIO/SUPERVISED OR") ~ "Sup_Vio_Pre",
                               book_type_original %in% c("Book, Print, Release", "BPR CITE") ~ "Cite_Release",
...
book_demo <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Jail Dataset1-Jail Individuals-20210107.csv") 


# Variable name standardization--See pg. xxx for more details. 
names(book_demo) <- c("lent_key", "cii", "fbi", "dl_state", "cdl_id", "first_name", "middle_name", "last_name", "dob", "sex_original", "race_original")
...
# book_conviction <- fread("Historical Data Submissions/Sonoma/P2 Submission-June 30/Jail Data Set - File 3 of 3 - 20200713.csv")
book_conviction <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Jail Dataset4-Convictions-20210111.csv") 


# Variable name standardization--See pg. xxx for more details. 
names(book_conviction) <- c("lent_key", "ccas_key", "court_docket_id", "court_case_id", "conv_date", "conv_charge_code", "conv_charge_original", "conv_charge_level")


# Date Reformatting (as well as booking sequence and courtcase_id)--See pg. xxx for more details.
book_conviction <- book_conviction %>%
...
## book_df and book_case sometimes have different book types, ASK SONOMA WHICH TO USE
book_df <- left_join(book_df, book_demo) %>%
  left_join(book_case, by = c("lent_key", "book_num", "book_date_time", "book_date"))
...
# save(book_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Booking Collapse.RData")
...
# Court Case Data Import--- Sonoma, min "1999-08-30" 1st qu "2016-05-16" to max "2020-06-29" (cumulative)
  ...
  ## court_hearing contains one row per hearing.distinct was called to remove duplicate rows
    ### court_hearing may not be identifying all arraignments. Sonoma said we should use event code instead of hearing type, but it is unclear if we have this data element in our extract.
...
files <-list.files("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Court Data 2.4.21", full.names = T)
...
# Court cases
  ## court_cases contains one row per court case and includes some basic information about the court case
## all of the court cases in "Cases" are also found in "NonPTR_Cases", but the fields "ra_referal" and "ra_report" are almost universally assigned "Yes" in "NonPTR_Cases", potentially erroneously. Since that will likely be taken care of by the probation data, we are ignoring that for now and just bringing in "NonPTR_Cases" since it appears to be inclusive of all court case ids. 
#court_cases <- fread("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/Cases.csv") 

court_cases <- lapply(files[grepl("Cases", files)], vroom) %>%
  bind_rows() %>%
  mutate(`Court Number` = if_else(is.na(`Court Number`), `Case Number`, `Court Number`)) %>%
  select(-`Case Number`) %>%
  distinct()
...
court_charges <- lapply(files[grepl("Charges", files)], read_csv) %>%
  bind_rows()


names(court_charges) <- c("court_case_id", "court_docket_id", "charge_id", "file_type", "court_charge_original", "court_charge_level_original", "court_charge_description")
...
court_disposition <- lapply(files[grepl("PTRDispositions", files)], fread) %>%
  bind_rows() %>%
  distinct()
...
court_disposition <- court_disposition %>%
  mutate_at(vars(disposition_date),mdy) 
...
# Court sentence
  ## court_sentence contains one row per sentence
court_sentence <- lapply(files[grepl("Sentences", files)], fread) %>%
  bind_rows() %>%
  distinct() %>%
  select(-V22)
...
# Date Reformatting--See pg. xxx for more details.
court_sentence <- court_sentence %>%
  mutate_at(vars(sentence_date, probation_status_date),mdy)
...
# Court plea
  ## court_plea has one row per plea. This is generally one plea per charge, but some charges have multiple pleas. In the collapse we will filter down to the earliest plea.
court_plea <- lapply(files[grepl("Pleas", files)], fread) %>%
  bind_rows() %>%
  distinct() 
...
# Court person
  ## court_person contains one row per court case, with basic information on the defendant
court_person <- lapply(files[grepl("Defendants", files)], read_csv) %>%
  bind_rows()
...
# Court fta
  ## court_fta contains one row per fta and court case. distinct was called to remove duplicate rows
court_fta <- lapply(files[grepl("FTA", files)], fread) %>%
  bind_rows() %>%
  distinct()
...
# Date Reformatting--See pg. xxx for more details.
court_fta <- court_fta %>%
  mutate_at(vars(fta_date_time, fta_warrant_date_time),mdy_hms) %>%
...
# Court hearing
  ## court_hearing contains one row per hearing.distinct was called to remove duplicate rows
court_hearing <- lapply(files[grepl("Hearings", files)], fread) %>%
  bind_rows() %>%
  distinct()
...
court_ra <- lapply(files[grepl("RiskAssessments", files)], read_csv) %>%
  bind_rows()
...
court_final_disposition <- read.csv("Historical Data Submissions/Sonoma/P2 Submission-June 30/PTR_Final_Disp_Date.csv", stringsAsFactors = FALSE) 
...
#court_ptr_final_disp <- read.csv("Historical Data Submissions/Sonoma/P2 Submission-June 30/PTR_Final_Disp_Date.csv", stringsAsFactors = FALSE) 

# court_final_disposition %>%
#   filter(!is.na(final_dispo_date))
# 
# court_df %>%
#   select(court_case_id = court_docket_id, disposition_date, sentence_date) %>%
#   left_join(court_final_disposition) %>%
#   #mutate(final_disposition_date = if_else(disposition_date <= sentence_date|is.na(sentence_date), disposition_date, sentence_date)) %>%
#   filter(sentence_date != disposition_date) %>%
#   filter(sentence_date < disposition_date)
```
...
# Court FTAs are collapsed to each case. 
  ## Court Ftas are not separate from warrants
court_fta_war <- court_fta %>%
  mutate(court_fta_flag = if_else(court_fta_flag == "Yes", 1, 0)) %>%
  arrange(fta_date) %>%
  group_by(court_case_id) %>%
  mutate(fta_flag = max(court_fta_flag),
         fta_flag_count = sum(court_fta_flag)) %>%
...
# save(court_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Court Collapse.RData")
...
prob_person <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset4_Individuals.csv", stringsAsFactors = FALSE) #this appears to be a person table

names(prob_person) <- c("lent_key", "cii", "fbi", "dl_state", "cdl_id", "first_name", "middle_name", "last_name", "dob", "sex_original", "race_original")
...
prob_person <- prob_person %>%
    mutate(race_original = trimws(race_original),
           sex_original = trimws(sex_original),
        race = case_when(race_original %in% c("White") ~ "White",
                          race_original %in% c("Black") ~ "Black",
                          race_original %in% c("Hispanic") ~ "Hispanic",
                          T ~ "Other_Unknown"),
...
prob_book <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset1_AssessmentsAndGrants.csv", stringsAsFactors = FALSE)

names(prob_book) <- c("record_type", "assessment_id", "lent_key", "pretrial_grant_id", "tool_name", "assessment_date_time", "assessment_custody_status",
                      "book_num", "book_release_date_time", "book_release_type_original", "city", "zip_code", "pretrial_start_date",
                      "pretrial_termination_date", "pretrial_release_level_original", "other_pretrial_service", "ptr_grant_prearraignment",
                      "release_authorization", "outcome_fta_signup", "outcome_fta_court", "outcome_new_arrest")

prob_book <- prob_book %>%
  mutate(across(c(assessment_date_time, book_release_date_time), mdy_hm),
         assessment_date = as_date(assessment_date_time),
         book_release_date = as_date(book_release_date_time),
         across(c(pretrial_start_date, pretrial_termination_date), mdy))

## check with sonoma re monitoring levels. Is Level 1 monitoring the same as basic supervision? Level 2/moderate? Level 3/enhanced?
## what is assessment custody status? is that their status at assessment?
## pretrial release type -- need input from Sonoma, there are mixed messages looking at pretrial release level and book release type, people who have pretrial release levels indicating they were released to pretrial services nevertheless often have other release types in book release type. ASK SONOMA 
prob_book <- prob_book %>%
   mutate(assess_flag = if_else(tool_name == "No Assessment", 0, 1),
          
          tool_name = if_else(tool_name == "No Assessment", NA_character_, tool_name),
         
         monitor_level = case_when(pretrial_release_level_original %in% c("Basic Supervision", "Level 1 Monitoring") ~ "Low",
                                   pretrial_release_level_original %in% c("Moderate Supervision", "Level 2 Monitoring") ~ "Medium",
                                   pretrial_release_level_original %in% c("Enhanced Supervision", "Level 3 Monitoring") ~ "High",
                                   pretrial_release_level_original %in% c("Release to Pretrial Services with Court Reminder") ~ "None",
                                   T ~ NA_character_),
         
         release_decision = case_when(monitor_level %in% c("Low", "Medium", "High") ~ "Monitor",
                                      monitor_level %in% c("None") ~ "OR",
                                      T ~ "Other_Unknown"),
         
         pretrial_termination_reason = case_when(outcome_new_arrest == "New Booking Charges" ~ "New_Crime",
                                                 outcome_fta_court == "FTA Court" ~ "FTA",
                                                 outcome_fta_signup == "Failed to report to Pretrial Services" ~ "Violation",
         ...
         book_release_type_original = trimws(book_release_type_original),
         
         pretrial_ineligible_reason = case_when(book_release_type_original %in% c("BAIL BOND", "CASH BAIL") ~ "Bail_Bond",
                                                book_release_type_original %in% c("TIME SERVED", "PROBATION/COND SENTENCE", "NO COMPLAINT FILED",
                                                                                  "DETENTION ONLY", "CREDIT FOR TIME SERVED", "TIME SERVED",
                                                                                  "COMPLAINT DISMISSED", "MODIFICATION-TIME SERVED", 
                                                                                  "INSUFF PROBABLE CAUSE", "MANDATORY SUPERVISION", 
                                                                                  "849B1", "849B2") ~ "Case_Disposed",
                                                T ~ "Other_Unknown"),
         
         
         release_authorization = case_when(release_authorization %in% c("Judge", "Commissioner", "Judge/Commissioner") ~ "Judge",
                                           T ~ NA_character_)
  )
...
## how should we treat the recommendation overrides?? ASK SONOMA
prob_sprat <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset2_SPRATassessments.csv", stringsAsFactors = FALSE)

names(prob_sprat) <- c("assessment_id", "x1_2orless_charges", "x2_dui_arrest", "x3_dv_arrest",  "x4_violence_arrest", "x5_no_prior_FTA",
                        "x6_employed_or_student", "x7_prior_jail", "x8_pending_cases", "x9_gang_affiliation", "x10_homeless",
                        "x11_MH_code_violent_behavior", "enh_bac", "enh_victim_injury", "enh2_dui_conv", "enh_victim_injury_pc243",
                       "enh_children_present", "enh_1dv_order_last7yr", "enh_2dv_conv", "enh_5prior_conv", "enh_3FTAs",
                       "enh_on_probation", "enh_victim_injury_pc273_5", "enh_dv_injury", "risk_score",
                        "recommendation_before_enhancements", "release_recommendation_original", "release_recommendation_override", "override_reason")

prob_sprat <- prob_sprat %>%
  mutate(release_recommendation_original = trimws(release_recommendation_original),
         ...
         monitor_level_recommended = case_when(release_recommendation_original == "Basic Supervision" ~ "Low",
                                               release_recommendation_original == "Moderate Supervision" ~ "Medium",
                                               release_recommendation_original %in% c("Enhanced Supervision", "Detain or Enhanced Supervision") ~ "High",
                                               release_recommendation_original == "Court Reminder" ~ "None",
                                               T ~ NA_character_))


####

## ASK SONOMA about presumptive release level
prob_psa <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset3_PSAassessments.csv", stringsAsFactors = FALSE)

names(prob_psa) <- c("assessment_id", "psa_tool_response_1", "psa_tool_response_2", "psa_tool_response_2a", "psa_tool_response_3", "psa_tool_response_4",
                     "psa_tool_response_5", "psa_tool_response_5a", "psa_tool_response_6", "psa_tool_response_7", "psa_tool_response_8",
                     "psa_tool_response_9", "psa_fta_risk_score", "psa_nca_risk_score", "psa_nvca_risk_score", "prearraignment_review",
                     "presumptive_release_level")

prob_psa <- prob_psa %>%
  mutate(risk_level = case_when(presumptive_release_level == "Release Level 1" ~ "Low",
                                presumptive_release_level == "Release Level 2" ~ "Medium",
                                presumptive_release_level == "Release Level 3" ~ "High"))


####

prob_violations <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset5_PretrialViolations.csv", stringsAsFactors = FALSE)

names(prob_violations) <- c("lent_key", "pretrial_grant_id", "pretrial_violation_original", "pretrial_violation_date", "pretrial_violation_outcome_original")

prob_violations <- prob_violations %>%
  mutate(pretrial_violation_date = mdy(pretrial_violation_date))

prob_violations <- prob_violations %>%
  mutate(pretrial_termination_reason = case_when(pretrial_violation_outcome_original == "Yes" ~ "Violation",
                                                 T ~ "Other_Unknown"),
         
         pretrial_termination_outcome = case_when(pretrial_violation_outcome_original == "Yes" ~ "Unsuccessful",
                                                  T ~ "Other_Unknown"))


####

prob_conditions <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset6_TermsandConditions.csv", stringsAsFactors = FALSE)

names(prob_conditions) <- c("pretrial_grant_id", "lent_key", "release_conditions_original", "release_condition_type")

prob_conditions <- prob_conditions %>%
  mutate(condition_em_flag = if_else(release_condition_type == "Electronic Monitoring Condition", 1, 0),
         condition_drug_test_flag = if_else(release_condition_type == "Drug Testing Condition", 1, 0),
         condition_no_contact_flag = if_else(grepl("Do not contact", release_conditions_original), 1, 0))


####

## should we make a court date reminder flag for every attempt or only for successful attempts??
prob_reminders <- read.csv("Historical Data Submissions/Sonoma/P3 Submission-Jan 15/ProbationDataset7_CourtDateReminders.csv", stringsAsFactors = FALSE)

names(prob_reminders) <- c("lent_key", "court_date_time", "notification_attempt_date_time", "notification_result")

prob_reminders <- prob_reminders %>%
  mutate(across(c(court_date_time, notification_attempt_date_time), mdy_hm),
         court_date = as_date(court_date_time),
         notification_attempt_date = as_date(notification_attempt_date_time),
         court_date_reminder_flag = if_else(notification_result %in% c("Delivered to Answering Machine", "Delivered to Person", "Delivered to Person - Right Party", "Delivered to Person - Take Message"), 1, 0))


```

Old probation data, new data is in different format and file divisions
```{r}
# Variable name standardization--See pg. xxx for more details. 
...
#collapse violations to each release, only keeping violations that resulted in termination
prob_vio <- prob_violations %>%
  filter(pretrial_violation_outcome_original == "Yes") %>%
  arrange(pretrial_violation_date) %>%
  distinct(pretrial_grant_id, .keep_all = T)


#collapse conditions to each release
  ##
prob_conds <- prob_conditions %>%
  group_by(pretrial_grant_id) %>%
  mutate(across(contains("flag"), max)) %>%
  ungroup() %>%
  distinct(pretrial_grant_id, .keep_all = T)
  
#collapse court date reminders to each person, bc there is no pretrial grant id (CAN WE ASK SONOMA TO ADD THIS?), otherwise would have to fuzzy join using court date
prob_rem <- prob_reminders %>%
  filter(court_date_reminder_flag == 1) %>%
  distinct(lent_key, .keep_all = T) 


assess_df <- prob_book %>%
  left_join(prob_vio, by = c("pretrial_grant_id", "lent_key")) %>%
  left_join(prob_conds, by = c("pretrial_grant_id", "lent_key")) %>%
  left_join(prob_psa, by = c("assessment_id")) %>%
  left_join(prob_sprat, by = c("assessment_id")) %>%
  left_join(prob_rem, by = c("lent_key")) %>%
  left_join(prob_person, by = c("lent_key"))
  

# save(assess_df, file =  "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Assessment Collapse.RData")
...
assess_table <- assess_df %>%
  ...
  mutate(release_recommendation = NA_character_,
         risk_level = NA_character_,
         pretrial_termination_outcome = if_else(pretrial_termination_outcome.y == "Unsuccessful", "Unsuccessful", pretrial_termination_outcome.x),
         pretrial_termination_reason = if_else(pretrial_termination_outcome.y == "Unsuccessful" & pretrial_termination_outcome.x == "Successful", "Violation", pretrial_termination_reason.x)) %>%
  # Select relevants variables.
...
# Save file  
#save(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Sonoma Assessment Output 02-2021.RData")
...
#Create an excel workbook with all data table. Make sure to include the cii table first.
write.xlsx(list(doj_cii, doj_book, doj_court_person, doj_court_ra, doj_assess), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/February 2021/Sonoma P3 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court person", "court ra", "assessment"))
...
assess_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Assessment Collapse.csv") %>%
  mutate(across(contains("time"), as_datetime)) %>%
  mutate(across(contains("date") & where(is.character), as_date))%>%
  as_tibble()

book_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Booking Collapse.csv")%>%
  mutate(across(c(contains("time"), -contains("time_first_release")), as_datetime)) %>%
  mutate(across(contains("date") & where(is.character), as_date)) %>%
  as_tibble() %>%
  rename("court_num" = "court_case_id",
         "court_case_id" = "court_docket_id") %>%
  rename("court_docket_id" = "court_num")

court_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Court Collapse.csv")%>%
  mutate(across(c(contains("time"), -contains("local")), as_datetime)) %>%
  mutate(across(contains("date") & where(is.character), as_date))%>%
  as_tibble() %>%
  rename("court_num" = "court_case_id",
...
## ASK SONOMA: why are there so few court cases?


#joining court df to book df on jail_court_id, match rate = 20.6% of bookings, 96.2% of court cases
...
#joining assess_df to joined booking and court data on book num, court docket id, and lent key (person identifier), match rate = 91%
## assessment data has longer date range than booking data by a few months

nrow(assess_df %>%
 semi_join(join_df, by = c("book_num", "lent_key", "court_docket_id"), na_matches = "never"))/nrow(assess_df)

join_df2 <- join_df %>%
 left_join(assess_df, by = c("book_num", "lent_key", "court_docket_id"), na_matches = "never") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) 
...
#fwrite(join_collapse, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sonoma Join Collapse.csv")
