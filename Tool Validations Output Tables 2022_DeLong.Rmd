...
Connection
```{r include=FALSE}
con <- dbConnect(odbc::odbc(), "snowflake_pretrial", uid ="jeffrey.wu@jud.ca.gov", pwd = rstudioapi::askForPassword("Password"))
...
```{sql connection=con, include=FALSE, output.var='join_df'}
SELECT *
...
# Indiv intro tables for reports
```{r}

county_list = eval_df %>% distinct(county) %>% pull(county)
tool_list = eval_df %>% distinct(tool_name) %>% pull(tool_name)

lapply(c("Los Angeles", "Sacramento", "Ventura", "Sonoma", "Tulare", "San Joaquin", "Modoc", "Napa", "Calaveras", "Alameda", "Santa Barbara", "San Mateo"),
  function(x){
...
tool_df <- auc_df %>%
 ...
 mutate(outcome = case_when(outcome == "fta_flag" ~ "FTA",                              
                            ...
                            outcome == "any_pretrial_failure_flag" ~ "FTA or New"))
...
San Joaquin  ---- removed until data is fixed
\newline
```{r}
tool_df <- auc_df %>%
...
VPRAI-R--With Interactions at score 5
\newline
```{r results='asis'}
m1 <- glm(family = binomial(),fta_flag ~ vprair_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
m2 <- glm(family = binomial(),recid_arrested_flag ~ vprair_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
m3 <- glm(family = binomial(),recid_filed_flag ~ vprair_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
m4 <- glm(family = binomial(),recid_convicted_flag ~ vprair_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
m5 <- glm(family = binomial(),recid_arrest_violent_psa_flag ~ vprair_risk_score * race + release_to_pretrial_end, data =
            race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
m6 <- glm(family = binomial(), any_pretrial_failure_flag ~ vprair_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "VPRAI-R") %>% mutate(across(contains("score"), ~.x-5)))
...
ORAS--With Interactions at lowest score 0
\newline
```{r results='asis'}
m1 <- glm(family = binomial(),fta_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
m2 <- glm(family = binomial(),recid_arrested_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
m3 <- glm(family = binomial(),recid_filed_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
m4 <- glm(family = binomial(),recid_convicted_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
m5 <- glm(family = binomial(),recid_arrest_violent_psa_flag ~ oras_risk_score * race + release_to_pretrial_end, data =
            race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
m6 <- glm(family = binomial(), any_pretrial_failure_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x)))
...
ORAS--With Interactions at score 9
\newline
```{r results='asis'}
m1 <- glm(family = binomial(),fta_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
m2 <- glm(family = binomial(),recid_arrested_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
m3 <- glm(family = binomial(),recid_filed_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
m4 <- glm(family = binomial(),recid_convicted_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
m5 <- glm(family = binomial(),recid_arrest_violent_psa_flag ~ oras_risk_score * race + release_to_pretrial_end, data =
            race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
m6 <- glm(family = binomial(), any_pretrial_failure_flag ~ oras_risk_score * race + release_to_pretrial_end, data = race_df %>% filter(tool_name == "ORAS") %>% mutate(across(contains("score"), ~.x-9)))
...
Santa Barbara--Without Interactions
\newline
```{r results='asis'}
county_df <- race_df %>%
  filter(county == "Santa Barbara") %>%
...
Santa Barbara--With Interactions
\newline
```{r results='asis'}
county_df <- race_df %>%
  filter(county == "Santa Barbara") %>%
...
kable(tool_df[1:5], booktabs = T, format.args = list(big.mark = ","), 
  ...
  add_footnote(paste("N White =", tool_df$N_White[1],  ", N Hispanic =", tool_df$N_Hispanic[1]))
```
\newpage
...
ggplot(risk_df) +
  ...
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent", 
       title = "Comparison of Racial Differences in Logistic Regression Curves--PSA, Sonoma") +
  scale_x_continuous(breaks = 1:6)
...
ORAS AUC race comparison
```{r}
rocW <- roc(fta_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(fta_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(fta_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_arrested_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_arrested_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_arrested_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_filed_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_filed_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_filed_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_convicted_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_convicted_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_convicted_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_arrest_violent_psa_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_arrest_violent_psa_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_arrest_violent_psa_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "White"), ci = T)
rocB <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Black"), ci = T)
rocH <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)
```

ORAS AUC gender comparison
```{r}
rocW <- roc(fta_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(fta_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrested_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrested_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_filed_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_filed_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_convicted_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_convicted_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrest_violent_psa_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrest_violent_psa_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Female"), ci = T)
rocM <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)
```
...
rocW <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(race == "White"), ci = T)
rocB <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(race == "Black"), ci = T)
rocH <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)
```
...
```{r}
rocW <- roc(fta_flag ~ psa_fta_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(fta_flag ~ psa_fta_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrested_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrested_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_filed_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_filed_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_convicted_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_convicted_flag ~ psa_nca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrest_violent_psa_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrest_violent_psa_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Female"), ci = T)
rocM <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)
```
...
rocW <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(race == "White"), ci = T)
rocB <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(race == "Black"), ci = T)
rocH <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)
...
```{r}
rocW <- roc(fta_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
rocM <- roc(fta_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrested_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrested_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
rocM <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

# rocW <- roc(recid_filed_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
# rocM <- roc(recid_filed_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
# roc.test(rocW, rocM)
# 
# rocW <- roc(recid_convicted_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
# rocM <- roc(recid_convicted_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
# roc.test(rocW, rocM)
# 
# rocW <- roc(recid_arrest_violent_psa_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Female"), ci = T)
# rocM <- roc(recid_arrest_violent_psa_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(sex == "Male"), ci = T)
# roc.test(rocW, rocM)
```

VPRAI-R AUC race comparison
```{r}
rocW <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)

rocW <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "White"), ci = T)
rocB <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Black"), ci = T)
rocH <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW, rocB)
roc.test(rocW, rocH)
roc.test(rocB, rocH)
```

VPRAI-R AUC gender comparison
```{r}
rocW <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)

rocW <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Female"), ci = T)
rocM <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(sex == "Male"), ci = T)
roc.test(rocW, rocM)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Alameda") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Alameda") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Alameda") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Alameda") %>% filter(sex == "Female"), ci = T)
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Alameda") %>% filter(sex == "Male"), ci = T)

roc.test(rocW_newpretrial, rocM_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Los Angeles") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Los Angeles") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Los Angeles") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```

Los Angeles PSA AUC gender comparisons
```{r}
...
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Los Angeles") %>% filter(sex == "Male"), ci = T)
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Los Angeles") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newpretrial, rocW_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sacramento") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sacramento") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sacramento") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sacramento") %>% filter(sex == "Female"), ci = T)
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sacramento") %>% filter(sex == "Male"), ci = T)
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(county == "San Joaquin") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(county == "San Joaquin") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(county == "San Joaquin") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(county == "San Joaquin") %>% filter(sex == "Female"), ci = T)
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ vprai_risk_score, data = eval_df %>% filter(tool_name == "VPRAI") %>% filter(county == "San Joaquin") %>% filter(sex == "Male"), ci = T)
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```

San Mateo VPRAI-R AUC gender comparisons
```{r}
rocM_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_FTA, rocW_FTA)


rocM_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newarrest, rocW_newarrest)


rocM_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newfiling, rocW_newfiling)


rocM_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newconviction, rocW_newconviction)


rocM_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newviolent, rocW_newviolent)


rocM_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Male"), ci = T)
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "San Mateo") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newpretrial, rocW_newpretrial)
```

Santa Barbara VPRAI-R AUC race comparisons
```{r}
rocW_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_FTA, rocB_FTA)
roc.test(rocW_FTA, rocH_FTA)
# roc.test(rocB_FTA, rocH_FTA)

rocW_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_newarrest, rocB_newarrest)
roc.test(rocW_newarrest, rocH_newarrest)
# roc.test(rocB_newarrest, rocH_newarrest)

rocW_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_newfiling, rocB_newfiling)
roc.test(rocW_newfiling, rocH_newfiling)
# roc.test(rocB_newfiling, rocH_newfiling)

rocW_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_newconviction, rocB_newconviction)
roc.test(rocW_newconviction, rocH_newconviction)
# roc.test(rocB_newconviction, rocH_newconviction)

rocW_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_newviolent, rocB_newviolent)
roc.test(rocW_newviolent, rocH_newviolent)
# roc.test(rocB_newviolent, rocH_newviolent)

rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "White"), ci = T)
# rocB_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(race == "Hispanic"), ci = T)
# roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
# roc.test(rocB_newpretrial, rocH_newpretrial)
```

Santa Barbara VPRAI-R AUC gender comparisons
```{r}
rocM_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_FTA <- roc(fta_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_FTA, rocW_FTA)


rocM_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_newarrest <- roc(recid_arrested_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newarrest, rocW_newarrest)


rocM_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_newfiling <- roc(recid_filed_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newfiling, rocW_newfiling)


rocM_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_newconviction <- roc(recid_convicted_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newconviction, rocW_newconviction)


rocM_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_newviolent <- roc(recid_arrest_violent_psa_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)

roc.test(rocM_newviolent, rocW_newviolent)


rocM_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sonoma") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sonoma") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sonoma") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sonoma") %>% filter(sex == "Male"), ci = T)
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Sonoma") %>% filter(sex == "Female"), ci = T)
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Tulare") %>% filter(race == "White"), ci = T)
rocB_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Tulare") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Tulare") %>% filter(race == "Hispanic"), ci = T)
roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Tulare") %>% filter(sex == "Male"), ci = T)
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ psa_nvca_risk_score, data = eval_df %>% filter(tool_name == "PSA") %>% filter(county == "Tulare") %>% filter(sex == "Female"), ci = T)
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(county == "Ventura") %>% filter(race == "White"), ci = T)
#rocB_newpretrial <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(county == "Ventura") %>% filter(race == "Black"), ci = T)
rocH_newpretrial <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(county == "Ventura") %>% filter(race == "Hispanic"), ci = T)
#roc.test(rocW_newpretrial, rocB_newpretrial)
roc.test(rocW_newpretrial, rocH_newpretrial)
#roc.test(rocB_newpretrial, rocH_newpretrial)
```
...
rocW_newpretrial <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(county == "Ventura") %>% filter(sex == "Female"), ci = T)
rocM_newpretrial <- roc(any_pretrial_failure_flag ~ oras_risk_score, data = eval_df %>% filter(tool_name == "ORAS") %>% filter(county == "Ventura") %>% filter(sex == "Male"), ci = T)
