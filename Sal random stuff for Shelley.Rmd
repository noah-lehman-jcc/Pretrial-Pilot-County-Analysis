...
full_df <- full_df %>%
  mutate(release_type = case_when(trimws(release_type_original) %in% c("OR", "PRETRIAL SERVICES RELEASE", "PRETRIAL PROBATION OR", "RELEASE W/COURT CONDITIONS", "PRE-ARRAIGNMENT RELEASE", "O/R- PRETRIAL PROG", "OREM", "OWN RECOGNIZANCE TO PROGRAM", "OWN RECOGNIZANCE", "Pretrial O/R with Terms", "Pre-Trial Monitoring", "Court Ordered Pretrial Release", "Pretrial Release-2", "Pretrial Release-3") ~ "OR",
                                  ...
                                  trimws(release_type_original) %in% c("SENTENCED/ STR", "SENTENCED TO CDCR", "STATE PRISON", "STATE INSTITUTE/PRIS", "Sentenced to State Prison", "PROB", "PROBATION", "PROBATION (FORMAL)", "Probation", "Pandemic Sentence Release", "Sentenced") ~ "Sentenced",
                                  trimws(release_type_original) %in% c("TIME SERVED – SC43450", "TIME SERVED - OVERCROWDING", "TIME SERVED", "CAP/TIME SERVED", "TIME SERVED", "GROSS TIME SERVED") ~ "Time_Served",
                                  T ~ release_type))
...
write.xlsx(date_range, file = "I:/Random for Shelley/2021.06.03 pretrial stuff/All bookings date ranges by county.xlsx", asTable = F)
```


Look at counties with bad matching and use their release types to determine whether release was pretrial or not
```{r}
#look at ppl who do not have completed pretrial periods
#this may falsely represent pretrial releases for counties that have poor court data matching, because they may be released due to a disposition that we do not have a record of
...
write.xlsx(arrest_charge_level_df, file = "I:/Random for Shelley/2021.06.03 pretrial stuff/2A offense type - using release type instead of dates draft2.xlsx", asTable = F)
```

FILTERED TO RESOLVED BOOKINGS: Look at counties with bad matching and use their release types to determine whether release was pretrial or not
```{r}
#look at ppl who do not have completed pretrial periods
#this may falsely represent pretrial releases for counties that have poor court data matching, because they may be released due to a disposition that we do not have a record of
...
write.xlsx(arrest_charge_level_df, file = "I:/Random for Shelley/2021.06.03 pretrial stuff/2A offense type - resolved vs unresolved 2 day releases using release type v2.xlsx", asTable = F)
```
