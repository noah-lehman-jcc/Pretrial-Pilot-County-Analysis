...
match_df <- full_df %>%
  mutate(pretrial_end_date_doj = if_else(is.na(pretrial_period_end_date), pretrial_period_end_date_doj, pretrial_period_end_date)) %>% #changed this, used to be assigned to pretrial_end_date, need to fix variable names going forward ****************************************************
...
save(match_df, file = "Match df 4.6.22.RData")
...
match_df <- match_df %>%
  ...
  filter(pretrial_period_end_date - as_date(release_date) > 0) %>%
...
save(eval_df, file = "Eval df 4.6.22.RData")
