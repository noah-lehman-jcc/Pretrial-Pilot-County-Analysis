...
# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("tool_name", "name", "dob", "sex_original", "race_original", "fbi", "cdl_id", "cii", "book_num", "zip_code", "referral_status", "court_date_reminder", "release_recommendation_original", "termination_outcome_original", "termination_date", "assessment_date", "vpraio_risk_score", "vpraio_tool_response_1", "vpraio_tool_response_2", "vpraio_tool_response_3", "vpraio_tool_response_4", "vpraio_tool_response_5", "vpraio_tool_response_6", "vpraio_tool_response_7", "vpraio_tool_response_8", "vpraio_tool_response_9", "release_type_original",  "release_date_time", "release_authorization", "release_conditions_original", "pretrial_violation_date", "pretrial_violation")


# Date Reformatting--See pg. xxx for more details.
assess_df <- assess_df %>%
  mutate(across(c(termination_date, assessment_date, dob, pretrial_violation_date), mdy),
...
assess_df <- assess_df %>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         release_recommendation = case_when(release_recommendation_original %in% c("AA Release w/Conditions", "PA Release w/Conditions") ~ "Monitor",
                                            ...
                                            T ~ "Other_Unknown"),
         release_decision = case_when(termination_outcome_original %in% c("Denied by On Call Judge") ~ "Detain",
                                      release_type_original %in% c("Pretrial - Supervision") ~ "Monitor",
                                      termination_outcome_original %in% c("Bail Before Arraignment", "DA Declines to File", "No Bail Warrant") ~ "Ineligible",
                                      T ~ "Other_Unknown"),
         
         pretrial_ineligible_reason = case_when(termination_outcome_original %in% c("Bail Before Arraignment") ~ "Bail_Bond",
                                                termination_outcome_original %in% c("DA Declines to File") ~ "Case_Disposed",
                                                T ~ "Other_Unknown"),
         tool_name = "vpraio") 
...
assess_df %>%
  count(termination_outcome_original, release_type_original)
```
...
# Variable name standardization--See pg. xxx for more details. 
names(prob_cond) <- c("pretrial_id", "local_id", "court_case_id", "condition_status", "condition_original")
...
# Save file  
#fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Napa Assessment Output 08-2020.csv")
```
...
prob_assess <- prob_assess %>%
  ...
  mutate(assess_flag = if_else(is.na(sprat_id), 0, 1),
         ...
         monitor_level_recommended = case_when(release_recommendation == "Basic Supervision" ~ "Low",
                                               ...
                                               T ~ NA_character_),
         monitor_level = case_when(release_type_original == "Basic Supervision" ~ "Low",
                                   release_type_original == "Moderate Supervision" ~ "Medium",
                                   release_type_original == "Enhanced Supervision" ~ "High",
                                   release_type_original == "Court Reminder" ~ "None",
                                   T ~ NA_character_),
         release_decision = case_when(release_type_original %in% c("Basic Supervision", "Moderate Supervision", "Enhanced Supervision") ~ "Monitor",
                                      release_type_original %in% c("Court Reminder") ~ "OR",
                                      T ~ "Other_Unknown")
  )
...
assess_table <- prob_assess %>%
  ...
  # Select relevants variables.
  select(county, release_recommendation, release_decision, assessment_date, risk_score, tool_name)

# Save file  
#fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Sonoma Assessment Output 08-2020.csv")
