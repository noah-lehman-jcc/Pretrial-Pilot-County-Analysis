...
person_df <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 1)


# Variable name standardization--See pg. xxx for more details. 
names(person_df) <- c("fbi", "cii", "local_id", "cdl_id", "cdl_id_state", "name", "dob", "race_original") #P2 submission missing sex. Pursuant to call 7.16.20 with Dan they will add and re-send
...
book_df <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 2, col_types = "text")
...
book_charge <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 3, col_types = "text")
...
prob_df <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020.xlsx", sheet = 12)
...
##prob_bookings contains mostly one row per book_num. There are a few duplicated rows which are eliminated, and one case where a book_num has two rows with the only difference is one has excluded value 1 the other has excluded value 0. Not sure what the "excluded" column represents. 
## prob_bookings has date range 2020-02-05 to 2020-06-30
prob_bookings <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020_ProbData.xlsx", sheet = 1) %>%
  distinct()

# Variable name standardization--See pg. xxx for more details. 
names(prob_bookings) <- c("book_num", "book_date_time", "local_id", "excluded")
...
prob_reports <- read_excel("Historical Data Submissions/Napa/P2 Submission-June 30/Historical_7-1-2020_ProbData.xlsx", sheet = 2)

# Variable name standardization--See pg. xxx for more details. 
names(prob_reports) <- c("book_num", "report_date_time", "report_type", "release_recommendation_original", "release_decision_original")
...
#High level casees
court_df <- fread("Historical Data Submissions/Sacramento/P2 Submission-June 30/SacCases07132020.csv")
...
#Court Hearing Data
court_hearing <- fread("Historical Data Submissions/Sacramento/P2 Submission-June 30/SacAppear07132020.csv")
...
court_dispos <- fread("Historical Data Submissions/Sacramento/P2 Submission-June 30/SacDispos07122020.csv")
...
#Court person ids
court_person <- fread("Historical Data Submissions/Sacramento/P2 Submission-June 30/SacPersonDetail07132020.csv")
...
#Court Warrants
court_warrants <- fread("Historical Data Submissions/Sacramento/P2 Submission-June 30/SacWarrants07132020.csv")
...
court_df %>%
  ...
  semi_join(court_dispos, by = "case_id")

court_dispos %>%
  distinct(case_id) %>%
  semi_join(court_df, by = "case_id")

# low match rate, but this is due to there being far fewer unique case ids in court_dispos than in court_df. Why???
...
court_person %>%
  ...
  semi_join(court_dispos, by = "jail_court_id")
...
#missing data: tool responses, score, violation, violation date, release_type is missing for over 4,000 records, and will need some help on termination_outcome (~800 missing, blank not na, not sure what the rest mean)
