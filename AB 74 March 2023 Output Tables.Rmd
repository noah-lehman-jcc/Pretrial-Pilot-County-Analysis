...
df <- df %>%
  mutate(tool_name_orig = tool_name,
         # remove this once the tool name logic is fixed
         ...
         county = county_name,
         County = case_when(
                            county %in% c("Sacramento", "Alameda", "Los Angeles") ~ "Large Counties",
                            county %in% c("Sonoma", "San Joaquin", "San Mateo", "Tulare", "Santa Barbara", "Ventura") ~ "Medium Counties",
         ...
         County = fct_relevel(County, levels = c("Small Counties", "Small/Medium Counties", "Medium Counties", "Large Counties")),
         ...
         vprai_risk_score = if_else(tool_name == "VPRAI" & risk_score_raw %in% 0:9, risk_score_raw, NULL),
         ...
         scored_assessment = if_else(tool_name %in% c("ORAS", "VPRAI", "VPRAI-R") & !is.na(risk_score_raw) |
                                       ...
                                       tool_name %in% c("VPRAI-O", "Local") & !is.na(generic_tool_total_score_raw), 1, 0),
...
df %>%
  ...
  count(risk_score_raw)

df %>%
  filter(county == "Kings") %>%
  select(risk_level) %>%
  mutate(risk_level_part = parse_number(risk_level))

df %>%
  count(county, tool_name)

df %>%
  count(COUNTY_NAME, TOOL_NAME, ASSESSMENT_DATE < as.Date("2019-10-01"))

df %>%
  count(COUNTY_NAME, TOOL_NAME, TOOL_NAME_STANDARD)

df %>%
  filter(COUNTY_NAME == "Santa Barbara") %>%
  count(is.na(RISK_SCORE_RAW))

df %>%
  count(scored_assessment, county, tool_name)

df %>%
  filter(tool_name == "Local") %>%
  count(county, generic_tool_total_score_raw)

df %>%
  count(tool_name, county)

eval_df %>%
  count(county, race_standard) %>%
  filter(race_standard == "Asian")
```
...
kable(date_range, booktabs = T, format.args = list(big.mark = ","), 
        # table.attr = "style = \"color: black;\"",
        format = "latex") %>%
    kable_styling(position = "center", full_width = F, 
...
## The Number of Assessed Individuals by Booking Charge Level with df
```{r}
charge_level_df <- df %>%
  # filter(arrest_charge_level != "I") %>%
  mutate(arrest_charge_level = case_when(is.na(arrest_charge_level)|arrest_charge_level %in% c("Other/Unknown", "I") ~"Other_Unknown",
  ...
  select(County, 'F', 'M', Other_Unknown, Total) %>%
  bind_rows(df %>%
              mutate(arrest_charge_level = case_when(is.na(arrest_charge_level)|arrest_charge_level %in% c("Other/Unknown", "I") ~"Other_Unknown",
                                         T ~ arrest_charge_level)) %>%
...
## The Number of Assessed Individuals by Booking Charge Level with join_df -- do not use
```{r eval=FALSE, include=FALSE}
...
# FIX THIS CHUNK -- column "Other" doesn't exist
```{r}
release_df <- df %>%
  filter(la_flag == 0) %>%
  mutate(release_decision = if_else(release_decision_arraignment == "Monitor", "Own Recognizance", release_decision_arraignment)) %>%
  mutate(release_decision = if_else(release_decision %in% c("Deny Release", "Own Recognizance"), release_decision, "Other")) %>%
...
release_df <- release_df[c(1,3,2, 4)]

names(release_df) <- c("Court Size", "Granted Program Release", "Denied Program Release", "Other/Unknown")
...
\begin{landscape}
Number with no arraignment release decision, without LA
```{r}
release_df <- df %>%
  filter(la_flag == 0) %>%
  mutate(release_decision = case_when(release_decision_arraignment == "Monitor" ~ "Own Recognizance", 
                                      release_decision_arraignment %in% c("Deny Release", "Own Recognizance") ~release_decision_arraignment,
                                      T ~ "Ineligible/Unknown")) %>%
  count(County, release_decision) %>%
  group_by(County) %>%
  mutate(Total = sum(n)) %>%
  pivot_wider(names_from =  release_decision, values_from =  n, values_fill = 0) %>%
  select(County, Total, 'Ineligible/Unknown', 'Own Recognizance', 'Deny Release')

names(release_df) <- c("Court Size", "Total", "Ineligible/Unknown", "Granted Program Release", "Denied Program Release")
...
Number with no prearraignment decision, LA only
```{r}
release_df <- df %>%
  filter(la_flag == 1) %>%
  mutate(release_decision = case_when(release_decision_prearraignment == "Monitor" ~ "Own Recognizance", 
                                      release_decision_prearraignment %in% c("Deny Release", "Own Recognizance") ~release_decision_prearraignment,
                                      T ~ "Ineligible/Unknown")) %>%
  count(County, release_decision) %>%
  group_by(County) %>%
  mutate(Total = sum(n)) %>%
