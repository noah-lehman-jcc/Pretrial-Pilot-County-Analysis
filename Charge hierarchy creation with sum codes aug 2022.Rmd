...
Missing Charges - used pilot data to identify charges that were not in the charge code table
```{r}
#these have no summary codes or bcs codes
...
#save(bcs, file = "bcs and summary code mapping.RData")

#map combinations of bcs code and jbsis code
bcsj <- charge_hier %>%
  select(jbsis_code, bcs_code) %>%
  distinct()

bcsj <- bcsj %>%
  filter(!is.na(jbsis_code)) %>%
  arrange(bcs_code) %>%
  add_count(bcs_code) %>%
  filter(n==1) %>%
  select(-n)

# save(bcsj, file = "bcs and jbsis code mapping.RData")
...
```{r}
file <- "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/Legal Review of Offense Flags_SL.xlsx"
...
# updated 1/27/2021 to also match summary codes to charges with missing summary codes in the same way
...
charge_hier <- charge_hier %>%
  mutate(charge_start = as.integer(parse_number(charge)),
         ...
         summary_code = if_else(is.na(summary_code)|summary_code == 0, 1000, summary_code)) %>%
  group_by(charge_level, charge_code, charge_start) %>%
  mutate(n = sum(!(unique(jbsis_code) %in% c(1000, 90, 250, 270))),
         ns = sum(!unique(summary_code) %in% c(1000))) %>%
  mutate(jbsis_code = if_else(jbsis_code == 1000 & n %in% 0:1, min(jbsis_code), jbsis_code),
         summary_code = if_else(summary_code == 1000 & ns %in% 0:1, min(summary_code), summary_code)) %>%
  ungroup() %>%
  mutate(#Fixing property and drug flags to be off of summary codes
         property_flag = if_else(summary_code %in% c(8:11, 24, 31:33, 64), 1, 0),
         drug_flag = if_else(summary_code %in% c(12:15, 34:36), 1, 0)) %>%
  select(-n, -ns, -charge_start, -bcs_code)

#load("G:/CrimJustice/PretrialAssessmentPilot/Complete Charge Code Hierarchy_SL.RData")

#there are a few instances where the charge hierarchy is incorrectly less than 1000
charge_hier <- charge_hier %>%
  mutate(hierarchy = if_else(hierarchy < 1000, 1e6, hierarchy),
         charge_code = if_else(!charge_code %in% c("PC", "VC", "HS", "BP", "WI", "US"), "Other", charge_code)) %>%
  unite(join_var, charge_level, charge_code, charge, remove = F) %>%
  select(join_var, charge_level, charge_code, charge, master_charge_description, everything())


save(charge_hier, file = "Complete Charge Code Hierarchy_SL.RData")

#save(charge_hier, file = "G:/CrimJustice/PretrialAssessmentPilot/Complete Charge Code Hierarchy_SL.RData")
#save(charge_hier, file = "G:/CrimJustice/PretrialAssessmentPilot/Complete Charge Code Hierarchy.RData")

```

Update with 2022 DOJ master charge code table
```{r}
doj_hierarchy_2022ls <- fread("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/charge code table 1.7.22 livescan.csv")
doj_hierarchy_2022lei <- fread("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/charge code table 1.7.22 with lei.csv")
doj_hierarchy_2022sec <- fread("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/charge code table 1.7.22 by section.csv")
# it looks like the livescan and lei ones have the same number of records, but the by section version has much fewer
# lei version has an additional column that looks like it is bcs code
# by section version has summary code, bcs code, and possible sentence
# columns that start with "maybe" are ones I'm not sure about what they mean and am guessing
#trash2 seems to be some kind of indicator, it is usually 1 but is 2 sometimes (it seems for prior enhancements) and 3 sometimes (it seems for sex registration things)
#trash1 is a mystery. Usually 1, but sometimes 2 or 4 (and once 3, whyyyy)
# there can be more than one cjis code for a given charge, charge_code, charge_level combination, the distinct will keep only one. the hierarchy value is usually the same (except 164 rows), and when not the same it is usually similar, except in a few weird instances.


names(doj_hierarchy_2022ls) <- c("trash1", "cjis_code", "trash2", "charge", "charge_code", "master_charge_description", "maybe_wobbler","charge_level", "maybe_commonabbreviation", "maybe_degree", "hierarchy", "enact_date", "repeal_date", "trash3")

names(doj_hierarchy_2022lei) <- c("trash1", "cjis_code", "trash2", "charge", "charge_code", "master_charge_description", "maybe_wobbler","charge_level", "maybe_commonabbreviation", "maybe_degree", "bcs_code", "hierarchy", "enact_date", "repeal_date", "trasn3")

names(doj_hierarchy_2022sec) <- c("summary_code", "bcs_code", "cjis_code", "charge_level", "charge", "charge_code", "master_charge_description", "possible_sentence")


doj_hierarchy_2022lei <- doj_hierarchy_2022lei %>%
  mutate(charge_level_orig = charge_level,
         charge_orig = charge,
         charge_code_orig = charge_code) %>%
  mutate(charge_level = toupper(charge_level),
         charge_level = if_else(!charge_level %in% c("F", "I", "M"), "Other", charge_level),
         charge = toupper(gsub("\\/|\\(|\\)| |\\.|\\.*|^[A-Z]*|\\\\.*|\\{|\\}|EX|\\+|","", charge)),
         charge_code = if_else(!charge_code %in% c("PC", "VC", "HS", "BP", "WI", "US"), "Other", charge_code)) %>%
  distinct(charge, charge_code, charge_level, .keep_all = T) %>%
  unite(join_var, charge_level, charge_code, charge, remove = F) %>%
  select(join_var, master_charge_description, hierarchy, charge_level, charge_code, charge, bcs_code, charge_level_orig, charge_orig, charge_code_orig) %>%
  mutate(bcs_code = as.numeric(bcs_code))

doj_hierarchy_2022sec <- doj_hierarchy_2022sec %>%
  mutate(charge_level_orig = charge_level,
         charge_orig = charge,
         charge_code_orig = charge_code) %>%
  mutate(charge_level = toupper(charge_level),
  ...
  distinct(charge, charge_code, charge_level, .keep_all = T) %>%
  unite(join_var, charge_level, charge_code, charge, remove = F) %>%
  select(join_var, master_charge_description, charge_level, charge_code, charge, bcs_code, summary_code, possible_sentence, charge_level_orig, charge_orig, charge_code_orig) %>%
  mutate(summary_code = as.numeric(summary_code),
         bcs_code = as.numeric(bcs_code))

doj_hierarchy_2022 <- doj_hierarchy_2022lei %>%
  left_join(doj_hierarchy_2022sec %>% select(-bcs_code, -master_charge_description, -charge_level_orig, -charge_orig, -charge_code_orig), by = c("join_var", "charge_level", "charge_code", "charge")) 
```
...
new_codes <- doj_hierarchy_2022 %>%
  anti_join(charge_hier, by = "join_var")

#2022 hierarchy has 415 new join_var values not found in previous charge hierarchy

write.xlsx(new_codes, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes with orig.xlsx", asTable = T, tableStyle = "TableStyleMedium1")

```
Diagnostic
```{r}
new_codes <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes with orig.xlsx")
...
Diagnostic
```{r}
doj_hierarchy_2022ls %>%
  mutate(charge_level = toupper(charge_level),
         charge_level = if_else(!charge_level %in% c("F", "I", "M"), "Other", charge_level),
         charge = toupper(gsub("\\/|\\(|\\)| |\\.|\\.*|^[A-Z]*|\\\\.*|\\{|\\}|EX|\\+|","", charge)),
         charge_code = if_else(!charge_code %in% c("PC", "VC", "HS", "BP", "WI", "US"), "Other", charge_code)) %>%
  add_count(charge, charge_code, charge_level) %>%
  filter(n>1) %>%
  add_count(charge, charge_code, charge_level, hierarchy) %>%
  filter(nn != n) %>%
  arrange(charge, charge_code, charge_level) %>%
  select(charge, charge_code, charge_level, cjis_code, hierarchy, master_charge_description, everything())
  #count(trash1)
  filter(trash1 ==3)
  
doj_hierarchy_2022lei %>%
  left_join(doj_hierarchy_2022sec, by = c("join_var", "charge_level", "charge_code", "charge")) %>%
  filter(bcs_code.x != bcs_code.y)

doj_hierarchy_2022lei %>%
  left_join(doj_hierarchy_2022sec %>% select(-bcs_code), by = c("join_var", "charge_level", "charge_code", "charge")) %>%
  filter(master_charge_description.x != master_charge_description.y) %>%
  select(join_var, master_charge_description.x, master_charge_description.y, everything())
```

Look at 2021 JBSIS DOJ codes
```{r}
jbsis_2021doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2021_September07_DOJ_Offense_Table_JBSIS.xlsx")
jbsis_2021exc <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2021_June07_DOJ_Offense_Table_JBSIS_Exclude.xlsx")

jbsis_2021 <- rbind(jbsis_2021doj, jbsis_2021exc)

names(jbsis_2021) <- c("summary_code", "bcs_code", "hierarchy", "charge_level_orig", "charge_orig", "charge_code_orig", "description", "possible_sentence", "jbsis_code")

jbsis_2021 <- jbsis_2021 %>%
  mutate(charge_level = toupper(charge_level_orig),
         charge_level = if_else(!charge_level %in% c("F", "I", "M"), "Other", charge_level),
         charge = toupper(gsub("\\/|\\(|\\)| |\\.|\\.*|^[A-Z]*|\\\\.*|\\{|\\}|EX|\\+|","", charge_orig)),
         charge_code = if_else(!charge_code_orig %in% c("PC", "VC", "HS", "BP", "WI", "US"), "Other", charge_code_orig)) %>%
  distinct(charge, charge_code, charge_level, .keep_all = T) %>%
  unite(join_var, charge_level, charge_code, charge, remove = F) %>%
  select(join_var, description, charge_level, charge_code, charge, bcs_code, summary_code, jbsis_code, possible_sentence, charge_level_orig, charge_orig, charge_code_orig) %>%
  mutate(summary_code = as.numeric(summary_code),
         bcs_code = as.numeric(bcs_code),
         jbsis_code = as.numeric(jbsis_code))

jbsis_2021 %>%
  anti_join(doj_hierarchy_2022, by = "join_var") %>%
  anti_join(charge_hier, by = "join_var")

doj_hierarchy_2022 %>%
  anti_join(jbsis_2021, by = "join_var") %>%
  filter(charge_level != "I")
...
new_codes <- new_codes %>%
  left_join(jbsis_2021 %>% select(join_var, jbsis_code)) 
...
new_codes <- new_codes %>%
  mutate(capital_flag = if_else(charge_code == "PC" & charge %in% c("37", "128", "206", "2095", "218", "2361C2", "273AB"), 1, 0),
         ...
         minor_flag = if_else(charge %in% c("fix this"), 1, 0),
         pre_arr_inel_flag = if_else(charge %in% c("fix this"), 1, 0),
         serious_felony_flag = 0,
         violent_felony_flag = 0,
         violent_psa_flag = 0) %>%
  mutate_at(vars(contains("flag")), funs(if_else(is.na(.), 0, as.numeric(.))))


write.xlsx(new_codes, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes with incorrect flags.xlsx", asTable = T, tableStyle = "TableStyleMedium1")
```

```{r}
new_codes <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes with incorrect flags.xlsx")

new_codes %>%
  select(contains("flag"))

# need to add special_flag
# remove sb10 flags
# remove minor flag
# add dv_possible_flag (maybe? why?)

new_codes <- new_codes %>%
  select(-contains("sb10"), -minor_flag) %>%
  mutate(special_flag = 0,
         dv_possible_flag = 0) %>%
  mutate(charge_level = charge_level_orig,
         charge_code = charge_code_orig,
         charge = charge_orig) %>%
  select(-contains("orig"))

write.xlsx(new_codes, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes for lawyer review.xlsx", asTable = T, tableStyle = "TableStyleMedium1")

new_codes_mf <- new_codes %>%
  filter(charge_level != "I")

write.xlsx(new_codes_mf, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes for lawyer review without infractions.xlsx", asTable = T, tableStyle = "TableStyleMedium1")
...
charge_hier %>%
  bind_rows(new_codes) %>%
  filter(is.na(jbsis_code) | jbsis_code == 1000)

charge_hier %>%
  select(contains("flag"))

setdiff(names(new_codes_mf), names(charge_hier))


```
#fix missing summary codes using bcs codes
```{r}
load("bcs and summary code mapping.RData")
load("bcs and jbsis code mapping.RData")


bcs
new_codes_mf %>%
  count(is.na(summary_code))

# add missing summary codes using bcs codes
new_codesb <- new_codes_mf %>%
  left_join(bcs, by = 'bcs_code') %>%
  mutate(summary_code = if_else(!is.na(summary_code.x), summary_code.x, summary_code.y)) %>%
  select(-summary_code.x, -summary_code.y) 

new_codes_mf %>%
  count(is.na(jbsis_code))

# add missing jbsis codes using bcs codes
new_codes_jb <- new_codesb %>%
  left_join(bcsj, by = 'bcs_code') %>%
  mutate(jbsis_code = if_else(!is.na(jbsis_code.x), jbsis_code.x, jbsis_code.y)) %>%
  select(-jbsis_code.x, -jbsis_code.y) 

#remove flags so we can re-add with the supplemented codes
new_codes_jb <- new_codes_jb %>%
  select(-contains("flag"))

# re-create flags

new_codes_flagged <- new_codes_jb %>%
  mutate(capital_flag = if_else(charge_code == "PC" & charge %in% c("37", "128", "206", "2095", "218", "2361C2", "273AB"), 1, 0),
         dv_flag = if_else(grepl("DOMESTIC|SPOUSE|PARTNER|COHABITANT", master_charge_description) &
                             !grepl("MARRY|CRT ORD|COURT ORDER|FOREIGN|LAND|FAIL|SHELTER", master_charge_description), 1, 0),
         #Attach Flags DV and Sup Vio Flags #Sup Vio Based on Charge Descriptions--Including Flash Incarceration
...
#write.xlsx(new_codes_flagged, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes for lawyer review.xlsx", asTable = T, tableStyle = "TableStyleMedium1")
```
# new additions august 2022
Add on flags reviewed by lawyers
```{r}
# load prior charge hierarchy
load("Complete Charge Code Hierarchy_SL.RData")

# read in new codes flagged by lawyers
kara_codes <- read.xlsx("New 2022 codes for lawyer review_Kara_1thru82.xlsx")
kara_codes <- kara_codes %>%
  slice(1:81)

sarah_codes <- read.xlsx("New 2022 codes for lawyer review_Sarah_83thru164.xlsx")
sarah_codes <- sarah_codes %>%
  slice(82:163) %>%
  select(-Column1, -Column2)

jamie_codes <- read.xlsx("New 2022 codes for lawyer review_Jamie_165thru247.xlsx")
jamie_codes <- jamie_codes %>%
  slice(164:246)

lawyer_codes <- kara_codes %>%
  bind_rows(sarah_codes) %>%
  bind_rows(jamie_codes)

# join new codes to full hierarchy
charge_hier22 <- charge_hier %>%
  bind_rows(lawyer_codes)

# add gang flag
charge_hier22 <- charge_hier22 %>%
  mutate(gang_flag = if_else(grepl("GANG", master_charge_description), 1, 0))

save(charge_hier22, file = "charge hierarchy with added codes august 2022.Rdata")
write.xlsx(charge_hier22, file = "charge hierarchy with added codes august 2022.xlsx")
```

Remove special characters
```{r}
charge_hier22 <- charge_hier22 %>%
  mutate(master_charge_description = gsub("[^a-zA-Z0-9]", "_", master_charge_description))
save(charge_hier22, file = "charge hierarchy with added codes august 2022 spchr rmvd.Rdata")
write.xlsx(charge_hier22, file = "charge hierarchy with added codes august 2022 spchr rmvd.xlsx")
```

Diagnostic
```{r}
load("charge hierarchy with added codes august 2022.Rdata")
charge_hier22

charge_hier22 %>%
  #filter(grepl("\u20AC", master_charge_description, ignore.case = T))
  filter(grepl("\".*\"", master_charge_description, ignore.case = T))


test_case <- charge_hier22 %>%
  filter(grepl("\u20AC", master_charge_description, ignore.case = T))
...
stri_extract_all_regex(charge_hier22$master_charge_description, "\\p{S}")
```


# xxx not currently used, would need lawyer consult xxx Combine historical JBSIS doj files from Savet
```{r}
jbsis_2021doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2021_September07_DOJ_Offense_Table_JBSIS.xlsx")
jbsis_2020doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2020_March04_DOJ_Offense_Table_JBSIS.xlsx")
jbsis_2019doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2019_January14_DOJ_Offense_Table_JBSIS.xlsx")
jbsis_2018doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/2018_March26_DOJ_Offense_Table_JBSIS.xlsx")
jbsis_2016doj <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/Copy of 2016_Nov21_DOJ_Offense_Table_JBSIS.xlsx")

jbsis_comb <- rbind(jbsis_2021doj, jbsis_2020doj, jbsis_2019doj, jbsis_2018doj, jbsis_2016doj)

names(jbsis_comb) <- c("summary_code", "bcs_code", "hierarchy", "charge_level_orig", "charge_orig", "charge_code_orig", "description", "possible_sentence", "jbsis_code")
...
new_codes <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes for lawyer review.xlsx")
#new_codes <- read.xlsx("G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes.xlsx")
load("G:/CrimJustice/PretrialAssessmentPilot/Complete Charge Code Hierarchy_SL.RData")

bind_rows(new_codes, charge_hier) %>%
  distinct(join_var, .keep_all = T) %>%
  anti_join(jbsis_comb, by = "join_var")

missing_jbsis <- new_codes %>%
  anti_join(jbsis_comb, by = "join_var") %>%
  filter(is.na(jbsis_code))


# THIS IS NOT WORKING ANYWAY
# match jbsis codes to charges with missing jbsis codes determined by the base charge, when there is no conflict of jbsis codes.
# also match summary codes to charges with missing summary codes in the same way
### Ask a lawyer about this, whether this is legitimate to do
filled_in <- bind_rows(new_codes, charge_hier) %>%
  mutate(charge_start = as.integer(parse_number(charge)),
         jbsis_code = if_else(is.na(jbsis_code)|jbsis_code == 0, 1000, jbsis_code),
         summary_code = if_else(is.na(summary_code)|summary_code == 0, 1000, summary_code)) %>%
  group_by(charge_level, charge_code, charge_start) %>%
  ...
  mutate(jbsis_code = if_else(jbsis_code == 1000 & n %in% 0:1, min(jbsis_code), jbsis_code),
         summary_code = if_else(summary_code == 1000 & ns %in% 0:1, min(summary_code), summary_code),
         jbsis_filled_in = if_else(jbsis_code == 1000 & n %in% 1, 1, 0),
         summ_filled_in = if_else(summary_code == 1000 & ns %in% 1, 1, 0)) %>%
  ungroup() %>%
  #Fixing property and drug flags to be off of summary codes
  mutate(property_flag = if_else(summary_code %in% c(8:11, 24, 31:33, 64), 1, 0),
         drug_flag = if_else(summary_code %in% c(12:15, 34:36), 1, 0)) %>%
  select(-n, -ns, -charge_start)


# %>%
#   count(summary_code == 1000, jbsis_code == 1000)

# need to go back and re-add bcs codes to the charge_hier if i want to use those for anything, they were removed


#isolate instances where the codes would be getting filled in

filled_in %>%
  filter(jbsis_code == 1000) %>%
  anti_join(jbsis_2021exc, by = "join_var") %>%
  count(charge_code)
...
doj_hierarchy_2022lei %>%
  anti_join(doj_hierarchy_2022sec, by = "join_var") %>%
  filter(charge_level != "I") %>%
  filter(parse_number(charge) == 243)
...
filled_in %>%
  filter(summ_filled_in == 1 & summary_code != 1000)
  filter(jbsis_filled_in == 1 & jbsis_code != 1000)
```
