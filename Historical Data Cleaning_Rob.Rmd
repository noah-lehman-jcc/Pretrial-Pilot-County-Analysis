...
####
  ...
  ## Check why there are ~300 duplicate book_num values
  ## what does local_id mean?
  ## why not more individuals with no release date?
...
# Booking Collapse--See pg. xxx for more details about this process.
  ## Some 13 book_nums are still duplicated after this collapse because of different court case ids
...
# Court Case Data Import -- Yuba, 2015-01-01 to 2019-12-31
  ## court_status contains one per row and includes information on the 
...
# Court Filing Charges 
  ## court_charge contains one row per disposition including VOPs stemming from original case NEED TO FILTER OUT DISPOS AFTER FINAL_DATE FOR SAME CASE 
...
# Court Hearings
  ## court_hearing contains one row per hearing event (check this) on each court case and includes informatioin about each hearing. 
...
# Court FTAs
  ## court_fta contains one hearing at which fta occurred and warrant was issued per row
...
# Standardize Factors--See pg. xxx for more details.
court_sent_prob <- court_sent_prob %>%
  mutate(sentence_type = case_when(sentence_type_original %in% c("Court (Summary/Unsupervised)", "Formal (Supervised)") ~ "Probation"))

####

# Court person
  ## court person contains one row per case/docket id
court_person <- lapply(c("Yuba/Yuba.xlsx", "Yuba SQL Results 1-30-2020.xlsx"),
                       function(x) {
                         read_excel(paste0("Historical Data Submissions/Yuba/",
                                           x),
...
# Variable name standardization--See pg. xxx for more details. 
names(court_person) <- c("court_docket_id", "court_case_id", "last_name", "first_name", "middle_name", "file_date", "dob", "cii", "cdl_id", "fbi", "sex_original", "race_original", "cdl_state", "risk_report_date")
...
court_sent <- bind_rows(court_sentence, court_sen_prob) %>%
  arrange(sentence_date) %>%
  distinct(court_case_id, .keep_all = T) %>%
  select(court_case_id, court_docket_id, sentence_date, sentence_type_original, sentence_type)

# All data are joined.
court_df <- reduce(list(court_df, court_dispo, court_fta_war, court_hearing, court_plea, court_sent), left_join, by = "court_case_id") %>%
  select(-contains(".y")) %>%
