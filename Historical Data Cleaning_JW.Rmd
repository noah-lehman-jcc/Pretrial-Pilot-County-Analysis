...
court_charge <- court_charge %>%
  ...
  mutate(court_charge_code = gsub("[0-9].*", "", charge_description),
...
court_hearing <- court_hearing %>%
  ...
  distinct(court_case_id, .keep_all = T) %>%
  select(court_case_id, case_num, arraignment_date)
...
court_sent <- court_sent %>%
  ...
  distinct(court_case_id, .keep_all = T) %>%
  select(court_case_id, case_num, sentence_date, sentence_type_original, sentence_type)
...
court_df <- reduce(list(court_person, court_dispo, court_fta_war, court_hearing, court_plea, court_sent, court_probation), left_join, by = "court_case_id") %>%
...
Jail Data
```{r}
# Jail Bookings Data Import--- Modoc, 
...
court_charge <- court_charge %>%
  mutate(attempt_flag = if_else(grepl("664", court_charge_original), 1, 0),
         court_charge = if_else(grepl("^PC664$|664/[A-Z]", court_charge_original), gsub("PC664/|PC664", "", court_charge_original), gsub("664/", "", court_charge_original))) %>%
  separate_rows(court_charge, sep = "/") %>%
  ...
  court_charge = toupper(gsub("\\/|\\(|\\)| |\\.|\\.*|^[A-Z]*|\\\\.*|\\{|\\}", "", court_charge)),
  # These variables did not match the initial hierarchy. Check if they match the new hierarchy after the joining.
  court_charge_code = if_else(court_charge %in% c("460B", "488", "460A"), "PC", court_charge_code),
...
# Variable name standardization--See pg. xxx for more details.
names(court_sent) <- c("court_case_id", "sentence_date", "sentence_type_original", "term", "term_suspended", "sentence_location", "sentence_status", "count", "court_charge_original")

# Date Reformatting--See pg. xxx for more details.
court_sent <- court_sent %>%
  mutate(sentence_date = as_date(sentence_date))

# Charge Standardization--See pg. xxx for more details.
court_sent <- court_sent %>%
  mutate(attempt_flag = if_else(grepl("664/", court_charge_original), 1, 0),
         court_charge = if_else(grepl("664/[A-Z]", court_charge_original), gsub("PC664/", "", court_charge_original), gsub("664/", "", court_charge_original))) %>%
  separate_rows(court_charge, sep = "/") %>%
  mutate(court_charge_code = case_when(
    grepl("PC", court_charge, ignore.case = T) ~ "PC",
    grepl("VC", court_charge, ignore.case = T) ~ "VC",
