...
Connection
```{r}
con <- dbConnect(odbc::odbc(), "snowflake_pretrial", uid ="Sal.Lempert@jud.ca.gov", pwd = "")
...
eval_df <- full_df %>%
  filter(booking_assessed_count == 1,
         ...
         tool_name %in% c("ORAS", "PSA", "VPRAI", "VPRAIR")
         ) %>%
...
```{r}
race_df <- eval_df %>%
  mutate(race = if_else(race %in% c("Black", "White", "Hispanic", "Asian"), race, "Other_Unknown")) %>%
  count(tool_name, race) %>%
  group_by(tool_name) %>%
  mutate(Total = sum(n)) %>%
  pivot_wider(names_from =  race, values_from =  n, values_fill = 0) %>%
  select(tool_name, Black, White, Hispanic, Asian, "Other" = Other_Unknown, Total) %>%
  bind_rows(eval_df %>%
              mutate(race = if_else(race %in% c("Black", "White", "Hispanic", "Asian"), race, "Other_Unknown")) %>%
              count(race) %>%
              mutate(Total = sum(n)) %>%
              ...
              select(tool_name, Black, White, Hispanic, Asian, "Other" = Other_Unknown, Total)) %>%
  mutate_at(vars("Black", "White", "Hispanic", "Asian", "Other"), funs(round(./Total*100)))
...
risk_df <- eval_df %>%
   # filter(book_date <= as_date("2021-03-31"),
   #       book_date >= as_date("2019-10-01")) %>%
  filter(tool_name == "VPRAI", !is.na(vprai_risk_score), county == "Alameda") %>%
  group_by(vprai_risk_score) %>%
  summarise(across(c(fta_flag, recid_arrested_flag, recid_filed_flag, recid_convicted_flag, 
                    any_pretrial_failure_flag, recid_arrest_violent_psa_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
...
ggplot(risk_df %>% distinct(vprai_risk_score, Total)%>% mutate(percent = Total/sum(Total))) +
  ...
  # ylim(0,100) +
  # facet_wrap(~name) +
  labs(x = "Risk Category", y = "Percent", title = "Alameda Distribution of Assessments by VPRAI Risk Category")  

ggplot(risk_df) +
  aes(x = as.factor(vprai_risk_score), y = value) +
  geom_col(fill = "blue") +
  geom_text(vjust = -.3, aes(label = paste0(value, "%")), size = 2.6) +
  theme_bw() +
  ylim(0,100) +
  facet_wrap(~name) +
  labs(x = "Risk Category", y = "Percent", title = "Alameda VPRAI Outcomes by Risk Category")
...
race_df <- eval_df %>%
  bind_rows(race_df) %>%
  mutate(race = factor(race, levels = c("All", "White", "Black", "Hispanic", "Asian", "Other_Unknown")))
...
roc_df <- lapply(c("All", "White", "Black", "Hispanic", "Asian"), 
                 function(i){
...
```{r}
tool_df <- auc_df %>%
  filter(tool_name == "VPRAI", race == "All", sex == "All", county == "Alameda") %>%
  select(outcome, AUC, CI, n) %>%
  mutate(outcome = c("FTA", "New Arrest", "New Filing","New Violent Arrest", "FTA or New Arrest"))
...
```{r include=FALSE}
race_df <- eval_df %>%
  mutate(race = factor(race, levels = c("White", "Black", "Hispanic", "Asian", "Other_Unknown"))) %>%
...
VPRAI--With Interactions at score 6 XXXXXXXX there is no VPRAI score 5
\newline
```{r results='asis'}
...
VPRAI  --- removed temporarily until San Joaquin data augments cell size
\newline
```{r eval=FALSE, include=FALSE}
tool_df <- auc_df %>%
  filter(tool_name == "VPRAI", race != "All", sex == "All", county == "All") %>%
...
tool_df <- auc_df %>%
  ...
  mutate(risk_score = rep(c("PSA FTA", rep("PSA NCA",3), "PSA NVCA"),4),
         outcome = rep(c("FTA", "New Arrest", "New Filing", "New Conviction", "New Violent Arrest"),4))
...
ggplot(risk_df %>% distinct(vprai_risk_score, Total) %>% 
  ...
  geom_text(vjust = -.2, aes(label = str_c(round(percent*100), "%")), size = 2.6, position = position_dodge(width = .9)) +
  theme_bw() +   scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(x = "Risk Category", y = "Percent", title = "Alameda Distribution of Assessments by VPRAI Risk Category", fill = "Gender")  
...
San Joaquin  --- removed until data are fixed
\newline
```{r eval=FALSE, include=FALSE}
risk_df <- sex_df %>%
   # filter(book_date <= as_date("2021-03-31"),
...
San Joaquin--Without Interactions  --- removed until data are fixed
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- sex_df %>%
  filter(county == "San Joaquin") %>%
...
San Joaquin--With Interactions --- removed until data are fixed
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- sex_df %>%
  filter(county == "San Joaquin") %>%
...
VPRAI AUC race comparison --- removed until San Joaquin is fixed
```{r eval=FALSE, include=FALSE}
