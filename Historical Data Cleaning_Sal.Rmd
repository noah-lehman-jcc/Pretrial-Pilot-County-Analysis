...
# original_df[[3]] <- tibble(table_name = "book_charge", original_field = original_field, modified_field = names(book_charge))
...
# check with anti join


anti_join(book_charge, book_df, by = c("book_num")) # %>% distinct(book_num) %>% arrange(book_num)
anti_join(book_df, book_charge, by = c("book_num", "court_case_id")) # %>% distinct(book_num) %>% arrange(book_num)

book_df <- book_df %>% 
  mutate(warrant_flag = if_else(grepl("Warrant", book_type), 1, 0),
         onview_flag = if_else(book_type == "On_View", 1, 0)) %>%
  group_by(book_num, court_case_id) %>%
  mutate(warrant_flag = max(warrant_flag)) %>%
  add_count() %>%
  filter(n == 1 | onview_flag == 1 | sum(onview_flag) == 0) %>%
  ungroup() %>%
  distinct(book_num, court_case_id, .keep_all = TRUE)
  
  

book_df <- left_join(book_df, book_charge, by = c("book_num", "court_case_id")) %>% mutate(local_id.x = as.numeric(local_id.x)) %>% rename()
book_df <- left_join(book_df, person_df, by = c("local_id.x" = "local_id"))

book_df <- book_df %>%
  rename(local_id = local_id.x)
...
#original_df[[7]] <- tibble(table_name = "court_df", original_field = original_field, modified_field = names(court_df))
...
#original_df[[8]] <- tibble(table_name = "court_charge", original_field = original_field, modified_field = names(court_charge))
...
#original_field <- names(court_war)

names(court_war) <- c("court_case_id", "local_id", "war_date", "warrant_description", "warrant_code", "warrant_type")

#original_df[[9]] <- tibble(table_name = "court_war", original_field = original_field, modified_field = names(court_war))
...
#original_df[[10]] <- tibble(table_name = "court_fta", original_field = original_field, modified_field = names(court_fta))
...
#original_df[[11]] <- tibble(table_name = "court_hearing", original_field = original_field, modified_field = names(court_hearing))
...
court_df <- reduce(list(court_dispo, court_fta, court_war, court_hearing), left_join, by = "court_case_id") %>%
  select(-contains(".y")) %>%
  select(-contains(".x.x")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%
  mutate(final_disposition_date = if_else(disposition_date <= sentence_date|is.na(sentence_date), disposition_date, sentence_date),
