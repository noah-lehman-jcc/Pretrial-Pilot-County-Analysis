...
assess_table <- assess_df %>%
  ...
  select(county, risk_level, release_recommendation, release_decision, assessment_date, vprair_risk_score, tool_name, assessment_date, dob, sex, race, book_num)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Alameda Assessment Output 09-2020.csv")
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Los Angeles Assessment Output 09-2020.csv")
...
court_person %>%
  semi_join(court_fta_war, by = "court_case_id") 
```
...
assess_table <- assess_df %>%
  ...
  # Select relevants variables.
  select(county, risk_level, release_recommendation, release_decision, assessment_date, vprai_risk_score, vprair_risk_score, tool_name, assessment_date, dob, sex, race, pretrial_termination_outcome, pretrial_termination_reason)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Santa Barbara Assessment Output 09-2020.csv")
...
assess_table <- assess_df %>%
  ...
  # Select relevants variables.
  select(county, release_recommendation, release_decision, assessment_date, psa_nvca_risk_score, psa_nca_risk_score, psa_fta_risk_score, tool_name, assessment_date, dob, sex, race, pretrial_termination_outcome, pretrial_termination_reason)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Tulare Assessment Output 09-2020.csv")
...
# Assessments Data Import
  ## assess_df contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The questions are widened into columns.
assess_df <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/P2 Submission-June 30/Pre-Trial Spreadsheet April-June 2020.xlsx")

assess_df2 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/P2 Submission-June 30/6.30.20 - 9.30.20 Probation Pretrial Spreadsheet.xlsx")

bind_rows(assess_df, assess_df2)

# Start Here.
names(assess_df)[!names(assess_df) %in% names(assess_df2)]
...
assess_df <- assess_df %>%
  ...
  mutate_at(vars(contains("date"), -court_date_reminder_original, -arrest_date_time), 
...
assess_df <- assess_df %>%
  mutate(tool_name = "tuol_co_pretrial",
         ...
         risk_level = case_when(risk_level_original %in% c("High", "Mod-High") ~ "High",
