...
save(charge_hier, file = "Complete Charge Code Hierarchy")
```
...
court_df %>%
  semi_join(date_df, by = c("cii", "file_date"))
  arrange(desc(n))
...
court_join <- court_df %>%
  ...
  distinct(court_case_id, hearing_date, .keep_all = T) %>%
  # distinct(cii, court_case_id, hearing_date, hearing_type_original) %>%
  semi_join(date_df, by = c("cii", "hearing_date" = "file_date")) 
  # count(hearing_type_original) %>%
  # arrange(desc(n))
...
book_df %>%
  semi_join(court_df, by = "court_docket_id")

court_join %>%
  distinct(court_case_id, court_docket_id, .keep_all = T) 

court_df %>%
  semi_join(book_df, by = "court_docket_id")

date_df %>%
  filter(!is.na(arrest_date), !is.na(file_date))
...
date_df %>%
  filter(!is.na(disposition_date), !is.na(file_date)) %>%
...
court_df %>% 
  left_join(date_df, by = c("cii")) %>%
  filter(file_date.x - file_date.y <= 100, file_date.x - file_date.y >= -100) %>%
  distinct(cii, file_date.x)
  select(file_date.x, file_date.y)

date_df %>%
  distinct(cii, file_date)

court_join_2 <- court_df %>%
  semi_join(book_df, by = "court_docket_id")

doj_case_match <- unique(court_join$court_docket_id)

docket_case_match <- unique(court_join_2$court_docket_id)

length(intersect(doj_case_match, docket_case_match))
