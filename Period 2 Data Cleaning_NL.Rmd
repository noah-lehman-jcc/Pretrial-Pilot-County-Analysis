...
charge_df %>%
  count(notes) %>%
  arrange(desc(n))
  filter(missing_offense)
  
df_list

charge_df %>%
  # filter(join_var == "F_1361C")
  filter(!grepl("_.*_", join_var)) 
  view()
```
...
assess_df3 <- assess_df3 %>%
  mutate(rec_em_flag = if_else(grepl("COURT ORDERED EM|FOUND SUITABLE", toupper(release_recommendation_original)), 
         ...
         condition_em_flag = if_else(release_decision_original == "ELECTRONIC MONITORING", 1, 0),
         condition_drug_test_flag = if_else(release_decision_original == "ALCOHOL MONITORING", 1, 0))
