...
knitr::opts_knit$set(root.dir = "C:/Work/PretrialAssessmentPilot")
```
...
```{r}
file <- "C:/Work/PretrialAssessmentPilot/Charge Definitions and Hierarchy/Legal Review of Offense Flags.xlsx"
...
book_df <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/2_Arrest And Booking.csv")
...
book_df <- book_df %>%
  ...
  mutate_at(vars(arrest_date, book_date, release_date), mdy) %>%
  select(-arrest_time, -book_time) %>%
...
# Jail Booking Charges Data Import.
  ## book_charge contains one row per charge and includes the charge information on each of the bookings from book_df which are joined on the book_num.  
book_charge <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/2a_Arrest and Booking Charges.csv")
...
book_charge <- book_charge %>%
  mutate(attempt_flag = if_else(grepl("664", arrest_charge_original), 1, 0),
         arrest_charge_original = if_else(arrest_charge_original == "RE-ARREST", "REARREST", arrest_charge_original),
         arrest_charge_original = if_else(arrest_charge_original == "NULL", NA_character_, arrest_charge_original)) %>%
...
book_df <- left_join(book_df, book_charge %>% mutate(booking_seq = as.integer(booking_seq)))
...
# fwrite(book_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Booking Collapse.csv")
```

Alameda Extra
```{r}
# df1 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Alameda/P2 Submission-June 30/1_Identifiers-and-Demographics.xls")

files <- list.files("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020", full.names = T)
...
# Court Filing Charges 
  ## court_charge contains one row per each charge associated with a court case and includes information on the charges and dispostions of the court case. 
court_charge <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/6_Charges.csv")
...
# Court Dispositions
court_dispo <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/9_Dispositions.csv")

# Variable name standardization--See pg. xxx for more details. 
names(court_dispo) <- c("event_id", "local_id", "court_case_id", "court_docket_id", "court_charge_id", "court_charge_count", "court_charge_sequence_number", "court_charge_stage", "disposition_date", "court_charge_level_original", "court_charge_original", "court_charge_description", "disposition_type_original")
...
court_hearing <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/7_Hearings.csv")

# Variable name standardization--See pg. xxx for more details.
names(court_hearing) <- c("event_id", "local_id", "court_case_id", "court_docket_id", "hearing_type_original", "hearing_date_time", "hearing_result_original", "fta_all_flag", "fta_flag")


# Date Reformatting--See pg. xxx for more details.
court_hearing <- court_hearing %>%
  mutate(hearing_date_time = as_datetime(paste(hearing_date_time), tz = "America/Los_Angeles"),
         hearing_date = as_date(hearing_date_time),
         fta_date = if_else(fta_all_flag == "Yes", hearing_date, NA_Date_)) 
...
court_sent <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/11_Sentence.csv")
...
court_sent <- court_sent %>%
  mutate_at(vars(contains("date")), funs(if_else(grepl("-", .), as_date(.), mdy(.)))) %>%
  distinct()
...
# Court Pleas 
  ## court_sent contains one row per sentence event component in which the sentence event is confinement and one NULL row for each court case without a sentence event. Sometimes terms for each sentence event are split across rows and are considered different sentence components.  
court_plea <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/8_Pleas.csv")
...
court_charge2 <- court_charge %>%
  select(-file_date) %>%
  anti_join(court_dispo, by = c("court_charge_id", "event_id"))

  ## Combining
court_charge2 <- bind_rows(court_dispo, court_charge2)
...
fwrite(court_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Court Collapse.csv")
```
...
assess_rec <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/3_Assessment And Recommendation.csv", colClasses = "character")
...
assess_prearraignment <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/4_Prearraignment Order.csv", colClasses = "character")
...
assess_arraignment <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/5_Arraignment Order.csv", colClasses = "character")
...
assess_rec_conditions <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/3a_Assessment and Recommendation - DPO Conditions.csv", colClasses = "character", fill = T)
...
assess_prearraignment_conditions <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/4a_Prearraignment Order - Judge Conditions.csv", colClasses = "character")
...
assess_arraignment_conditions <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/5a_Arraignment Order - Judge Conditions.csv", colClasses = "character")
...
assess_conditions <- bind_rows(assess_rec_conditions, assess_prearraignment_conditions, assess_arraignment_conditions)
...
fwrite(assess_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Assessment Collapse.csv")
```
...
book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Booking Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Court Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date)

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date)
...
join_collapse <- join_df2 %>%
  ...
  ungroup() %>%
  filter(pretrial_end_date >= book_date, book_type %in% c("On_View", "New_Warrant", "Other_Warrant")) %>%
  group_by(event_id) %>%
  mutate(across(contains("flag"), max)) %>%
  ungroup() %>%
  distinct(event_id, .keep_all = T)

fwrite(join_collapse, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Alameda Join Collapse.csv")
``` 
  Join Diagnostics
```{r}
...
cii_df <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Alameda/P2 Submission-June 30/1_Identifiers-and-Demographics.xls")  

names(cii_df) <- c("local_id", "court_docket_id", "cii", "fbi", "local_id2", "last_name", "first_name", "dob", "cdl_id", "cdl_state", "court_case_id",  "sex_original", "race_original", "employed", "zip_code")
...
#CIIs for DOJ
doj_cii <- bind_rows(doj_integrated %>% select(cii), doj_court %>% select(cii), doj_jail %>% select(cii)) %>%
  select(cii) %>%
  distinct() %>%
  filter(nchar(cii) == 10)
...
write.xlsx(list(doj_cii, doj_integrated, doj_jail, doj_court), file = "CIIs for DOJ/October 2020/Alameda P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "integrated", "jail", "court"))
...
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, vprair_risk_score, tool_name, dob, sex, race, risk_level, monitor_level, pretrial_termination_reason)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Alameda Assessment Output 10-2020.csv")
...
# fwrite(book_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Booking Collapse.csv")
```
...
fwrite(court_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Court Collapse.csv")

# court_df %>%
#   filter(court_case_id == 9999889) %>%
#   group_by(court_case_id) %>%
#   mutate_at(vars(contains("flag")), funs(if_else(is.na(.), 0, .))) %>%
#     ## Add this back in later. It's now working right now due to computational issues.
#   # mutate_at(vars(contains("flag")), funs("count" = sum(.))) %>%
...
# Assessments Data Import
  ## assess_df contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The questions are widened into columns.
assess_df <- fread("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Los Angeles/P2 Submission-June 30/ISAB_PsaHistoricalExtract_20201021.csv",
                  sep = "|", colClasses = "character")
...
assess_df2 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Los Angeles/P2 Submission-June 30/Los Angeles County Pretrial Services Bureau - JCC Quarterly SB10 Data (2020-04-01 to 2020-06-30) - Revised.xlsx", sheet = 2, col_types = "text")


names_df2 <- tibble(original_field = names(assess_df2)) 

names(assess_df2) <- c("pretrial_id", "cii", "fbi", "local_id", "cdl_id", "first_name", "last_name", "dob", "sex_original", "race_original", "pretrial_stage", "tool_name", "assessment_date", "zip_code", "ccat_tool_response_1", "ccat_tool_response_2", "ccat_tool_response_3", "ccat_tool_response_4", "ccat_tool_response_5", "ccat_tool_response_6", "ccat_tool_response_7", "ccat_tool_response_8", "ccat_tool_response_9", "ccat_tool_response_10", "ccat_tool_response_11", "ccat_tool_response_12", "ccat_tool_response_13", "ccat_tool_response_14", "ccat_tool_response_15", "ccat_tool_response_16", "ccat_tool_response_17", "ccat_tool_response_18", "ccat_tool_response_19", "ccat_tool_response_20","risk_level_original", "release_recommendation_original", "release_authorization_original", "release_decision_original", "pretrial_release_date", "release_conditions_original", "pretrial_violation_original", "pretrial_violation_date", "court_date_reminder_original", "other_pretrial_service_original", "pretrial_termination_outcome_original", "pretrial_termination_date")
...
#Ask about EM in LA.
assess_df3 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Los Angeles/P2 Submission-June 30/Los Angeles County Pretrial Services Bureau - JCC Quarterly SB10 Data (2020-04-01 to 2020-06-30) - Revised.xlsx", sheet = 3, col_types = "text")

names_df3 <- tibble(original_field = names(assess_df3)) 
...
names_df3$field <- names(assess_df3) 
...
fwrite(assess_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Assessment Collapse.csv")
```
...
Join
```{r}
 book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Booking Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date) %>%
  mutate(court_docket_id = court_case_id) %>%
  select(-court_case_id)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Court Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date) %>%
  mutate(court_docket_id = gsub("^...", "", court_docket_id),
         court_docket_id = gsub(" ", "", court_docket_id)) 

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date")), as_date)

date_df <- fread("C:/Work/PretrialAssessmentPilot/County Join Dates/Los Angeles Join Dates.csv", colClasses = "character") %>%
  # rename(file_date = filing_date) %>%
  mutate_at(vars(contains("date")), as_date)
...
# Match rate when matching on court_docket_id from booking to court data ~ 63%
nrow(book_df %>%
  semi_join(court_df, by = c("court_docket_id"), na_matches = "never")) / nrow(book_df) * 100

# Match rate when matching on court_docket_id from booking to court data ~ 47%
nrow(court_df %>%
...
nrow(assess_df %>%
  ...
  filter(assessment_date <= book_date + 10) %>%
...
join_collapse <- join_df2 %>%
  ...
  mutate(across(contains("sup_vio"), max)) %>%
  ungroup() %>%
  filter(final_disposition_date >= book_date) %>%
  group_by(book_num) %>%
  mutate(across(contains("flag"), max)) %>%
  ungroup() %>%
  distinct(book_num, .keep_all = T)

fwrite(join_collapse, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Los Angeles Join Collapse.csv")
``` 
...
write.xlsx(list(factor_df, flag_df, date_df), file = "C:/Work/PretrialAssessmentPilot/Code Books/Assessment Code Book/Los Angeles Assessment Code Book EM 10-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
```
...
doj_book <- bind_rows(df_list[[3]])

# CIIs from court tables
doj_court <- bind_rows(df_list[[4]]) 
...
#Create an excel workbook with all data table. Make sure to include the cii table first. 
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "CIIs for DOJ/October 2020/Los Angeles P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Los Angeles Assessment Output 10-2020-v2.csv")
...
book_df3 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Santa Barbara/P2 Submission-June 30/JailData.Jan.Jun.20.xlsx", col_types = "text")
...
book_df <- bind_rows(book_df, book_df2, book_df3) %>% distinct()
...
fwrite(book_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Booking Collapse.csv")
```
...
court_charge <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/Charges.csv", header = F)
...
# Court hearing data
  ## court_hearing contains one row per hearing event on each court case and includes information on the hearing type and result. 
court_hearing <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/Hearings.csv", header = F)
...
# Court Sentences to Confinement 
  ## court_sent contains one row per sentence event and term/sentence location combinations. E.g. The same case will appear twice if they receive a term of 3 years and 80 days. Additionally, some exact duplicates are removed.    
court_sent <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/SentConf.csv")
...
# Court Sentences to Supervision
  ## court_sent2 contains one row per sentence event component in which the sentence event is supervision (e.g. probation). Many court_case_ids have multiple sentence events to supervision, unlike confinement. Exact duplicates are removed.  
court_sent2 <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/SentProb.csv", header = F)
...
# Court Person IDs
  ## Court person contains one row per defendant, court_case, and assessment date combination. It includes information on the person, the filing date, and the assessment date.--Seperate in the collapse.    
court_person <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/TechElements.csv", header = F)
...
# Court FTA Data
  ## court_fta_war contains one row per fta instance on each court case. Some court cases have an fta but are missing the fta date. 
court_fta_war <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/Warrants.csv", header = F)
...
#Court Status
  ## Court status shows whether cases are active or inactive on a given date.
court_status <- fread("Historical Data Submissions/Santa Barbara/P2 Submission-June 30/CaseStatus.csv", header = F)
...
# fwrite(court_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Court Collapse.csv")
```
...
# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("pretrial_id", "cii", "court_case_id", "court_docket_id", "fbi", "local_id", "cdl_id", "first_name", "last_name", "dob", "sex_original", "race_original", "hispanic_flag", "tool_name", "assessment_date_time", "zip_code", "vprair_risk_score", "release_recommendation_original", "recommendation_comments", "release_authorization_original", "pretrial_release_type_original", "release_date", "release_conditions_original", "pretrial_violation_original", "pretrial_violation_date",  "court_date_reminder_original", "other_pretrial_service_original", "pretrial_termination_reason_original", "pretrial_termination_date", "open_date", "assignment_date", "arrest_date", "vprair_tool_response_1", "vprair_tool_response_4", "vprair_tool_response_2", "vprair_tool_response_8", "vprair_tool_response_7", "vprair_tool_response_3", "vprair_tool_response_5", "vprair_tool_response_6", "vprai_tool_response_1", "vprai_tool_response_3", "vprai_tool_response_8", "vprai_tool_response_6", "vprai_tool_response_7", "vprai_tool_response_2", "vprai_tool_response_4", "vprai_tool_response_5")
...
fwrite(assess_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Assessment Collapse.csv")
```
...
Join
```{r}
book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Booking Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Court Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

# date_df <- fread("C:/Work/PretrialAssessmentPilot/County Join Dates/SANTA BARBARA Join Dates.csv") %>%
#   rename(file_date = filing_date) %>%
#   mutate_at(vars(contains("date")), as_date)
...
Collapse
```{r}
# Alameda Final Collapse
  ## Re-run Jail Collapse
...
join_collapse <- join_df2 %>%
  ...
  filter(pretrial_end_date >= book_date, book_type %in% c("On_View", "New_warrant", "Other_Warrant")) %>%
  group_by(book_num) %>%
  mutate(across(contains("flag"), max)) %>%oo
  ungroup() %>%
  distinct(book_num, .keep_all = T)

fwrite(join_collapse, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Santa Barbara Join Collapse.csv")
...
write.xlsx(factor_df, file = "C:/Work/PretrialAssessmentPilot/Code Books/Factor Code Book/Santa Barbara/P2 Submission-June 30 Factor Code Book.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto")
```
...
#Create an excel workbook with all data table. Make sure to include the cii table first. 
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "CIIs for DOJ/October 2020/Santa Barbara P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
...
o
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Santa Barbara Assessment Output 10-2020.csv")
...
#   unnest_auto(addressLine1) %>%
#   unnest_auto(city) %>%
#   unnest_auto(phonenumber) %>%
#   unnest_auto(arrestdate) %>%
#   unnest_auto(officer) %>%
#   unnest_auto(arrestingagency) %>%
#   unnest_auto(arrestlocation) %>%
#   unnest_auto(phonenumber) %>%
#   unnest_auto(arrestdate) %>%
#   unnest_auto(officer) %>%
#   unnest_auto(arrestingagency) %>%
...
#   
# jail_df %>%
#   unnest_auto(c(jacket, bookingdate))
# 
# names(jail_df)
# 
# df <- sapply(1:length(jail_df), function(i){
#   jail_df %>%
#     unnest_auto(i)
# })
# 
# 
# for(i in 1:length(jail_df)){
#   df <- jail_df %>%
#     unnest_auto(i)
# }
# 
...
# jail_df <- lapply(doc_list, function(i){
#   xmlToDataFrame(xmlParse(i))
#   }) %>%
#   bind_rows()
...
# fwrite(jail_df, file = "C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tulare/Tulare Jail xml to csv.csv")
...
names(book_df) <- c("book_id", "book_date", "book_time", "book_num", "arrest_charge_level_original", "local_id", "last_name", "first_name", "middle_name", "dob", "sex_original", "ht", "wt", "hair", "eye", "race_original", "veteran", "cld_id", "fbi", "ssn", "cii", "employed", "address", "city", "phone_num", "arrest_date", "arrest_time", "officer_id", "arrest_agency", "arrest_location", "arrest_charge_original", "book_charge_level_original_drop", "division", "department", "court_date", "bail_type", "bail_amt", "release_date", "release_type_original") 

# Removing unnecessary variables
book_df <- book_df %>%
  #Time variables are empty
  select(book_id, book_num, local_id, cii, fbi, cld_id, zip_code = address, dob,
         matches("name|date|original|bail"), -book_charge_level_original_drop)
...
fwrite(book_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tulare Booking Collapse.csv")
```
...
court_df2 <- fread("Historical Data Submissions/Tulare/P2 Submission-June 30/PretrialStats_Jan 2020 to Jun 2020.csv", colClasses = "character")

court_df3 <- fread("Historical Data Submissions/Tulare/P2 Submission-June 30/Pretrial2019.csv", colClasses = "character")
...
fwrite(court_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tulare Court Collapse.csv")
```
...
# Assessments Data Import
  ## assess_df contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The questions are widened into columns.
assess_df <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tulare/P2 Submission-June 30/PreTrialProject_DataElementInventory 20200101-20200630 run 20200819 1506.xlsx", col_types = "text")
...
assess_response <-  read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tulare/P2 Submission-June 30/PreTrialProject_AssessmentPSA 20200819 1502.xlsx", col_types = "text")
...
fwrite(assess_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tulare Assessment Collapse.csv")
```
...
#Create an excel workbook with all data table. Make sure to include the cii table first. 
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "CIIs for DOJ/October 2020/Tulare P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
...
write.xlsx(list(factor_df, flag_df, date_df), file = "C:/Work/PretrialAssessmentPilot/Code Books/Assessment Code Book/Tulare Assessment Code Book 9-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Tulare Assessment Output 10-2020.csv")
...
# fwrite(jail_df, file = "C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/Tuolumne Jail xml to csv Period 2.csv")

#Jail Data Loaded from fixed XML bullshit

book_df <- fread("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/Tuolumne Jail xml to csv Period 2.csv")
...
fwrite(book_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Booking Collapse.csv")
```
  Jail Diagnostics
...
# Court Cases
court_case <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 1, col_types = "text")
...
# Court Filing Charges
  ## court_charge contains one row per court case and charge combination, as well as information on each of the charges. Initially, it also duplicated information when a charge had multiple disposition dates. The first date is taken.
court_charge <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 2, col_types = "text")
...
court_sent <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 3, col_types = "text") %>%
  select(-Violation)
...
# Court Hearings
  ## court_hearing contains one row per court_case_id and hearing event.
court_hearing <- lapply(4:8, function(i){
  read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = i, col_types = "text")
  }) %>%
  bind_rows()
...
court_person <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 10, col_types = "text")
...
court_cii <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 11, col_types = "text")
...
# Court FTAs
  ## court_fta contains one row per FTA event.
court_fta <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 13, col_types = "text")
...
# Court FTAs with Warrants
  ## court_fta_war contains one row per FTA event with a bench warrant issued.
court_fta_war <- read_excel("Historical Data Submissions/Tuolumne/P2 Submission-June 30/Tuolumne Pretrial Assessment Information_2015 to 2020.xls", sheet = 14, col_types = "text")
...
# fwrite(court_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Court Collapse.csv")
...
# Assessments Data Import
  ## assess_df contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The questions are widened into columns.
assess_df <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/P2 Submission-June 30/Pre-Trial Spreadsheet April-June 2020.xlsx", col_types = "text")


# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("name", "local_id", "cii", "fbi", "cdl_id", "dob", "sex_original", "race_original", "zip_code", "arrest_date_time", "book_num",  "court_case_id", "book_type","tool_name", "assessment_date", "local_risk_score", "risk_level_original", "release_recommendation_original", "release_authorization_original", "case_status", "OR_release_date", "release_conditions_original", "pretrial_violation_original", "pretrial_violation_date", "next_hearing_date", "court_date_reminder_original", "emp_status", "pretrial_termination_outcome_original", "pretrial_termination_reason_original", "pretrial_termination_date", "fta_count", "other_pretrial_service_original", "notes")

# Date Reformatting--See pg. xxx for more details.
  ## Violation dates are duplicated and inconsistent. Fix later. 
assess_df <- assess_df %>%
  mutate(assessment_date = gsub(" -.*|,.*", "", assessment_date),
         pretrial_termination_date = gsub(" -.*|,.*", "", pretrial_termination_date)) %>%
  mutate_at(vars(contains("date"), dob, -court_date_reminder_original, -arrest_date_time), 
            funs(if_else(grepl("/", .), mdy(.), convertToDate(.)))) %>%
  mutate(arrest_date_time = mdy_hm(arrest_date_time),
         arrest_date = as_date(arrest_date_time))

# Risk Score Standardization--Tool Response values are changed to their numeric point value.
assess_df <- assess_df %>%
  mutate(tool_name = "tuol_co_pretrial",
...
assess_df <- assess_df %>%
  mutate(risk_level = case_when(risk_level_original %in% c("High", "Mod-High") ~ "High",
         ...
         release_decision = case_when(
           ...
           # pretrial_release_type_original %in% c("No Case Filed, Released") ~ "Ineligible",
           T ~ "Other_Unknown"),
         pretrial_ineligible_reason = case_when(
           # pretrial_release_type_original %in% c("No Case Filed, Released") ~ "Case_Disposed",
           T ~ "Other_Unknown"),
         pretrial_termination_reason = case_when(
...
assess_df <- assess_df %>%
  mutate(assess_flag = if_else(is.na(assessment_date), 0, 1),
         ...
         # condition_drug_test_flag = if_else(grepl("SCRAM|TEST", toupper(release_conditions_original))|
                                                    # grepl("SCRAM|TEST", toupper(pretrial_release_type_original)), 1, 0),
         # condition_no_contact_flag = if_else(grepl("NO CONTACT|136", toupper(release_conditions_original)), 1, 0),
         court_date_reminder_flag = if_else(is.na(court_date_reminder_original) | court_date_reminder_original == "No number", 0, 1),
         # Transit service doesn't actually exist here.  
         # transit_service_flag = if_else(grepl("BUS", toupper(other_pretrial_service_original)), 1, 0),
         ## Are all violations violations that result in termation? 
         pretrial_violation_flag = if_else(!is.na(pretrial_violation_original), 1, 0))
...
assess_df2 <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tuolumne/P2 Submission-June 30/6.30.20 - 9.30.20 Probation Pretrial Spreadsheet.xlsx", col_types = "text")

names_df <- tibble(original_field = names(assess_df2)) 

# Variable name standardization--See pg. xxx for more details.
names(assess_df2) <- c("name", "local_id", "cii", "fbi", "cdl_id", "dob", "sex_original", "race_original", "zip_code", "arrest_date", "arrest_time", "book_num",  "court_case_id", "book_type", "assessment_date", "assessment_time", "psa_fta_risk_score", "psa_nca_risk_score", "psa_nvca_risk_score", "monitor_level_original", "eligible_flag", "release_recommendation_original", "release_decision_prearraignment", "release_decision_post_arraignment", "release_authorization_original", "case_status", "OR_release_date", "release_conditions_original", "pretrial_violation_original", "pretrial_violation_date", "next_hearing_date", "next_hearing_time",  "emp_status", "pretrial_termination_outcome_original", "pretrial_termination_reason_original", "pretrial_termination_date", "pretrial_termination_time", "fta_count", "other_pretrial_service_original", "notes")

# Date Reformatting--See pg. xxx for more details.
  ## Violation dates are duplicated and inconsistent. Fix later. 
assess_df2 <- assess_df2 %>%
  mutate_at(vars(contains("date"), dob), 
            funs(if_else(grepl("/", .), mdy(.), convertToDate(.)))) %>%
...
assess_df2 <- assess_df2 %>%
  mutate(assess_flag = if_else(is.na(assessment_date), 0, 1),
         ...
         # condition_drug_test_flag = if_else(grepl("SCRAM|TEST", toupper(release_conditions_original))|
                                                    # grepl("SCRAM|TEST", toupper(pretrial_release_type_original)), 1, 0),
         # condition_no_contact_flag = if_else(grepl("NO CONTACT|136", toupper(release_conditions_original)), 1, 0),
         court_date_reminder_flag = if_else(monitor_level_original %in% 1:4, 0, 1),
         # Transit service doesn't actually exist here.  
         # transit_service_flag = if_else(grepl("BUS", toupper(other_pretrial_service_original)), 1, 0),
         ## Are all violations violations that result in termation? 
...
Assessment Collapse
```{r}
#Removing 2 duplicates on Assess Df
assess_df2 <- assess_df2 %>%
  distinct(local_id, assessment_date, court_case_id)

assess_df <- bind_rows(assess_df, assess_df2)

#Diagnostics
# names(assess_df2)[!names(assess_df2) %in% names(assess_df)]

# fwrite(assess_df, "C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Assessment Collapse.csv")
```
...
Join
  Jail Court Join
```{r}
book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Booking Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Court Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Tuolumne Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

date_df <- fread("C:/Work/PretrialAssessmentPilot/County Join Dates/TUOLUMNE Join Dates.csv", colClasses = "character") %>%
  # rename(file_date = filing_date) %>%
  mutate_at(vars(contains("date")), as_date)
...
court_df %>%
  # filter(court_docket_id != "DA NOT FILING", 
  #        court_docket_id != "") %>%
  semi_join(book_df, by = c("court_case_id" = "court_docket_id"))
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Tuolumne Assessment Output 10-2020.csv")
...
write.xlsx(list(factor_df, flag_df, date_df), file = "C:/Work/PretrialAssessmentPilot/Code Books/Assessment Code Book/Tuolumne Assessment Code Book 10-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))


# shell.exec("C:/Work/PretrialAssessmentPilot/Code Books/Assessment Code Book/Tuolumne Assessment Code Book 10-20.xlsx")
## Notes for Call:
  # Why use EM enhancements?
  # Are the conditions always consistent with the matrix?
  # Are the probation recommendations overridden?
...
#Create an excel workbook with all data table. Make sure to include the cii table first. 
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "CIIs for DOJ/October 2020/Tuolumne P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
...
book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Ventura Booking Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Ventura Court Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Ventura Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)
...
book_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Modoc Booking Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

court_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Modoc Court Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

assess_df <- fread("C:/Work/PretrialAssessmentPilot/Agency Collapse CSV Files/Modoc Assessment Collapse.csv") %>%
  mutate_at(vars(contains("date"), dob), as_date)

date_df <- fread("C:/Work/PretrialAssessmentPilot/County Join Dates/MODOC Join Dates.csv", colClasses = "character") %>%
  # rename(file_date = filing_date) %>%
  mutate_at(vars(contains("date")), as_date)
