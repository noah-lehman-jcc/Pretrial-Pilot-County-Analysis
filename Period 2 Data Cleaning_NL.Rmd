...
book_charge <- book_charge %>%
  ...
  mutate(arrest_charge_code = if_else(!arrest_charge_code_original %in% c("PC", "VC", "HS", "BP", "WI", "US"),
                                      "Other", arrest_charge_code_original),
         arrest_charge_level = if_else(!arrest_charge_level_original %in% c("F", "I", "M"), "Other",
                                       arrest_charge_level_original),
...
files <- list.files("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Alameda/P2 Submission-June 30/Resubmission 10-13-2020", full.names = T)

files <- files[grepl("xls", files)]

files <- files[c(1, 5:16, 2:4, 17:18)]



# files <- files[c(1, 10:17, 2, 4:9)]

df_list <- lapply(files, read_excel, col_types = "text")
...
assess_df <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/7_Phase-I-Assessment-and-Order.xls", col_types = "text")
...
book_df %>%
  filter(as_date(release_date) >= as_date("2020-01-01"))

summary(as_date(book_df$release_date))
...
court_df <- fread("Historical Data Submissions/Los Angeles/P2 Submission-June 30/ISAB_CourtCaseExtract_20201021.csv",
                  sep = "|")[,1:30]
...
court_fta <- fread("Historical Data Submissions/Los Angeles/P2 Submission-June 30/ISAB_BenchWarrantExtract_20201021.csv",
                  sep = "|")
...
assess_df <- fread("Historical Data Submissions/Los Angeles/P2 Submission-June 30/ISAB_PsaHistoricalExtract_20201021.csv",
                  sep = "|")

assess_df %>%
  count(psaReleaseDecision)
```
...
assess_response <- assess_response %>%
  mutate(tool_name = "psa",
         ...
         psa_tool_response_8 = if_else(psa_tool_response_1 == "Yes", 1, 0),
         psa_tool_response_9 = if_else(psa_tool_response_1 == "Yes", 2, 0))
...
```{r}
# Period 3 Assessment Data Import
...
Jail
```{r}
read.pattern("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/YubaQ419Jail.txt", header = F, pattern = " {2,}")

readr::read_fwf("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/YubaQ419Jail.txt", fwf_empty("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/YubaQ419Jail.txt"))
...
prob_df <- read_excel("", skip = 1) 
  # separate_rows(AssessmentAnswer, sep = ",")
...
# book_df <- fread("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Kings/P2 Submission-June 30/Pretrial Pilot Program Qtr 3-4 2020.csv", header = F)
...
assess_table <- assess_df %>%
  ...
  select(county, risk_level, release_recommendation, release_decision, assessment_date, risk_score = vpraio_risk_score, tool_name)

 

# Save file 

#fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Kings Assessment Output 08-2020.csv")
