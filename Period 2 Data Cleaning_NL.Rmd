...
#Renamind assess_status assess_df for now. 
...
book_df <- book_df %>%
  ...
  mutate(arrest_charge = if_else(arrest_charge_original %in% c("CC", "CCP", "CO", "CP", "CR", "OC", "PR"), "PC", arrest_charge_code_original),
         arrest_charge_code = if_else(!arrest_charge_code_original %in% c("PC", "VC", "HS", "BP", "WI", "US"), "Other", arrest_charge_code_original),
...
# Assessments Data Import
  ## assess_df contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The questions are widened into columns.
assess_df <- read_excel("C:/Work/PretrialAssessmentPilot/Historical Data Submissions/Tulare/P2 Submission-June 30/PreTrialProject_DataElementInventory 20200101-20200630 run 20200806 0840 (2).xlsx")
...
# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("tool_name", "assessment_date_time", "zip_code", "release_recommendation_original", "release_authorization", "pretrial_release_type_original", "release_date", "release_conditions_original", "pretrial_violation_original", "pretrial_violation_date", "court_date_reminder_original", "pretrial_termination_outcome_original", "pretrial_termination_date", "cii", "fbi", "local_id", "cdl_id", "name", "dob", "sex_original", "race_original", "psa_fta_raw_risk_score", "psa_nca_raw_risk_score", "psa_nvca_raw_risk_score")

# Date Reformatting--See pg. xxx for more details.
assess_df <- assess_df %>%
  mutate_at(vars(contains("date"), dob, -court_date_reminder_original, -assessment_date_time), ymd) %>%
  mutate(assessment_date_time = ymd_hms(assessment_date_time),
         assessment_date = as_date(assessment_date_time))
