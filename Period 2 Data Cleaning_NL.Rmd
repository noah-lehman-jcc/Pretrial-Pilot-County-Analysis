...
assess_df <- assess_df %>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         pretrial_termination_reason = case_when(
           ...
           pretrial_termination_reason_original %in% c("Bond Posted", "Cite to Appear", "Diversion", "Filed as Misdemeanor", "Release Denied", "Release Own Recognizance") ~ "Not_Tracked",
           T ~ "Other_Unknown"),
         pretrial_termination_outcome = case_when(pretrial_termination_reason %in% c("Case_Disposed") ~ "Successful",
                                                  pretrial_termination_reason %in% c("Absond", "FTA", "New_Crime", "Violation") ~ "Unsuccessful",
                                                  pretrial_termination_reason %in% c("Not_Tracked") ~ "Not_Tracked",
         ...
         # pretrial_ineligible_reason = case_when(pretrial_release_type_original|release_recommendation_original), 
         # release_authorization = case_when(release_authorization_original),
         # pretrial_termination_outcome = case_when(pretrial_termination_outcome_original),
         # pretrial_termination_reason = case_when(pretrial_termination_outcome_original))
         #F ollow up with SB about appropriate ways to understand monitoring levels in SB.  
         # pts_monitor_level_recommended = case_when(release_recommendation_original))
        # What does filed as misdemeanor entail?
        # SORR supervision level? 
...
assess_table <- assess_df %>%
  ...
  summarise_at(vars(c(release_recommendation, release_decision, 
         psa_nca_risk_score, psa_nvca_risk_score, psa_fta_risk_score, tool_name)),
         funs(if_else(!all(is.na(.)), max(., na.rm = T), max(.)))) %>%
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Tulare Assessment Output 08-2020.csv")
