...
book_df <- book_df %>%
  ...
  select(-arrest_time, -book_time, -date_time) %>%
...
# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("pfn", "zip_code", "assessment_date", "vprair_risk_score", "release_score_recommendation_original", "release_recommendation_original", "release_decision_original", "pretrial_status_original", "pretrial_release_type_original", "vprair_tool_response_1", "vprair_tool_response_2", "vprair_tool_response_3", "vprair_tool_response_4", "vprair_tool_response_5", "vprair_tool_response_6", "vprair_tool_response_7", "vprair_tool_response_8", "risk_level_original")
...
assess_table <- assess_df %>%
  ...
  distinct(pfn, assessment_date, .keep_all = T) %>%
...
# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Alameda Assessment Output 08-2020.csv")
...
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, 
         psa_nca_risk_score, psa_nvca_risk_score, psa_fta_risk_score, tool_name)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Los Angeles Assessment Output 08-2020.csv")
...
assess_table <- assess_df %>%
  ...
  # Select relevants variables.
  select(county, risk_level, release_recommendation, release_decision, assessment_date, vprai_risk_score, vprair_risk_score, tool_name, assessment_date, dob, sex, race, termination_outcome, termination_reason, book_num)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Santa Barbara Assessment Output 08-2020.csv")
...
doc <- read_xml("Historical Data Submissions/Tulare/P2 Submission-June 30/bookings_2020_03_26_to_2020_09_15.xml")
...
jail_df <- xmlToDataFrame(xmlParse(doc))
...
summary(as_date(jail_df$bookingdate))
```
...
assess_table <- assess_df %>%
  ...
  select(county, risk_level, assessment_date, 
         local_risk_score, tool_name)

# Save file  
fwrite(assess_table, file = "C:/Work/PretrialAssessmentPilot/Assessment Output Tables/Tuolumne Assessment Output 08-2020.csv")
