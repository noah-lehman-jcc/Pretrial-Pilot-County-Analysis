...
book_df <- book_df %>%
  ...
  group_by(event_id, book_num, book_date, court_case_id) %>%
...
#### START HERE ####
assess_df <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30//11_05_2020/3_Assessment And Recommendation.csv", colClasses = "character")

court_plea <- fread("Historical Data Submissions/Alameda/P2 Submission-June 30/11_05_2020/8_Pleas.csv")
####

# Pretrial Assessments and Recommendations
  ## Everything is contained in assess_df except for recommended pretrial conditions, most serious charge, and demographic information. 
assess_rec <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/4_Assessment-and-Recommendation.xls", col_types = "text")

####

# Pretrial judicial orders Pre-araignment
  ## Everything is contained in assess_df except for Pretrial Conditions and order date. 
assess_decision <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/5_Judicial-Order-Prearraignment.xls", col_types = "text")


####

# Pretrial Releases 
  ## Everything is contained in assess_monitor except for release time. 
assess_monitor <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/8_Pretrial-Status.xls", col_types = "text")


####

# Pretrial Service Referals
  ## Not in assess_df
assess_service <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/9_Service-Referrals.xlsx", col_types = "text")

####

# Pretrial Condition Violations
  ## Not in assess_df
assess_violation <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/10_Pretrial-Violations.xls", col_types = "text")


####

# Pretrial Judicical Orders Pre-arraignment Duplicate (Identical to assess decision. Not used.) 
  ## For now this just has court_case_id. We are waiting to get further linking variables.
# assess_decision2 <- read_excel("Historical Data Submissions/Alameda/P2 Submission-June 30/6_Judicial-Order-Hearing.xls", col_types = "text")

# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("local_id", "zip_code", "assessment_date", "vprair_risk_score", "release_score_recommendation_original", "release_recommendation_original", "release_decision_original", "pretrial_status_original", "pretrial_release_type_original", "vprair_tool_response_1", "vprair_tool_response_2", "vprair_tool_response_3", "vprair_tool_response_4", "vprair_tool_response_5", "vprair_tool_response_6", "vprair_tool_response_7", "vprair_tool_response_8", "risk_level_original")
  ...
  mutate(tool_name = "vprair",
    ...
    #                       T ~ "Other_Unknown"),
     release_recommendation = case_when(
       release_recommendation_original %in% c("Release") ~ "Release_Unspecified",
       release_recommendation_original %in% c("Detain") ~ "Detain",
       T ~ "Other_Unknown"),
     release_decision = if_else(is.na(release_decision_original), pretrial_release_type_original, release_decision_original),
     release_decision = case_when(
       release_decision %in% c("Release (Own Recognizance)") ~ "OR",
       release_decision %in% c("Release (Supervision Level I)", "Release (Supervision Level II)",
                                             "Release (Supervision Level II)", "Release (Monitoring)", 
                                             "Supervised (Pending R&S)") ~ "Monitor",
       release_decision %in% c("Detain") ~ "Detain",
       release_decision %in% c("Charges Dismissed", "Bail", "Ineligible - Other", "Sentenced",  
                                             "Cite", "Ineligible - Probation/Parole", "Diversion Program",
                                             "Ineligible - Warrant") ~ "Ineligible",
       T ~ "Other_Unknown"),
    ...
    monitor_level = case_when(release_decision_original == "Release (Supervision Level I)" ~ "Low",
                              release_decision_original == "Release (Supervision Level II)" ~ "Medium",
                              release_decision_original == "Release (Supervision Level III)" ~ "High",
                              T ~ "Other_Unknown"),
...
names(book_df) <- c("book_id", "book_date", "book_time", "book_num", "charge_level", "local_id", "last_name", "first_name", "middle_name", "dob", "sex_original", "ht", "wt", "hair", "eye", "race_original", "veteran", "cld_id", "fbi", "ssn", "cii", "employed", "address", "city", "phone_num", "arrest_date", "arrest_time", "officer_id", "arrest_agency", "arrest_location", "charge", "charge_level", "division", "department", "court_date", "bail_type", "bail_amt", "release_date", "release_type")

#Start Here--Mising all jail standardization. Also need court collapse. 
book_df
```
...
court_df <- reduce(list(court_case, court_person, court_cii, court_dispo, court_fta_hearing, court_hearing, court_sentence), left_join, by = "court_case_id") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%
  mutate(final_disposition_date = if_else(disposition_date <= sentence_date|is.na(sentence_date), disposition_date, sentence_date),
         conviction_flag = if_else(!is.na(sentence_date), 1, 0))
