...
court_df %>%
  filter(court_case_id %in% c("936245", "936250", "936259", "936283")) %>%
  arrange(court_case_id)

court_df %>%
  filter(court_case_id %in% c("913508", "936666", "936804", "955948")) %>%
  arrange(court_case_id)
...
book_df %>%
  ...
  filter(n>1) %>%
arrange(book_num) %>%
  select(book_num, first_name, last_name, arrest_date, book_date, release_date, book_type_original, release_type_original, court_docket_id)
```
...
full_df %>%
  filter(county == "los_angeles") %>%
  filter(year(book_date) == 2020) %>%
  summarise(mina = min(assessment_date, na.rm = T))
  #group_by(month(book_date)) %>%
...
full_df %>%
  filter(county == "los_angeles") %>%
  select(court_case_id, court_docket_id, court_charge_original, file_date, pretrial_end_date, disposition_date)
```
...
full_df %>%
  ...
  filter(book_date > "2018-07-01", 
         book_date <= "2020-12-31") %>%
  mutate(pts_release_flag = if_else(new_arrest_flag == 1 & release_decision == "Monitor", 1, 0)) %>%
...
full_df %>%
  ...
  filter(book_date > "2018-07-01", 
         book_date <= "2020-12-31") %>%
  mutate(pts_release_flag = if_else(new_arrest_flag == 1 & release_decision == "Monitor", 1, 0)) %>%
  # filter(book_num %in% c("110217","110231","110341","110532","110886","110925","110927","110922","111094","111125","111386","111576","111950","112018","112074","112955","113148","113565","115004","129122","129165","134054","134695","134696","135485","135487","135583","135646","135647","135698","135833","135838","136488")) %>%
  filter(book_num %in% c("135583","135646","135647","135833","135838","136488")) %>%
  select(book_num, pts_release_flag, release_decision, assessment_date, book_date, release_date, release_type, release_type_original)


full_df %>%
  filter(county == "napa") %>%
  #filter(year(assessment_date) == 2019) %>%
  count(year(assessment_date), month(assessment_date), day(assessment_date))
```
...
full_df %>%
  ...
  filter(book_date > "2018-07-01",
         book_date <= "2020-12-31") %>%
  summarise(mina = min(assessment_date, na.rm = T), maxa = max(assessment_date, na.rm = T))
  filter(!is.na(assessment_date)) %>%
  pull(book_num)
```
...
full_df %>%
  ...
  select(book_num, court_case_id, book_date, file_date, pretrial_end_date)
  pull(book_num)
  count(year(book_date), month(book_date))
  summarise(mind = min(book_date, na.rm = T), maxd = max(book_date, na.rm = T))
...
```{r}
#with updated definition of pretrial release to include individuals who were released, have no pretrial end date but do have a court case
# new arrest flag in yuba is super messed up, do not use
full_df %>%
  filter(county == "yuba") %>%
  filter(book_date > "2018-07-01", 
         book_date <= "2020-12-31") %>%
  mutate(pretrial_complete_flag = if_else(!is.na(pretrial_end_date), 1, 0),
         pretrial_release_flag = if_else((!is.na(release_date) & is.na(pretrial_end_date) & !is.na(court_charge_original) | release_date < pretrial_end_date), 1, 0),
         pts_release_flag = if_else(release_decision == "Monitor", 1, 0),
         pre_arr_release = if_else(pretrial_release_flag == 1 & release_date < arraignment_date, 1, 0),
         no_court_case = if_else(is.na(court_charge_original), 1, 0),
         has_case = if_else(!is.na(court_charge_original), 1, 0),
         fta_flag = if_else(pretrial_release_flag ==1 & (fta_flag == 1|arrest_fta_flag == 1|court_fta_flag == 1), 1, 0),
         recid_arrested_flag = if_else(pretrial_release_flag ==1, recid_arrested_flag, 0),
         violation_flag = if_else(fta_flag == 1 | recid_arrested_flag == 1, 1, 0),
         no_violation_flag = if_else(pretrial_release_flag == 1 & violation_flag != 1, 1, 0),
         release_elig_flag = if_else((is.na(pretrial_end_date)  | !pretrial_end_date < book_date), 1, 0)) %>%
  summarize(bookings_count = n(),
            #fresh_arrest_count = sum(new_arrest_flag, na.rm = T),
            release_eligible_count = sum(release_elig_flag, na.rm = T),
            pretrial_release_count = sum(pretrial_release_flag, na.rm = T),
...
court_df %>%
  count(is.na(arraignment_date))

load("G:/Crimjustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Yuba Join Collapse.RData")
...
```{r}
court_fta_war %>%
  summarise(earliest = min(fta_date, na.rm = T), latest = max(fta_date, na.rm = T))

load("G:/Crimjustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Yuba Join Collapse.RData")

yuba_collapse %>%
  count(arraignment_date < book_date)

yuba_collapse %>%
  summarise(early = min(book_date, na.rm = T), late = max(book_date, na.rm = T))
...
```{r}
table <- assess_df %>%
  select(ref_status_original, ref_status_reason_original, pretrial_termination_outcome, pretrial_termination_reason, release_recommendation, release_decision) %>%
  distinct()

write.csv(table, "probation_mapping_for_Bob.csv")
```
...
full_df %>%
  ...
  filter(book_date > "2018-07-01", 
         book_date <= "2020-12-31") %>%
  mutate(new_arrest_flag = if_else(book_type == "Bench_Warrant", 1, new_arrest_flag)) %>%  # modify to match snowflake's definition to check other numbers
  mutate(pretrial_complete_flag = if_else(new_arrest_flag == 1 & !is.na(pretrial_end_date), 1, 0),
         pretrial_release_flag = if_else(new_arrest_flag == 1 & (!is.na(release_date) & is.na(pretrial_end_date) & !is.na(court_charge_original) | release_date < pretrial_end_date), 1, 0),
         pts_release_flag = if_else(new_arrest_flag == 1 & release_decision == "Monitor", 1, 0),
         pre_arr_release = if_else(new_arrest_flag == 1 & pretrial_release_flag == 1 & release_date < arraignment_date, 1, 0),
...
full_df %>%
  ...
  filter(book_date > "2018-07-01", 
         book_date <= "2020-12-31") %>%
  count(new_arrest_flag, book_type, book_type_original)
...
```{r}
assess_df %>%
    filter(assessment_date > "2018-07-01", 
         assessment_date <= "2020-12-31") %>%
  distinct(assessment_id, book_num)
