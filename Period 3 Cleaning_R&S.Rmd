...
#reading in new P3 data (this was sent 4 6 2021 becasue the last file was missing so many CIIs)


book_df <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/P3 Submission-Jan 15/2021-04-06 Submission.xlsx",
...
book_df <- book_df %>%
  mutate(sex = case_when(sex_original %in% c("M") ~ "Male",
         ...
         release_type = case_when(release_type_original %in% c("CITE") ~ "Cite_Release",
                                  ...
                                  release_type_original %in% c("BAIL BOND", "CASH BAIL", "ZERO DOLLAR BAIL") ~ "Bail_Bond",
...
#code clip to look for zero bail people
# book_df %>%
#  filter(release_type=="Bail_Bond", year(release_date)==2020, month(release_date) %in% 1:12) %>%
#   group_by(release_type_original) %>%
#  count(year(release_date), month(release_date))
# 
# book_df %>%
...
names(assess_df_score) <- c("id", "pretrial_id",  "name","tool_name",
                            ...
                            "middle_name","suffix","oras_tool_response_1", "oras_tool_response_2",
...
assess_df_score <- assess_df_score %>%
  mutate(tool_name = "oras",
         oras_tool_response_1 = if_else(oras_tool_response_1 == "32 or older", 0, 1),
...
Nevada Assessment JoinPost-collapse join of Assessment and joined Court/Jail
```{r}
#Join Assessment data to court/jail joined data match rate = 73.9
# 

book_df_fnlndob <- book_df %>%
...
nrow(assess_df %>%
  ...
  filter(assessment_date <= book_date + 4) %>%
  distinct(pretrial_id, .keep_all = T) )/nrow(assess_df %>%  filter(assess_flag == 1) %>% distinct(pretrial_id, .keep_all = T))
...
# we found some people in the Sierra court data in both the Nevada jail data and the Nevada assessment data.
#seems like the best thing to do is to add sierra court to Nevada court and ten do the match--need to make court_case_ids match between sierra and Nevada
...
# look for share cases in book cases that match a court case (.1%)
nrow(book_df %>%
    semi_join(court_df, by = c("first_name", "last_name", "dob"), na_matches = "never"))/nrow(book_df) 

# Look share cases in court_df that match book_df (17.8)
nrow(court_df %>%
       mutate(across(c(first_name, last_name), toupper)) %>%
    semi_join(assess_df, by = c("first_name", "last_name", "dob"), na_matches = "never"))/nrow(court_df) 
...
book_df <- book_df %>%
  mutate(arrest_charge_code = gsub(".* ", "", arrest_charge_original),
         ...
         arrest_charge = gsub(">.*| .*|MISD|FEL|-FEL|INF" , "", arrest_charge_original),
          attempt_flag = if_else(grepl("664", arrest_charge_original), 1, 0),
...
book_df<- book_df %>%
  rename_at(vars(contains("flag")), funs(paste0("arrest_", .))) %>%
    mutate(arrest_misd_flag=if_else(arrest_charge_level=="M", 1, 0),
           arrest_felony_flag =if_else(arrest_charge_level=="F", 1, 0),
           attempt_flag=if_else(arrest_charge=="664", 1, arrest_attempt_flag),
           # not seeing any qualifier charges here  arrest_qualifier_flag=if_else()
...
book_df %>%
 filter(release_type_original=="COVID - PTA", year(release_date)==2020, month(release_date) %in% 1:12) %>%
  group_by(release_type_original) %>%
 count(year(release_date), month(release_date))


book_df %>%
  filter(release_type=="Bail_Bond") %>%
  mutate(zero_bail = if_else(bail_amt=="0", 1, 0)) %>%
  count(charge_level, zero_bail)
