...
book_df <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/P3 Submission-Jan 15/Pretrial Data Through December 2020.xlsx") %>%
  ...
  #c("text", rep("guess", 10), "date", rep("guess", 8)), na = "NULL")
  #col_types = "text"  Noah prefers that everything be read in as text, but doing this makes the dates come back as NA
...
fwrite(book_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Nevada Booking Collapse.csv")
...
court_df <- lapply(c("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/PTRiskAss-Case+PtyData.csv",
                     ...
                     "G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/P2 Submission-June 30/PTRiskAss-Case+PtyData.csv",
                     "G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Nevada/P2 Submission-June 30/PTRiskAss-Case+PtyData.csv"),
                   function(x) {
...
fwrite(court_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Nevada Court Collapse.csv")
...
# Readin updated data this data has 11 variables instead of 6, it looks like fields were added erroneously
court_df_s2 <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Sierra/P3 Submission-Jan 15/Pretrial_4_Demographics-07-01-2020thru12-31-2020.xls")
...
# read in updated data 7/1/2020-Dec 2020
court_warrant_s2 <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Sierra/P3 Submission-Jan 15/Pretrial_2_ScheduledEvents_Warrant-07-01-2020thru12-31-2020.xls")
...
# Name Standardization--See pg. xxx for more details. 
names(court_warrant_s) <- c("court_case_id", "case_name", "hearing_type", "hearing_result_date", "hearing_result_type", "fta_warrant_date", "fta_warrant_type")

#Make FTA flag
court_warrant_s <- court_warrant_s%>%  
  mutate(fta_flag = ifelse(hearing_result_type == "Fail to Appear: Bench Warrant Issued", 1, 0))

####

#Import Events Table, this is the first time they have submitted it should have data from June 2020 to Dec 2020
 court_hearings_s2<- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Sierra/P3 Submission-Jan 15/Pretrial_5_ScheduledEvents07-01-2020thru12-31-2020.xls") 
 
# Variable name standardization--See pg. xxx for more details.
names(court_hearings_s2) <-
  c( "court_case_id", "case_name", "hearing_type_original", "hearing_result_date",  "hearing_result_original")

# Date Reformatting--See pg. xxx for more details.
court_hearings_s2 <- court_hearings_s2 %>%
...
court_hearings_s2 <- court_hearings_s2  %>%
  mutate(
    hearing_type = case_when(
      hearing_type_original %in% c("Arraignment: Complaint","Arraignment: Complaint-Amended", 
                                   "Arraignment: Information",
                                   "Arraignment: Warrant", "Further Arraignment: Complaint") ~ "Arraignment",
      hearing_type_original %in% c("Sentencing Hearing") ~ "Sentencing",  
      T ~ "Other_Unknown"),
     # Other_Unknown hearing types include (Change of Plea, Change of Plea or Setting, Hearing: Other, Modification Hearing, Motion Hearing, Pay or Appear, Pre-Trial Conference, Pre Prelim, Prelim Setting, Preliminary Examination, Proof or Appear, Review Hearing, Warrant Surrender	)
    
# test to see if this yield same as warrants data    
    court_fta_flag = if_else(grepl("FTA|FAILURE TO APPEAR", hearing_result), 1, 0))
...
#Import Court Charges and Dispositions
 ...
 ## Disposition Date range 10/23/2007 to 7/21/2020
...
court_dispos_s2 <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Sierra/P3 Submission-Jan 15/Pretrial_3_Charges-07-01-2020thru12-31-2020.xls")

# Name Standardization--See pg. xxx for more details. 
names(court_dispos_s) <- c("court_case_id", "case_name", "file_date", "charge_number", "court_charge_level_original", "charge_original", "disposition_date", "disposition_type_original", "disposition_manner", "sentence_date", "sentence_type_original", "sentence_amount", "sentence_length", "sentence_length_unit")
...
court_dispos_s <- court_dispos_s %>%
  ...
  mutate(court_charge_code_original = str_extract(charge_original, "^[A-Z]*"),
         charge_original = gsub("^[A-Z]*", "", charge_original),
         court_charge = gsub("\\s.*", "", charge_original),
         charge_description = str_extract(charge_original, "\\s.*")) %>%
...
court_dispos_s <- left_join(court_dispos_s, charge_hier, by = "join_var") %>%
  mutate(felony_flag = if_else(court_charge_level == "F", 1, 0),
         misd_flag = if_else(court_charge_level == "M", 1, 0)) %>%
  mutate_at(vars(contains("flag")), funs(if_else(is.na(.), 0, as.numeric(.)))) %>%
  mutate_at(vars(contains("flag")), funs("conviction" = if_else(. == 1 & disposition_type %in% c("Guilty", "Nolo_Contendere"), 1, 0))) %>%
...
court_fta_war <- court_warrant_s %>%
  arrange(hearing_result_date) %>%
  group_by(court_case_id) %>%
  mutate(fta_flag_count = sum(fta_flag))%>%
  distinct(court_case_id, .keep_all = T) %>%  
 select(court_case_id,  fta_warrant_date, hearing_result_date, fta_flag, fta_flag_count)


#This code makes arraignment date
# We do not yet have a hearing table, we need to have Sierra make it based on what Tulane did
# court_arraign <- court_hearing %>%
#   filter(hearing_type == "Arraignment") %>%
#   rename(arraignment_date = hearing_date) %>%
#   arrange(arraignment_date) %>%
...
#this part does something not sure what
 ## leaving out mutate f final dispo date because we dont have sentence date in this dataset
 ## court_df  already came to us collapsed at level of case
court_df <- reduce(list(court_df_s, court_dispo_s, court_fta_war ), left_join, by = "court_case_id") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%
  #mutate(final_disposition_date = if_else(disposition_date <= sentence_date|is.na(sentence_date), disposition_date, sentence_date),
  mutate(conviction_flag = if_else(disposition_type %in% c("Guilty","Nolo_Contendere"), 1, 0))

fwrite(court_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Sierra Court Collapse.csv")
```
...
names_df1 <- tibble(original_field = names(assess_df_case))
...
names_df1$field <- names(assess_df_case)
...
names_df2 <- tibble(original_field = names(assess_df_demo))
...
names_df2$field <- names(assess_df_demo)
...
names_df3 <- tibble(original_field = names(assess_df_score))
# Standardize Names
...
names_df3$field <- names(assess_df_score)
...
names_df4 <- tibble(original_field = names(assess_df_terms))

names(assess_df_terms)<- c("pretrial_id", "pretrial_id1", "pretrial_id2", "arresting_agency", "name",
                           "release_conditions_original")

 
names_df4$field <- names(assess_df_terms)
...
names_df5 <- tibble(original_field = names(assess_df_viol))

names(assess_df_viol)<- c("pretrial_id", "pretrial_id1", "pretrial_id2", "arresting_agency", "name",
...
names_df5$field <-  names(assess_df_viol)
...
assess_df_score <- assess_df_score %>%
  ...
  distinct(pretrial_id2, assessment_date, .keep_all = TRUE)
  
assess_df_case <- assess_df_case%>%
  group_by(pretrial_id2) %>%
  mutate_at(vars(contains("flag")), funs(if_else(is.na(.), 0, .))) %>%
  mutate_at(vars(contains("flag")), max) %>%
  distinct(pretrial_id2, .keep_all = TRUE)
...
assess_df_demo <- assess_df_demo%>%
  distinct(pretrial_id2 , .keep_all = TRUE) %>%
  mutate (pretrial_id2 = as.numeric(pretrial_id2))
...
assess_df <- assess_df_score %>%
  left_join(assess_df_demo, by = "pretrial_id2") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%

  left_join(assess_df_case, by = "pretrial_id2") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%

  left_join(assess_df_terms, by = "pretrial_id2") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .)))  
...
fwrite(assess_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Nevada Assessment Collapse.csv")
...
Assessment Tables for Noah 9-2020
```{r}
##Assessment Table 
 ...
 assess_table <- assess_df %>%
  ...
  # Create county name.
  mutate(county = "Nevada", risk_score = oras_risk_score) %>%
  ...
  select(county, risk_level, release_recommendation, release_decision, assessment_date, risk_score, tool_name,
         assessment_date, dob, sex, race, pretrial_termination_outcome, pretrial_termination_reason)


# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Nevada Assessment Output 09-2020.csv")
```

Assessment Tables for Noah 10-2020
```{r}
assess_table <- assess_df %>%
  # Collapse to each assessment.
  ...
  mutate(county = "Nevada", risk_score = oras_risk_score) %>%
  # Select relevants variables.
  select(county, release_recommendation, release_decision, assessment_date, risk_score, tool_name, assessment_date, dob, sex, race, risk_level,  pretrial_termination_reason, monitor_level)
...
# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Nevada Assessment Output 10-2020v2.csv")


```

Assessment Codebook (10/2020) 
```{r}

names_df <- bind_rows(names_df1, names_df2, names_df3, names_df4, names_df5)
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      field == "release_conditions_original" ~ "condition_no_contact_flag, condition_drug_test_flag",
      ##field == "release_authorization_original" ~ "release_authorization",
      field == "pretrial_termination_outcome_original" ~ "pretrial_termination_reason, pretrial_termination_outcome, pretrial_ineligible_reason", 
      field == "court_date_reminder_original" ~ "court_date_reminder_flag",
      ##field =="other_pretrial_service_original" ~ "transit_service_flag",
      field =="oras_risk_score" ~ "risk_level",
      field =="oras_risk_score" ~ "monitor_level",
...
# these are the separated tables 

factor_df <- lapply(c("assess_df_case", "assess_df_demo", "assess_df_score", "assess_df_terms"), function(i){
  df <- get(i) %>%
...
#tried to figure this out and then gave up
# factor_df <- factor_df %>%
#   mutate(value = case_when(modified_field == "release_recommendation" & modified_value == "Ineligible" ~ "when release_recommendation is NULL
#                            and pretrial_termination_outcome_original is either Bond Posted, Charges Dismissed, or No Charges Filed we assign it
#                            ineligible",
#                            modified_field == "release_recommendation" & modified_value == "Other_Unknown" ~ "when release_recommendation is NULL
#                            and pretrial_termination_outcome_original not Bond Posted, Charges Dismissed, or No Charges Filed we assign it
#                            Other_Unknown",
#                            modified_field == "release_decision" & modified_value == "Ineligible" ~ "when release_recommendation is NULL and
#                            pretrial_termination_outcome_original is either Bond Posted, Charges Dismissed, or No Charges Filed we assign it
#                            ineligible",
#                            modified_field == "release_decision" & modified_value == "Other_Unknown" ~ "when release_recommendation is NULL and
#                            pretrial_termination_outcome_original not Bond Posted, Charges Dismissed, or No Charges Filed we assign it
#                            Other_Unknown",
#                            modified_field == "pretrial_termination_reason" & modified_value == "Other_Unknown" ~ "under what circumstaces is
#                            release denied used?"))  
...
flag_df <- lapply(c("assess_df_case", "assess_df_terms"), function(i){
  df <- get(i)
...
flag_df <- flag_df %>%
  ...
  modified_field == "condition_drug_test_flag" & modified_value == "1" ~ "MATCHES (SCRAM|TEST|CONTROLLED SUBSTANCES|DRUG|ALCOHOL|CHEMICAL)",
  ...
  modified_field == "condition_no_contact_flag" & modified_value == "1" ~ "Matches (NO CONTACT|136|HARASS|CONTACT|STAY AWAY|STAY.*AWAY FROM|THREATEN|ANNOY)",
...
date_df <- lapply(ls()[!grepl("original|^df|hier|names|factor|date|flag|book|test", ls())], function(i){
  df <- get(i)
...
write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Nevada Assessment Code Book 10-20v2.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
```
...
# CIIs from court tables--when we included fbi excel would not make the file because the data had some wield character
...
#Create an excel workbook with all data table. Make sure to include the cii table first.
write.xlsx(list(doj_cii, doj_book, doj_assess, doj_court), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/October 2020/Nevada P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "assess", "court"))



###diagnostics looking for source of character that excel could not interpret
doj_court <- doj_court %>%
...
# Jail Bookings Data Import--- San Mateo, 2015-01-01 to 09-30-2020
...
#Keeping the commented out lines below only for original names, this is the old Booking Data but it has column headers. 
...
#San Mateo Jail Data Import
  ## New data was submitted on 10/6/2020 and has IDs used by probation, they added local_id and added booking number 
...
book_df <- read_csv("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/41-SO-01OF01-JAN2015-SEP2020.csv", col_names = F, col_types = cols( X1= "c", .default="?"))
...
fwrite(book_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/San Mateo Booking Collapse.csv")
```
...
court_df <- bind_rows(court_df1, court_df2, court_df3, court_df4) %>%
  distinct()

rm(court_df1, court_df2, court_df3, court_df4)

# Variable name standardization--See pg. xxx for more details. 
names(court_df) <- c("court_docket_id", "court_case_id_original", "file_date", "court_case_type", "court_party_id", 
                     "cii", "fbi","local_id", "cdl_id", "name", "dob", "sex_original", "race_original")
...
# Court Sentences and Court Filing Charges 
 ...
 ##all f the dates in the merged file need to be checked.
...
court_charge_1 <- fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/41-COURT-02OF03-OCT2019-DEC2019.csv", na.strings=c("NULL",""))

court_charge_2 <- fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/41-COURT-02OF03-JAN2020-JUN2020.csv", na.strings=c("NULL",""))

court_charge_3 <- fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/41-COURT-02OF03-JUL2020-SEP2020.csv", na.strings=c("NULL",""))

court_charge_3 <- court_charge_3 %>%
  mutate(BookingNumber = as.character(BookingNumber))
...
rm(court_charge_1, court_charge_2, court_charge_3)
...
court_hearing <- fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/41-COURT-03OF03-JAN2016-SEP2019.csv") %>%
     mutate_at(vars(contains("date"), DtFile), as_date)
   
   
 court_hearing_1 <-
   fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/41-COURT-03OF03-OCT2019-DEC2019.csv",  na.strings = c("NULL", "")) %>%
       mutate_at(vars(contains("date"), DtFile), mdy)
 
  court_hearing_2 <-
   fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/41-COURT-03OF03-JAN2020-JUN2020.csv",  na.strings = c("NULL","")) %>%
        mutate_at(vars(contains("date"), DtFile), mdy)
  
   court_hearing_3 <-
   fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/41-COURT-03OF03-JUL2020-SEP2020.csv",  na.strings = c("NULL","")) %>%
        mutate_at(vars(contains("date"), DtFile), mdy)
...
court_hearing <- bind_rows(court_hearing, court_hearing_1, court_hearing_2, court_hearing_3) %>%
  distinct()

rm(court_hearing_1, court_hearing_2, court_hearing_3)
...
fwrite(court_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/San Mateo Court Collapse.csv")
...
San Mateo Assessment Data Sent 10/14/2020 
```{r}
# San Mateo Assessment Data Import
  ## this is their new data that I read in on 10 15 2020
  ## it has data (1110 observations) it primarily has data from July-Sept 2020
...
assess_df_new <- read_csv("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/San Mateo/P2 Submission-June 30/Pretrial Pilot eProb PROD data_10142020.csv", skip = 3)

names_df <- tibble(original_field = names(assess_df_new))

# Variable name standardization--See pg. xxx for more details.
names(assess_df_new) <- c("local_id", "book_num", "arrest_date", "book_date_time", "last_name", "first_name", "middle_name", "dob", "sex_original", "cii", "fbi", "ipid", "cdl_id", "hispanic_original", "race_original", "zip_code", "assessment_date_time",  "tool_name",  "vprair_tool_response_1", "vprair_tool_response_2", "vprair_tool_response_3", "vprair_tool_response_4", "vprair_tool_response_5", "vprair_tool_response_6", "vprair_tool_response_7", "vprair_tool_response_8",   "vprair_risk_score", "pretrial_release_type_original",  "release_recommendation_original", "release_type_at_arraign", "condition_drug_test_original", "conditions_cam_monitoring_original", "condition_gps_original", "condition_home_detention_original", "condition_curfew_original", "condition_search_original", "condition_stay_away_original", "condition_other_original", "pretrial_signature_date_time", "num_fta" , "num_new_arrests", "office_ftas", "scram_vio", "gps_vio", "total_tc_violations", "num_court_reminders", "pretrial_termination_date",  "pretrial_termination_outcome_original")


assess_df_new %>%
  count(total_tc_violations)

names_df$field <- names(assess_df_new)

# Date Reformatting--See pg. xxx for more details.
assess_df_new <- assess_df_new %>%
  #mutate(assessment_date_time = parse_date_time(assessment_date_time, c('%m/%d/%Y %H:%M'))) %>%
  mutate_at (vars(assessment_date_time,pretrial_signature_date_time, book_date_time),  mdy_hm) %>% 
  mutate_at(vars(dob, arrest_date, pretrial_termination_date), mdy) %>%
  mutate (assessment_date = as_date(assessment_date_time),
          pretrial_signature_date = as_date(pretrial_signature_date_time),
          book_date = as_date(book_date_time))
...
fwrite(assess_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/San Mateo Assessment Collapse.csv")
...
San Mateo Assessment Data (Probation) 1/23/2020- 6/30/2020 Go live date was Jan 23, 2020
```{r}
# San Mateo Assessment Data Import
  ...
  ##assessment date range is 1/23/2020 to 6/30/2020 and have 413 observations.
  ## SM said local_id is the number their jail uses, but their original jail extract does not have it. On         
 ##10/6/2020 their jail submitted data with the local_id (still need to check for match).  Probation currently ##doesnt have court_case_id, booking_num, or arrest_date 
 ## Condition under basic=phone 1 time per week, regular MOR= come in 1 x per week, enhanced=come in 1 x per week ##and additional conditions
...
# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("local_id", "name", "dob", "sex_original", "race_original", "hispanic_original", "tool_name", "assessment_date_time", "zip_code", "vprair_tool_response_1", "vprair_tool_response_2", "vprair_tool_response_3", "vprair_tool_response_4", "vprair_tool_response_5", "vprair_tool_response_6", "vprair_tool_response_7", "vprair_tool_response_8",   "vprair_risk_score",  "release_recommendation_original", "release_authorization_original", "pretrial_release_type_original", "release_date", "release_conditions_original", "pretrial_violation_type_original", "pretrial_violation_date",  "court_date_reminder_original", "other_pretrial_service_original", "pretrial_termination_outcome_original", "pretrial_termination_date")
...
##release_type_original has "N/A" (26 rows none with a release date) and "pending" (14 rows two with a release date)
## pretrial violation says nothing about abscond
##there is no recommended level of supervision 
##there is a supervision release type--Basic=phone 1 time/wk Regualar=inperson 1 time/wk, Enhanced=come in 1 time wk and additional conditions 6/7/2020 notes
...
assess_df <- assess_df %>%
  mutate(sex = case_when(sex_original %in% c("Male") ~ "Male",
         ...
         release_decision = case_when(pretrial_release_type_original %in% c("OR", "Promise to Appear") ~ "OR",
                                      pretrial_release_type_original %in% c("MOR Basic", "MOR BASIC", "Mor Enhanced", 
                                                                   "MOR Enhanced", "MOR Regular") ~ "Monitor",
                                      pretrial_release_type_original %in% c("Denied") ~ "Detain",
                                      pretrial_release_type_original %in% c("Bailed Out", "DA declined to file", 
          ...
          risk_level = case_when(vprair_risk_score %in% c("0", "1", "2", "3", "4") ~ "Low",
                                  ...
                                  vprair_risk_score %in% c( "9", "10" , "11", 
                                                           "12", "13", "14") ~ "High"),
          pretrial_release_type = case_when(pretrial_release_type_original %in% c("OR", "Promise to Appear") ~ "OR",
                                      pretrial_release_type_original %in% c("MOR Basic", "MOR BASIC", "Mor Enhanced", 
                                                                   "MOR Enhanced", "MOR Regular") ~ "Monitor",
                                      pretrial_release_type_original %in% c("Denied") ~ "Detain",
                                      pretrial_release_type_original %in% c("Bailed Out") ~ "Bail_Bond", 
                                      pretrial_release_type_original %in% c("DA declined to file", 
                                                                   "Case dismissed", "N/A", "Pending") ~ "Other_Unknown" ),
         pretrial_ineligible_reason = case_when(pretrial_release_type_original %in% c("Bailed Out") ~ "Bail_Bond",
                                                pretrial_release_type_original %in% c("DA declined to file", 
         ...
         pretrial_termination_reason = case_when(pretrial_violation_type_original %in% c("FTA") ~
                                                    "FTA",
                                                  pretrial_violation_type_original %in% c("New charges") ~
                                                   "New_Crime",
                                                 pretrial_violation_type_original  %in% c("New law violation, alcohol use") ~
                                                   "Violation",
                                                  pretrial_termination_outcome_original %in%
                                                    c("Unsuccessful - Case Dismissed", 
...
# Flag Standardization---See pg. xxx for more details.
  ## Need to find out what NA means in the context of recommended for release--looks like someone bails out or A declines to file before recommendation can be made
  ## drug, alcohol flags are from a notes field and are not consistent. 
  ## already asked SM what is difference between OR and Promise to Appear, they are the same
  ## clarify that denied means recommend for detention?
  ## why so many people recommended for MOR enhanced
  ## under release type what does denied mean? does it man bail set? 
  ## under release type what does N/A mean? does it mean bail set? 
  ## court_date_reminder_original has no values filled in may need to add when new data comes in
  ## all pretrial termination dates are missing
 ...
 assess_df <- assess_df %>%
    mutate(assess_flag = if_else(is.na(assessment_date), 0, 1),
         ...
         # Discuss release_conditions_original to see if they have GPS. 
         condition_phone_flag = if_else(grepl("ENHANCED|REGULAR|BASIC", toupper(pretrial_release_type_original)), 1, 0),
         # enhanced might not belong here becasue they have in-person ceck ins not phone check ins
         condition_inperson_flag = if_else(grepl("ENHANCED|REGULAR", toupper(pretrial_release_type_original)), 1, 0),
         condition_drug_test_flag = if_else(grepl("TEST|TST", toupper(release_conditions_original)), 1, 0),
         #condition_no_contact_flag = if_else(grepl("NO CONTACT|136", toupper(release_conditions_original)), 1, 0),
         # court_date_reminder_flag = if_else(court_date_reminder_original == "CourtAppearance", 1, 0),  ask San Mateo   what is diff b/w "NA" and "N/A"
         # Transit service doesn't actually exist here.  
         # transit_service_flag = if_else(court_date_reminder_original == "TRANSIT", 1, 0),
...
#   
# assess_df %>%
#   count(release_recommendation_original, pretrial_release_type_original) %>%
#   arrange(desc(n)) 
# assess_df %>%
#   count(pretrial_termination_outcome_original, pretrial_violation_type_original) %>%
#   arrange(desc(n))
# assess_df %>%
#   count( pretrial_release_type_original) %>%
#   arrange(desc(n))
# assess_df %>%
#   count(court_date_reminder_original) %>%
#   arrange(desc(n))

```

Assessment Table 9-2020 
```{r}
##Assessment Table 
 ...
 assess_table <- assess_df %>%
  ...
  # Select relevants variables.
  select(county, risk_level, release_recommendation, release_decision, assessment_date, vprair_risk_score, tool_name,
         assessment_date, dob, sex, race, pretrial_termination_outcome, pretrial_termination_reason)


# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/San Mateo Assessment Output 09-2020.csv")
```
Assessment Tables for Noah 10-2020
```{r}
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, risk_score, tool_name, assessment_date, dob, sex, race, risk_level,  pretrial_termination_reason, charge_level, monitor_level)

# no Monitoring Level still need to ask county, so removing monitor_level
 
# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/San Mateo Assessment Output 10-2020v2.csv")
...
#Assessment collapsed. 
  ## Book nums have a suffix of .1, .2 etc.  Sometime they had different booking dates with the same assessment date  
  ## this code needs to be corrected to make sure collapse is correct
assess_df <- assess_df %>%
  group_by(local_id, assessment_date) %>%
  mutate_at(vars(contains("flag")), funs(max(., na.rm = T))) %>%
  ungroup() %>%
  distinct(local_id, assessment_date,  .keep_all = TRUE) 

assess_df_new %>%
  select (-book_num, -book_date_time) %>%
  distinct(.keep_all = T) %>%
  add_count (local_id, assessment_date) %>%
  filter(n>1)

```
Assessment Codebook (9/2020 Data from January - June 2020) 
```{r}
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      field == "race_original" ~ "race",
      field == "release_recommendation_original" ~ "release_recommendation",
      field == "pretrial_release_type_original" ~ "release_decision, pretrial_release_type, pretrial_ineligible_reason,  condition_inperson_flag,
      pretrial_termination_reason", 
      field == "conditions_cam_monitoring" ~ "condition_em_flag",
      field == "condition_gps_monitoring" ~ "condition_em_flag",
      field == "condition_drug_test_original"  ~ "condition_drug_test_flag", 
      ##field == "release_authorization_original" ~ "release_authorization",
      field == "pretrial_termination_outcome_original" ~ "pretrial_violation_flag, pretrial_termination_outcome",
      ##field == "court_date_reminder_original" ~ "court_date_reminder_flag",
      ##field =="other_pretrial_service_original" ~ "transit_service_flag",
      field =="vprair_risk_score" ~ "risk_level")) %>%
  filter(!is.na(modified_field)) %>%
  separate_rows(modified_field, sep = ",") %>%
  mutate_all(trimws)
...
factor_df <- lapply(ls()[!grepl("original|^df|hier|names|factor|date|flag", ls())], function(i){
  ...
  left_join(names_df) %>%
  select(table_name, original_field, field, modified_field, value, modified_value, total, percent) %>%
  filter(modified_field != 'pretrial_termination_reason')
...
# factor_df <- factor_df %>%
#   mutate(value = case_when(
#   modified_field == "pretrial_ineligible_reason" & modified_value == "Bail_Bond" ~ "pretrial_release_type_original = 'Bailed Out'",
...
flag_df <- flag_df %>%
  filter(modified_value == 1) %>%
  mutate(value = case_when(
  modified_field == "condition_drug_test_flag" & modified_value == "1" ~ "if conditions drug test original = yes",
  modified_field == "condition_em_flag" & modified_value == "1" ~ "if conditions_cam_original and conditions_em_original=yes",
  # modified_field == "court_date_reminder_flag" & modified_value == "1" ~ "CourtAppearance / (PTS with court reminders, OR-with court reminders)",
...
termination_df <- assess_df_new %>%
  count(pretrial_termination_outcome_original, pretrial_termination_reason) %>%
  arrange(desc(n))  
...
##saved this as version 2 of San Mateo to fix the Other_Unknowns
write.xlsx(list(factor_df, flag_df, termination_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/San Mateo Assessment Code Book v2 10-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Terminations", "Dates"))
```

Assessment Codebook (10/2020 Data from July - September) 
```{r}
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      field == "race_original" ~ "race",
      field == "release_recommendation_original" ~ "release_recommendation",
      field == "pretrial_release_type_original" ~ "release_decision, pretrial_release_type, pretrial_ineligible_reason,  condition_inperson_flag, pretrial_termination_reason", 
      field == "conditions_cam_monitoring" ~ "condition_em_flag",
      field == "condition_gps_monitoring" ~ "condition_em_flag",
      field == "condition_drug_test_original"  ~ "condition_drug_test_flag", 
      ##field == "release_authorization_original" ~ "release_authorization",
...
##saved this as version 2 of San Mateo to fix the Other_Unknowns
write.xlsx(list(factor_df, flag_df, termination_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/San Mateo Assessment Code Book July to Sept 10-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Terminations", "Dates"))
```
...
#CIIs for DOJ
doj_cii <- bind_rows(doj_book %>% select(cii), doj_court %>% select(cii))  %>% 
  distinct() %>%
  filter(nchar(cii) == 10)
...
write.xlsx(list(doj_cii, doj_book, doj_court), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/October 2020/San Mateo P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court"))
