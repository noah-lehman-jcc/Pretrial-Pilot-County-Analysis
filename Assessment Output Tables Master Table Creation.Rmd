...
Import Data
```{r}
files <- list.files("G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables", pattern = "RData", full.names = T)

list <- lapply(files, function(x) {
  load(file = x)
  get(ls()[ls()!= "files"]) %>%
    mutate(across(contains("risk_level"), as.character)) %>%
    ungroup() %>%
     select(county, assessment_date, 
           matches("risk|score|monitor|release|pretrial|charge_level"), tool_name, assessment_date, dob, sex, race, -contains("original"), -contains("pretrial_id")) %>%
...
df <- bind_rows(list) %>%
    select(county, monitor_level, release_recommendation, release_decision, assessment_date, 
           matches("risk|score"), tool_name, assessment_date, dob, sex, race, pretrial_termination_reason, pretrial_ineligible_reason, charge_level)
...
save(df, file = "G:/CrimJustice/PretrialAssessmentPilot/Pretrial-Pilot-County-Analysis/Assessment Output Data Frame 04-2021.RData")
