...
monitor_df <- df %>%
  ...
  select("County" = county, Total, Low, Medium, High, Other_Unknown, "Tool Name" = tool_name) 
```

AB 74 (D): Tables
```{r}
outcome_df <- df %>%
  count(county, pretrial_termination_reason, tool_name) %>%
  group_by(county) %>%
  mutate(Total = sum(n), n = paste0(round(n = n/Total*100), "%")) %>%
  pivot_wider(names_from =  pretrial_termination_reason, values_from =  n) %>%
  select(county, FTA, New_Crime, Revoked, Total, tool_name) %>%
  bind_rows(df %>%
