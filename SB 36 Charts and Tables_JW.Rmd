...
```{r}
`Release_Decision_Pre-Arraignment_2E` %>%
  filter(County != "Los Angeles") %>%
  mutate(release_decision = case_when(
    release_decision %in% c("Monitor", "Own Recognizance") ~ "Granted Program Release",
    release_decision %in% c("Denied Program Release", "Ineligible") ~ "Denied Program Release",
    T ~ "Other/Unknown"
  )) %>%
  filter(release_decision != "Other/Unknown") %>%
  group_by(County, release_decision) %>%
...
Release_Decision_LA_only_2E %>%
  mutate(release_decision_prearraignment = if_else(release_decision_prearraignment == "Ineligible", "Denied Program Release", release_decision_prearraignment)) %>%
  filter(release_decision_prearraignment != "Unknown") %>%
