...
book_charge %>%
  count(book_num, event_id, book_date)

book_df %>%
  distinct(event_id, book_date) %>%
  add_count(event_id) %>%
  filter(n>1) %>%
  arrange(event_id)
```
...
#Removing Dates because they are found on the booking table.--FiX DOB
person_df <- person_df %>%
  select(-book_date, -arrest_date)
...
# book_df4 <- read_xlsx("Historical Data Submissions/Los Angeles/P2 Submission-June 30/LASD_Rls_Jan_Jun_2020.xlsx")
# 
# book_df3 <- read_xlsx("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LASD_JAN_DEC_2020.xlsx")

 ## Resubmitted with missing bookings 5/27/2021, Bookings before July 2019 are still missing. 

book_df2 <- lapply(c("Jul_Dec_2019", "JAN_MAR_2020", "April_June_2020", "Jul_Dec_2020"), function(i){
  fread(paste0("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LA_County_Arrest_",i, ".csv"))}) %>%
  bind_rows() 


# book_df2 <- fread("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LA_County_Arrest_Jul_Dec_2019.csv") 
# 
# book_df3 <- read_xlsx("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LA_County_Arrest_JAN_MAR_2020.csv")
# 
# book_df4 <- read_xlsx("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LA_County_Arrest_April_June_2020.csv")
# 
# book_df5 <- read_xlsx("Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/LA_County_Arrest_Jul_Dec_2020.csv")

names(book_df2) <- names(book_df)

#Matching Dates to book_df
book_df2 <- book_df2 %>%
  mutate(across(c("DOB", "Arrest Date", "Booking Date", "Release Date"), dmy)) %>%
  # Removing three incorrect rows
  filter(`Booking Date` >= `Arrest Date`)
...
LA Data Submission Check
```{r}
book_check <- book_charge %>%
  filter(arrest_date >= as_date("2020-01-01")) %>%
  distinct(book_num, .keep_all = T)

book_check2 <- bind_rows(book_df3 %>% distinct(BKG), book_df4 %>% distinct(BKG)) %>% distinct(BKG)

book_match <- book_check %>%
  semi_join(book_check2, by = c("book_num" = "BKG")) 

book_nomatch <- book_check %>%
  anti_join(book_check2, by = c("book_num" = "BKG"))
...
assess_df <- fread("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Los Angeles/P3 Submission-Jan 15/ISAB_PsaHistoricalExtract_20210505.csv",
                  sep = "|", colClasses = "character")
...
assess_df %>%
  select(contains("psa_tool_response"))
