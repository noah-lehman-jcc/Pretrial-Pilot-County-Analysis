...
#Sal's load path
load("C:/PretrialPilot/Pretrial-Pilot-County-Analysis/Assessment Output Data Frame v2.RData")
...
All 16 pretrial pilot program sites have implemented their programs and complied with data reporting requirements. The data contained in the following tables includes all assessments conducted from October 2019 when funding began until October 2020 when data was collected for this report; however, many programs were not fully operational until later in this time frame, with all programs fully operational by June 30th 2020. Also during this time period, the COVID-19 pandemic caused many disruptions to normal court operations that impact the pretrial pilot program, including the implementation of Emergency Rule 4 which set presumptive bail to \$0 for most low-level offenses on a state-wide basis from April 6th to June 20th, and local continuations of \$0 or reduced emergency bail schedules. The participating pilot sites met the original target go-live date despite the challenges brought by the pandemic, but the population of assessed individuals may be different than would be seen in the absence of the pandemic both in terms of numbers and composition. Crime and arrest patterns may have been affected by COVID-19 and local shelter-in-place orders, but also many low-level arrestees who would otherwise have been assessed may have been released with \$0 bail. 

The data processing and analysis for the pretrial pilot project is still in progress. As of the date of this report, the data has not yet been joined across agencies within a county, so the data presented in this report relies on probation data only. The joining of probation data with court, jail, and CA DOJ data will eventually yield more reliable results for data elements that are primarily tracked outside of probation. Data concerning demographic information, release decisions, and risk levels are intended to be drawn from probation data and therefore have a greater degree of reliability. Data on charge level, failures to appear in court, and new criminal activity, however, are primarily tracked by jails, courts, and the CA DOJ. Charge level data is not available at this time, since it is not tracked by probation. For failures to appear in court and new criminal activity, this report draws on the limited data on these outcomes available from probation, but it is likely these numbers will change when the data processing reaches the stage of cross-agency data linkage. Furthermore, the data in this report includes individuals whose pretrial period is not yet complete, and who may accrue adverse outcomes before completion of the pretrial period. The short time frame for this first reporting period means that that many programs have not been operational long enough to accurately measure outcomes.
...
See appendix for detailed information on each tool and differences in VPRAI versions.
...
```
Notes: This table includes data from three counties (Alameda, San Mateo, Santa Barbara).
\newpage
...
```
Notes: Booking charge level is reported in a separate data source from assessment data. Data on this measure is currently unavailable but will be provided in future reports. 
\newpage
...
```
Notes: 'Granted Program Release' includes individuals released on OR or monitoring. 'Denied Program Release' indicates individuals who judicial officers did not choose to release through the pretrial program. These individuals may still be released on bail. 
...
Not everyone who is assessed progresses to consideration for pre-arraignment release by a judicial officer. Even after being assessed, many individuals may bail out (including \$0 bail in response to the pandemic during a large portion of the reporting period) or are released due to their charges being dropped or dismissed. Many individuals are prevented by statute (PC 1270.1, PC 1319.5) from release without a hearing held in open court. These individuals are not considered at the pre-arraignment stage since the pre-arraignment review is not conducted in open court; they will be considered for program release for the first time at arraignment.  Some individuals may have other circumstances that prevent their consideration for pre-arraignment release or release decision data were missing. 

Out of the 24,388 individuals assessed in Los Angeles, 15,158 individuals are not shown in this table because no pre-arraignment program release decision was made. The reasons for the absence of a pre-arraignment program release decision include: these individuals were already released on bail, had their charges dropped or dismissed, or were ineligible for consideration for pre-arraignment release, or pre-arraignment release decision data were missing.
...
```
Notes: 'Granted Prearraignment Program Release' includes individuals released on OR or monitoring at the prearraignment stage. 'Denied Prearraignment Program Release' indicates individuals who judicial officers did not choose to release at the prearraignment stage through the pretrial program. These individuals may still be released on bail or through the pretrial program at arraignment. 
...
kable(monitor_df, booktabs = T, format.args = list(big.mark = ","),
  ...
  row_spec(5, bold = T)
```
Notes: 'Unspecified Supervision' includes individuals in counties that do not categorize supervision conditions into discrete levels. For counties that do use discrete supervision levels, their supervision levels were collapsed into 'Basic', 'Moderate', and 'Enhanced' supervision, though the specific conditions included in each of these supervision levels vary across counties.
\newpage
...
Data quality is low for failures to appear, new offenses during the pretrial period, and revocations of pretrial release. Probation departments keep limited records of FTA and new criminal offenses. For future reports, FTA and new offense data will come from different data sources that are not currently available. The data in this table includes individuals whose pretrial period is not yet complete, and may accrue adverse outcomes before completion of the pretrial period. The short time frame for this first reporting period means that that many programs have not been operational long enough to accurately measure outcomes.
