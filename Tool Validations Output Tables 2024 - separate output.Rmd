...
#knitr::opts_knit$set(root.dir = "C:/Pretrial-Pilot-County-Analysis")
...
outdate <- str_c(format(Sys.Date(), "%Y.%m.%d"), " output")
rd <- str_c("C:/Pretrial-Pilot-County-Analysis/", outdate)

if(file.exists(outdate)){

} else {
  dir.create(file.path("C:/Pretrial-Pilot-County-Analysis", outdate))
...
df <- df %>%
  mutate(tool_name_orig = tool_name,
         ...
         County = case_when(county %in% c("Calaveras", "Modoc", "Tuolumne", "Yuba") ~ "Small Counties",
                            ...
                            T ~ county),
         County = factor(County, ordered = T, levels = c("Small Counties", "Small/Medium Counties")),
         charge_level_orig = charge_level,
         charge_level = charge_level_standard,
         ...
         oras_risk_score = if_else(tool_name == "ORAS", risk_score_raw, NA),
         vprai_risk_score = if_else(tool_name == "VPRAI", risk_score_raw, NA),
         vprair_risk_score = if_else(tool_name == "VPRAI-R", risk_score_raw, NA),
         vpraio_risk_score = if_else(tool_name == "VPRAI-O", generic_tool_total_score_raw, NA),
         psa_fta_risk_score = score_failure_to_appear,
         psa_nca_risk_score = score_new_criminal_activity,
         psa_nvca_risk_score = score_new_criminal_violent_activity,
         scored_assessment = if_else(tool_name %in% c("ORAS", "VPRAI", "VPRAI-R") & !is.na(risk_score_raw) |
  ...
  arrange(County) %>%
  filter(assessment_date < as.Date("2023-01-01")) %>%
...
```
Create subdirectories for each county in df
```{r}
countylist <- unique(df$county)
countylist

#Create subdirectories
lapply(countylist, function(i){
  if(!file.exists(i)){
    dir.create(i)
  }
  })
...
xxx Test saving a file into each county folder
```{r eval=FALSE, include=FALSE}
lapply(countylist, function(i){
  tab <- df %>%
    filter(county == i) %>%
    count(county, sex)
  title <- str_c(i, "/gender counts.csv")
  write.csv(tab, file = title)
})
...
join_df %>%
  count(county, doj_matched) %>%
  group_by(county) %>%
  mutate(perc = round(100*n/sum(n),1)) %>%
  filter(doj_matched == 0)


join_df %>%
  filter(county == "Ventura") %>%
  count(release_date < pretrial_period_end_date)
  count(released_pretrial_count)
  filter(booking_assessed_count == 1,
         scored_assessment == 1,
         release_eligible_count ==1) #,
         released_pretrial_count == 1) #,
...
eval_df %>%
  filter(county == "Sonoma") %>%
  select(doj_matched, booking_date, release_date, pretrial_period_end_date)

eval_df %>%
  filter(vprai_risk_score > 9)
```
...
MyKableHTML function
```{r warning=FALSE, include=FALSE}
my_kablehtml <- function(x){
  kable(x, booktabs = T, format.args = list(big.mark = ","), 
        # table.attr = "style = \"color: black;\"",
        format = "html") %>%
    kable_styling(position = "center", full_width = F, 
...
write.xlsx(roc_df, file = "C:/Pretrial-Pilot-County-Analysis/AUCs by Demographic Category.xlsx", asTable = T)
save(roc_df, file= "C:/Pretrial-Pilot-County-Analysis/AUCs.RData")
...
Load AUC table
```{r}
#auc_df <- read.xlsx("C:/Pretrial-Pilot-County-Analysis/AUCs by Demographic Category.xlsx")
load("C:/Pretrial-Pilot-County-Analysis/AUCs.RData")
...
kable(date_df, booktabs = T, format.args = list(big.mark = ","), 
    ...
    kable_styling(position = "center", full_width = F, 
                  bootstrap_options = c("striped", "hover"), stripe_color = "gray", font_size = 14, latex_options = "hold_position") %>%
  save_kable(file = "temp.html")

chrome_print("temp.html", output = "tool_date_range.pdf", format = "pdf")
...
chrome_print("temp.html", output = "county_date_range.pdf", format = "pdf")


lapply(countylist, function(i){
  kable(date_dfc %>% filter(County == i), booktabs = T, format.args = list(big.mark = ","), 
        # table.attr = "style = \"color: black;\"",
        format = "html") %>%
    kable_styling(position = "center", full_width = F, 
                  bootstrap_options = c("striped", "hover"), stripe_color = "gray", font_size = 14, latex_options = "hold_position") %>%
  save_kable(file = "temp.html")
  title <- str_c(i, "/date_range.pdf")
  chrome_print("temp.html", output = title, format = "pdf")
})
```
# Drop-Off
```{r}
...
chrome_print("temp.html", output = "dropoffs.pdf", format = "pdf")
...
chrome_print("temp.html", output = "tool_dropoff.pdf", format = "pdf")
...
my_kablehtml(demo_df) %>%
  add_header_above(c(" " = 2, "Race/Ethnicity (%)" = 4, "Gender (%)" = 2, " " = 1)) %>%
  save_kable(file = "temp.html")

chrome_print("temp.html", output = "demographics.pdf", format = "pdf")
...
my_kablehtml(demo_df_tool) %>%
  add_header_above(c(" " = 2, "Race/Ethnicity (%)" = 4, "Gender (%)" = 2, " " = 1))%>%
  save_kable(file = "temp.html")

chrome_print("temp.html", output = "tool_demographics.pdf", format = "pdf")
...
chrome_print("temp.html", output = "arrest_offenses.pdf", format = "pdf")
...
chrome_print("temp.html", output = "adverse_outcomes.pdf", format = "pdf")
...
chrome_print("temp.html", output = "tool_adverse_outcomes.pdf", format = "pdf")
...
```{r}
lapply(countylist, function(i){
  kable(drop_df_out %>% filter(County == i), booktabs = T, format.args = list(big.mark = ","), 
        # table.attr = "style = \"color: black;\"",
        format = "html") %>%
    column_spec(3:6, width = "1.5cm") %>%
    kable_styling(position = "center", full_width = F, 
                  bootstrap_options = c("striped", "hover"), stripe_color = "gray", font_size = 10, 
                  latex_options = "hold_position") %>%
  save_kable(file = str_c(i,"/temp.html"))
  title <- str_c(i, "/dropoff.pdf")
  chrome_print(str_c(i,"/temp.html"), output = title, format = "pdf")
})
```
...
lapply(c("Los Angeles", "Sacramento", "Sonoma", "Tulare", "San Joaquin", "Napa", "Alameda", "San Mateo", "Santa Barbara", "Ventura", "Tuolumne"),
  function(x){
...
kable(demo_df %>% filter(County == x), booktabs = T, format.args = list(big.mark = ","), 
  ...
  add_header_above(c(" " = 2, "Race/Ethnicity (%)" = 4, "Gender (%)" = 2, " " = 1)) %>%
  save_kable(file = "temp.html")
    title <- str_c(x, "/demographics.pdf")
    chrome_print("temp.html", output = title, format = "pdf")
  })
...
kable(arrest_df %>% filter(County == x) , booktabs = T, format.args = list(big.mark = ","), 
    ...
    kable_styling(position = "center", full_width = F, 
                  ...
                  latex_options = "hold_position") %>%
      save_kable(file = "temp.html")
    title <- str_c(x, "/arrest_offenses.pdf")
    chrome_print("temp.html", output = title, format = "pdf")
...
lapply(c(tool_list),
  function(x){
kable(drop_df_out_tool %>% filter(`Tool Name` == x) , booktabs = T, format.args = list(big.mark = ","), 
        # table.attr = "style = \"color: black;\"",
        format = "html") %>%
    #column_spec(3:6, width = "1.5cm") %>%
    kable_styling(position = "center", full_width = F, 
...
chrome_print("temp.html", output = str_c(x, "_dropoff.pdf"), format = "pdf")
    })
...
chrome_print("temp.html", output = str_c(x, "_demographics.pdf"), format = "pdf")
    })
...
chrome_print("temp.html", output = str_c(x, "_outcomes.pdf"), format = "pdf")      
    }) 
...
chrome_print("temp.html", output = "tool_assessments.pdf", format = "pdf")      
...
chrome_print("temp.html", output = "tool_assessment_demographics.pdf", format = "pdf")      
...
chrome_print("temp.html", output = "tool_outcomes.pdf", format = "pdf")    
...
ggsave("ORAS_risk_distribution.png")
...
ggsave("ORAS_outcomes_by_risk.png")
...
chrome_print("temp.html", output = "ORAS_risk_distribution.pdf", format = "pdf")    
...
risk_df <- eval_df %>%
   # filter(book_date <= as_date("2021-03-31"),
   #       book_date >= as_date("2019-10-01")) %>%
  filter(tool_name == "VPRAI", !is.na(vprai_risk_score), !vprai_risk_score >5) %>%
...
ggsave("VPRAI_risk_distribution.png")
...
ggsave("VPRAI_outcomes_by_risk.png")
...
chrome_print("temp.html", output = "VPRAI_risk_distribution.pdf", format = "pdf")    
...
ggsave("VPRAIR_risk_distribution.png")
...
chrome_print("temp.html", output = "VPRAIR_risk_distribution.pdf", format = "pdf")    
...
ggsave("PSA_risk_distribution.png")
...
ggsave("PSA_outcomes_by_risk.png")
...
chrome_print("temp.html", output = "PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "PSA_NVCA_distribution.pdf", format = "pdf")    
...
```{r}
risk_df <- eval_df %>%
  filter(tool_name == "PSA", county == "Alameda") %>%
  group_by(psa_fta_risk_score) %>%
  summarise(across(c(fta_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(fta_flag) 
...
risk_df3 <- eval_df %>%
  filter(tool_name == "PSA", !is.na(psa_nvca_risk_score), county == "Alameda") %>%
  group_by(psa_nvca_risk_score) %>%
  summarise(across(c(recid_arrest_violent_psa_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(recid_arrest_violent_psa_flag) 
...
ggplot(risk_df %>%
  ...
  # ylim(0,100) +
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent", title = "Alameda Distribution of Assessments by PSA Risk Category")  +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
ggsave("Alameda/risk_distribution.png")
...
ggplot(risk_df) +
  ...
  # scale_x_discrete(limits = 1:6) +
  facet_wrap(~name, scales = "free_x") +
  labs(x = "Risk Category", y = "Percent", title = "Alameda PSA Outcomes by Risk Category")
ggsave("Alameda/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Alameda/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Alameda/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Alameda/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage


XXX OLD -- Alameda VPRAIR
\newline
```{r eval=FALSE, include=FALSE}
risk_df <- eval_df %>%
   # filter(book_date <= as_date("2021-03-31"),
...
ggplot(risk_df %>%
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
ggsave("Los Angeles/PSA_risk_distribution.png")
...
ggsave("Los Angeles/PSA_outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Los Angeles/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Los Angeles/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Los Angeles/PSA_NVCA_distribution.pdf", format = "pdf")    
...
ggsave("San Mateo/risk_distribution.png")
...
ggsave("San Mateo/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "San Mateo/VPRAIR_distribution.pdf", format = "pdf")    
...
ggplot(risk_df %>%
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
ggsave("Sacramento/risk_distribution.png")
...
ggsave("Sacramento/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Sacramento/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Sacramento/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Sacramento/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage
...
```{r}
risk_df <- eval_df %>%
  filter(tool_name == "PSA", county == "Tuolumne") %>%
  group_by(psa_fta_risk_score) %>%
  summarise(across(c(fta_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(fta_flag) 
...
risk_df3 <- eval_df %>%
  filter(tool_name == "PSA", !is.na(psa_nvca_risk_score), county == "Tuolumne") %>%
  group_by(psa_nvca_risk_score) %>%
  summarise(across(c(recid_arrest_violent_psa_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(recid_arrest_violent_psa_flag) 
...
ggplot(risk_df %>%
  ...
  # ylim(0,100) +
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent", title = "Tuolumne Distribution of Assessments by PSA Risk Category")  +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
ggsave("Tuolumne/risk_distribution.png")
...
ggplot(risk_df) +
  ...
  # scale_x_discrete(limits = 1:6) +
  facet_wrap(~name, scales = "free_x") +
  labs(x = "Risk Category", y = "Percent", title = "Tuolumne PSA Outcomes by Risk Category")
ggsave("Tuolumne/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Tuolumne/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Tuolumne/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Tuolumne/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage


xxx DUPLICATED Alameda PSA
\newline
```{r}
risk_df <- eval_df %>%
  filter(tool_name == "PSA", county == "Alameda") %>%
...
ggsave("San Joaquin/risk_distribution.png")
...
ggsave("San Joaquin/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "San Joaquin/VPRAI_distribution.pdf", format = "pdf")    
...
ggsave("Santa Barbara/risk_distribution.png")
...
ggsave("Santa Barbara/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Santa Barbara/VPRAIR_distribution.pdf", format = "pdf")    
...
ggplot(risk_df %>%
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) 
ggsave("Sonoma/risk_distribution.png")
...
ggsave("Sonoma/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Sonoma/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Sonoma/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Sonoma/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage



***** Ventura -- NEEDS WORK, NO VALUES
\newline
```{r}
risk_df <- eval_df %>%
  filter(tool_name == "PSA", county == "Ventura") %>%
  group_by(psa_fta_risk_score) %>%
  summarise(across(c(fta_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(fta_flag) 
...
risk_df3 <- eval_df %>%
  filter(tool_name == "PSA", !is.na(psa_nvca_risk_score), county == "Ventura") %>%
  group_by(psa_nvca_risk_score) %>%
  summarise(across(c(recid_arrest_violent_psa_flag),
                   ~round(mean(.x)*100, 1)), Total = n()) %>%
  pivot_longer(recid_arrest_violent_psa_flag) 
...
ggplot(risk_df %>%
  ...
  # ylim(0,100) +
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent", title = "Ventura Distribution of Assessments by PSA Risk Category")  +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) 
ggsave("Ventura/risk_distribution.png")
...
ggplot(risk_df %>%
  ...
  # scale_x_discrete(limits = 1:6) +
  facet_wrap(~name, scales = "free_x") +
  labs(x = "Risk Category", y = "Percent", title = "Ventura PSA Outcomes by Risk Category")
ggsave("Ventura/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Ventura/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Ventura/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Ventura/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage

XXXX Ventura OLD, SWITCHED TO PSA
\newline
```{r}
risk_df <- eval_df %>%
   # filter(book_date <= as_date("2021-03-31"),
...
ggsave("Napa/risk_distribution.png")
...
chrome_print("temp.html", output = "Napa/ORAS_distribution.pdf", format = "pdf")    
...
ggplot(risk_df %>%
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) 
ggsave("Tulare/risk_distribution.png")
...
ggsave("Tulare/outcomes_by_risk.png")
...
chrome_print("temp.html", output = "Tulare/PSA_FTA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Tulare/PSA_NCA_distribution.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "Tulare/PSA_NVCA_distribution.pdf", format = "pdf")    
```
\newpage

Calaveras - TEMPORARILY REMOVED
\newline
```{r eval=FALSE, include=FALSE}
risk_df <- eval_df %>%
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "PSA_regression.html")
```
\newpage

XXXX Alameda - VPRAI-R *** OLD **** SWITCHED TO PSA
\newline
...
XXXX Alameda with zero bail dummy - VPRAI-R
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- eval_df %>%
  filter(county == "Alameda")
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Los Angeles/PSA_regression.html")
```
\newpage
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Los Angeles/PSA_regression_zb_main.html")
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Los Angeles/PSA_regression_zb_interaction.html")
...
ggsave("Los Angeles/PSA_regression_with_zb.png")
# eval_df %>%
#   filter(tool_name == "PSA") %>%
#   count(county)
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Sacramento/PSA_regression.html")
```
\newpage
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Tuolumne/PSA_regression.html")
```
\newpage
...
```{r, results='asis'}
county_df <- eval_df %>%
  filter(county == "Alameda", tool_name == "PSA")
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Alameda/PSA_regression.html")
```
\newpage
...
stargazer(list(m1,m2,m3,m4,m5,m6),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "San Mateo/VPRAIR_regression.html")
```
\newpage

San Mateo with zero bail dummy --- ignored for now
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- eval_df %>%
  filter(county == "San Mateo")
...
stargazer(list(m1,m2,m3,m4,m5,m6),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "San Joaquin/VPRAI_regression.html")
```
\newpage

San Joaquin with zero bail dummy ---- IGNORED FOR NOW
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- eval_df %>%
  filter(county == "San Joaquin")
...
stargazer(list(m1,m2,m3,m4,m5,m6),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Santa Barbara/VPRAIR_regression.html")
```
\newpage
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Sonoma/PSA_regression.html")
```
\newpage

xxxxVentura ---- ORAS *** OLD TOOL **** 
\newline
```{r eval=FALSE, include=FALSE, results='asis'}
county_df <- eval_df %>%
  filter(county == "Ventura")
...
stargazer(list(m1,m2,m3,m4,m5),
          ...
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small",
          out = "Tulare/PSA_regression.html")
```
\newpage
...
chrome_print("temp.html", output = "VPRAI_AUC.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "VPRAIR_AUC.pdf", format = "pdf")    
```
\newpage
...
chrome_print("temp.html", output = "ORAS_AUC.pdf", format = "pdf")    
```
\newpage
...
chrome_print("temp.html", output = "PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
chrome_print("temp.html", output = "Los Angeles/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
chrome_print("temp.html", output = "San Mateo/VPRAIR_AUC.pdf", format = "pdf") 
...
chrome_print("temp.html", output = "Sacramento/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
```{r}
tool_df <- auc_df %>%
  filter(tool_name == "PSA", race == "All", sex == "All", county == "Tuolumne") %>%
  select(risk_score, outcome, AUC, CI, n) %>%
  mutate(risk_score = case_when(risk_score == "psa_fta_risk_score" ~ "PSA FTA",                                 
                                risk_score == "psa_nca_risk_score" ~ "PSA NCA",                                 
                                risk_score == "psa_nvca_risk_score" ~ "PSA NVCA")) %>%
...
chrome_print("temp.html", output = "Tuolumne/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
```{r}
tool_df <- auc_df %>%
  filter(tool_name == "PSA", race == "All", sex == "All", county == "Alameda") %>%
  select(risk_score, outcome, AUC, CI, n) %>%
  mutate(risk_score = case_when(risk_score == "psa_fta_risk_score" ~ "PSA FTA",                                 
                                risk_score == "psa_nca_risk_score" ~ "PSA NCA",                                 
                                risk_score == "psa_nvca_risk_score" ~ "PSA NVCA")) %>%
...
chrome_print("temp.html", output = "Alameda/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
chrome_print("temp.html", output = "San Joaquin/VPRAI_AUC.pdf", format = "pdf") 
```
\newpage
...
kable(tool_df %>% select(-starts_with("N")), booktabs = T, format.args = list(big.mark = ","), 
    ...
    kable_styling(position = "center", full_width = F, 
                  bootstrap_options = c("striped", "hover"), stripe_color = "gray", font_size = 12, 
                  latex_options = "hold_position") %>%
  footnote(paste("N =", tool_df$N[1]), footnote_as_chunk = T)  %>%
      save_kable(file = "temp.html")

chrome_print("temp.html", output = "Santa Barbara/VPRAIR_AUC.pdf", format = "pdf") 
...
chrome_print("temp.html", output = "Sonoma/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
chrome_print("temp.html", output = "Napa/ORAS_AUC.pdf", format = "pdf") 
```
\newpage
...
chrome_print("temp.html", output = "Tulare/PSA_AUC.pdf", format = "pdf") 
```
\newpage
...
ggsave("ORAS_regression_race_graph.png")
...
chrome_print("temp.html", output = "ORAS_regression_race_table.pdf", format = "pdf")    
...
ggsave("VPRAI_regression_race_graph.png")
...
chrome_print("temp.html", output = "VPRAI_regression_race_table.pdf", format = "pdf")    
```
\newpage
...
ggsave("VPRAIR_regression_race_graph.png")
...
chrome_print("temp.html", output = "VPRAIR_regression_race_table.pdf", format = "pdf")    
```
\newpage
...
ggsave("PSA_regression_race_graph.png")
...
chrome_print("temp.html", output = "PSA_FTA_regression_race_table.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "PSA_NCA_regression_race_table.pdf", format = "pdf")    
...
chrome_print("temp.html", output = "PSA_NVCA_regression_race_table.pdf", format = "pdf")    
...
Santa Barbara--Without Interactions - White and Hispanic only
\newline
```{r results='asis'}
county_df <- race_df %>%
  filter(county == "Santa Barbara") %>%
...
stargazer(list(m1,m2,m3,m4,m5, m6),
          ...
          dep.var.labels = c("FTA", "New Arrest", "New Filing", 
                             "New Conviction", "New Violent Arrest", "FTA or New Arrest"), 
          covariate.labels = c("VPRAI-R Risk Score", "Race:Hispanic", "Days Released"),
          star.char = c("*", "**", "***"),
          star.cutoffs = c(.05, .01, .001),
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small")
...
stargazer(list(m1,m2,m3,m4,m5, m6),
          ...
          dep.var.labels = c("FTA", "New Arrest", "New Filing", 
                             "New Conviction", "New Violent Arrest", "FTA or New Arrest"), 
          covariate.labels = c("VPRAI-R Risk Score", "Race:Hispanic",
                               "Days Released", "VPRAI-R*Hispanic"),
          star.char = c("*", "**", "***"),
          star.cutoffs = c(.05, .01, .001),
          notes = c("*p<0.05; **p<0.01; ***p<.001"),
          notes.append = F, font.size = "small")
...
kable(tool_df[1:5], booktabs = T, format.args = list(big.mark = ","), 
    ...
    kable_styling(position = "center", full_width = F, 
                  ...
                  font_size = 10, latex_options = "hold_position") %>%
  add_header_above(c(" " = 1, "AUC" = 2, "CI (95%)" = 2)) %>% 
  footnote(paste("N White =", tool_df$N_White[1], ", N Hispanic =", tool_df$N_Hispanic[1]), footnote_as_chunk = T)
```
...
Santa Barbara--With Interactions at score 1 
\newline
```{r results='asis'}
county_df <- sex_df %>%
  filter(county == "Santa Barbara") %>%
...
kable(tool_df[1:5], booktabs = T, format.args = list(big.mark = ","), 
  ...
  add_header_above(c(" " = 1, "AUC" = 2, "CI (95%)" = 2)) %>% 
  footnote(paste("N Female =", tool_df$N_Female[1], ", N Male =", 
                     tool_df$N_Male[1]), footnote_as_chunk = T)
```
\newpage
...
rocW_anyfailure <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Female"), ci = T)
rocM_anyfailure <- roc(any_pretrial_failure_flag ~ vprair_risk_score, data = eval_df %>% filter(tool_name == "VPRAI-R") %>% filter(county == "Santa Barbara") %>% filter(sex == "Male"), ci = T)
...
Alameda-- graphs by court_date_reminder_flag
\newline
```{r fig.height=6, fig.width=10, dpi=1200}
risk_df <- eval_df %>%
  filter(county == "Alameda") %>%
  ...
  filter(tool_name == "VPRAI-R", !is.na(vprair_risk_score)) %>%
  group_by(vprair_risk_score, court_date_reminder_flag) %>%
  select(vprair_risk_score, court_date_reminder_flag, fta_flag, recid_arrested_flag, recid_filed_flag, recid_convicted_flag,
         any_pretrial_failure_flag, recid_arrest_violent_psa_flag) %>%
  pivot_longer(fta_flag:recid_arrest_violent_psa_flag) %>%
  mutate(name = case_when(name == "fta_flag" ~ "FTA",
                          name == "recid_arrested_flag" ~ "New Arrest",
...
ggplot(risk_df) +
  aes(x = vprair_risk_score, y = value, color = court_date_reminder_flag) +
  geom_smooth(method = "glm", method.args = list(family=binomial), alpha = .2) +
  # geom_line() +
  # geom_point() +
  theme_bw() +
  ...
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent",
       title = "Comparison of Court Date Reminder Differences in Logistic Regression Curves--VPRAI-R, Alameda")# +
  #scale_x_continuous(breaks = 0:14)
...
Alameda-- FTA graph only by court_date_reminder_flag
\newline
```{r fig.height=6, fig.width=10, dpi=1200}
risk_df <- eval_df %>%
  filter(county == "Alameda") %>%
  ...
  filter(tool_name == "VPRAI-R", !is.na(vprair_risk_score)) %>%
  group_by(vprair_risk_score, court_date_reminder_flag) %>%
  select(vprair_risk_score, court_date_reminder_flag, fta_flag) %>%
  pivot_longer(fta_flag) %>%
  mutate(name = case_when(name == "fta_flag" ~ "FTA"),
...
Tulare graphs by court_date_reminder_flag
\newline
```{r fig.height=6, fig.width=10, dpi=1200}
risk_df <- eval_df %>%
  filter(county == "Tulare") %>%
...
ggplot(risk_df) +
  ...
  # ylim(0,100) +
  facet_wrap(~name, scales = "free") +
  labs(x = "Risk Category", y = "Percent", title = "Comparison of Court Date Reminder Differences in Logistic Regression Curves--PSA, Tulare") +
  scale_x_continuous(breaks = 1:6)
...
Look at supervision flags
```{r}
tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county %in% c("Sonoma", "Tulare", "Alameda")), fta_flag ~ court_date_reminder_flag))

countylist <- join_df %>%
  distinct(county) %>%
  filter( county %in% c("Sonoma", "Tulare", "Alameda")) %>%
  pull(county)

#simple regression, fta by court date reminder
lapply(countylist, 
       function(f){
         tidy(lm(data = join_df %>%
         filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == f), fta_flag ~ court_date_reminder_flag)) %>%
           mutate(county = f)
       })


# regression by county, fta by risk score, days released, and court date reminder
lapply(countylist, 
       function(f){
         tidy(lm(data = join_df %>%
         filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == f) %>% 
           mutate(days_released = as.numeric(case_disposition_date - booking_date),
                  risk_fta_or_v = case_when(tool_name == "PSA" ~ psa_fta_risk_score,
                                            tool_name == "VPRAI-R" ~ vprair_risk_score)), fta_flag ~ court_date_reminder_flag + risk_fta_or_v + days_released)) %>%
           mutate(county = f)
       })

# regression by county, fta by risk score, days released, and court date reminder with interaction btwn reminder and risk score at risk score 1
lapply(countylist, 
       function(f){
         tidy(lm(data = join_df %>%
         filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == f) %>% 
           mutate(days_released = as.numeric(case_disposition_date - booking_date),
                  risk_fta_or_v = case_when(tool_name == "PSA" ~ psa_fta_risk_score,
                                            tool_name == "VPRAI-R" ~ vprair_risk_score)) %>%
           mutate(risk_fta_or_v = risk_fta_or_v -1), 
         fta_flag ~ court_date_reminder_flag*risk_fta_or_v + days_released)) %>%
           mutate(county = f)
       })
...
```{r}
tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == "Tulare") %>%
    mutate(days_released = as.numeric(case_disposition_date - booking_date)), fta_flag ~ court_date_reminder_flag + psa_fta_risk_score + days_released))

tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == "Sonoma") %>%
    mutate(days_released = as.numeric(case_disposition_date - booking_date)), fta_flag ~ court_date_reminder_flag + psa_fta_risk_score + days_released))

tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county == "Alameda") %>%
    mutate(days_released = as.numeric(case_disposition_date - booking_date)), fta_flag ~ court_date_reminder_flag + vprair_risk_score + days_released))


tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county %in% c("Sonoma", "Tulare", "Alameda")) %>%
    mutate(days_released = as.numeric(case_disposition_date - booking_date)), fta_flag ~ court_date_reminder_flag + risk_score_raw + days_released))

tidy(lm(data = join_df %>%
  filter(is_booking_assessed == "Yes", is_released_pretrial == "Yes", county %in% c("Tulare", "Sonoma")) %>%
    mutate(days_released = as.numeric(case_disposition_date - booking_date)), fta_flag ~ court_date_reminder_flag + psa_fta_risk_score + days_released))
