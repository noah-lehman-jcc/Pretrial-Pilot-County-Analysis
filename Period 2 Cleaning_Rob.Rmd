...
book_df <- lapply(c("Historical Data Submissions/Yuba/JailExtract.csv", 
                    "Historical Data Submissions/Yuba/JailExtractQ2.csv",
                    "Historical Data Submissions/Yuba/P2 Submission-June 30/pretrial.csv"), 
                  function(x) {
                    cbind(fread(x, colClasses = "character"),
                          data.frame(file = x))
                  }) %>%
  bind_rows() %>%
  arrange(desc(file)) %>%
  distinct(BOOKNX, .keep_all = TRUE) %>%
  select(-file)
...
court_charge <- court_charge %>%
  mutate_at(vars(plea_date, disposition_date, final_date), ymd_hms)
...
court_person %>%
  semi_join(court_fta_war, by = "court_docket_id")
...
summary(court_sent_prob$sentence_date)
...
assess_df <- assess_df %>%
  ...
  mutate(pretrial_termination_outcome = case_when(!is.na(`ref_status_reason_Pretrial - Closed Successful`) ~ "Successful",
                                                  ...
                                                  !is.na(`ref_status_reason_Pretrial - Closed Neutral`) ~ "Other_Unknown")) %>%
...
assess_df <- assess_df %>%
  left_join(assess_df2, by = "court_case_id") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .)))
...
assess_table <- assess_df %>%
  ...
  select(county, release_recommendation, release_decision, assessment_date, oras_risk_score, tool_name)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Yuba Assessment Output 08-2020.csv")
```
...
# Assessment data is duplicated across terms and conditions. These are collapsed. 
  ## data are already collapsed, so this step is optional 
assess_df <- assess_df %>%
  group_by(pretrial_id, assessment_date) %>%
  mutate_at(vars(contains("flag")), funs(max(., na.rm = T))) %>%
  distinct(pretrial_id, assessment_date, court_case_id, .keep_all = T) 
