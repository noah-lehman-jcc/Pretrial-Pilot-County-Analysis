...
prob_response <- read_excel("Historical Data Submissions/Los Angeles/California JCC Historical EM Records (Revised 2019-11-25).xlsx", 
                      sheet = 2)

original_field <- names(prob_response)

names(prob_response) <- c("prob_id", "risk_question", "risk_response", "score")

original_df[[1]] <- tibble(table_name = "prob_response", original_field = original_field, modified_field = names(prob_response))

#Probation Demographics 
prob_df <- read_excel("Historical Data Submissions/Los Angeles/California JCC Historical EM Records (Revised 2019-11-25).xlsx", 
                      sheet = 1)

original_field <- names(prob_df)

names(prob_df) <- c("prob_id", "app_id", "cii", "fbi", "local_id", "dln", "first_name", "last_name", "dob", "sex", "race",
                    "app_id_redundant", "tool_name", "risk_assessment_date_time", "zip_code", "tool_response", "score_1", 
                    "score_2", "release_recommendation", "release_authorization", "release_type", "release_date_time",
                    "pretrial_terms","violation_reasonr", "violation_date", "court_date_reminder", "other_service",
                    "termination_outcome", "termination_date")

original_df[[2]] <- tibble(table_name = "prob_df", original_field = original_field, modified_field = names(prob_df))

prob_df <- prob_df %>%
  mutate_at(vars(contains("date_time")), funs(ymd_hms(., tz = 'America/Los_Angeles'))) %>%
  mutate_at(vars(termination_date, violation_date, dob), as_date)
```
...
book_df$book_dateo_time <- as_datetime(book_df$book_date_time)
...
book_df<- book_df %>%
    mutate(arrest_misd_flag=if_else(arrest_charge_level=="M", 1, 0),
           ...
           arrest_attempt_flag=if_else(attempt=="1", 1, 0),
           arrest_fta_flag=if_else(arrest_charge%in% c("8537", "1320A", "1320B", "13205", "40508A"), 1, 0)) 
...
book_df <- book_df %>%
  ...
  group_by(local_id, book_date, court_case_id) %>%
  filter(hierarchy == min(hierarchy)) %>%
  ungroup() %>%
  distinct(local_id, book_date, court_case_id, .keep_all = T) 
...
court_charges<-court_charges %>%
mutate(enhancement = if_else(join_var %in% c("Other_VC_14601B2", "Other_VC_146011B2","Other_VC_146012D2","Other_VC_146015D2",
                                             "Other_VC_23578",       "Other_VC_231035", "Other_VC_23540", "Other_PC_117012C1",
                                             "Other_PC_120225A","Other_PC_1170H3","Other_PC_1170H", "Other_PC_120227A", "Other_PC_120221",
                                             "Other_PC_667A1", "Other_PC_667A1", "Other_PC_12022A1", "Other_PC_12022B1", "Other_PC_1203E4",
                                             "Other_VC_23577", "Other_PC_1202253C", "Other_PC_290018BZ", "Other_PC_290018BZ",
                                             "Other_PC_1202253B","Other_PC_6676D", "Other_PC_1202253D", "Other_PC_1203K", "Other_HS_113702A",
                                             "Other_PC_120307A11","Other_HS_11370A","Other_PC_6665", "Other_PC_6675B", "Other_PC_6675C21",
                                             "M_VC_23578", "M_VC_231035", "F_PC_117012C1", "F_PC_120225A", "F_PC_1170H3", "F_PC_1170H",
                                             "F_PC_120227A", "F_PC_120221", "F_PC_667A1", "F_PC_667A1", "F_PC_12022A1", "F_PC_12022B1",  
                                             "F_VC_23577", "F_PC_1202253C", "F_PC_290018BZ", "F_PC_290018BZ", "F_PC_1202253B", "F_PC_6676D",
                                             "F_PC_1202253D", "F_PC_1203K", "F_HS_113702A", "F_PC_120307A11", "F_HS_11370A", "F_PC_6675C21",
                                             "M_VC_231035A"), 1, 0), 
       hierarchy = if_else(is.na(hierarchy) & enhancement == 1, 1e6, hierarchy),
 enhancement=if_else(court_charge_level_original %in% c("E"), 1, 0), hierarchy = if_else(is.na(hierarchy) & enhancement == 1, 1e6, hierarchy)) %>%
  mutate(hierarchy = if_else(court_charge_level == "I" & is.na(hierarchy), 1e6, hierarchy))%>%
  mutate(hierarchy = if_else(join_var == "Other_Other_NA" & is.na(hierarchy), 1e6, hierarchy))
...
court_charges <- court_charges %>%
  mutate(disposition_type = case_when(disposition_type_original %in% c("DISMISSED") ~ "Dismissed",
                         ...
                         disposition_type_original %in% c("CONVICTED") ~ "Gulity",
...
#Make flags
court_charges <- court_charges %>%
  mutate(arrest_misd_flag=if_else(court_charge_level=="M", 1, 0),
          court_felony_flag =if_else(court_charge_level=="F", 1, 0),
          court_attempt_flag=if_else(attempt=="1", 1, 0),
          court_qualifier_flag=if_else(enhancement==1, 1, 0),
          court_attempt_flag=if_else(attempt==1, 1, 0))
...
# Diagnostic Code for Charge Hierarchy
# 1 - 7,901/267,372 = 97% non-match rate
(1 - nrow(court_charges %>% filter(is.na(hierarchy)))/nrow(court_charges))*100
...
court_hearing <- fread("Historical Data Submissions/Nevada/PTRiskAss-Evnt3.csv")


#original_field <- names(court_hearing)

names(court_hearing) <- c("court_docket_id", "court_case_id", "local_id", "hearing_id", "hearing_date", "hearing_type_original", "hearing_result")
...
court_hearing<-court_hearing %>%
  mutate(hearing_type = case_when(hearing_type_original %in% c("ARRAIGNMENT", "WARRANT ARRAIGNMENT", "ARRAIGNMENT ON VIOLATION OF PROBATION (VOP)",
                                                                       ...
                                                                       "ARR ON PETITION TO REV/MOD MANDATORY SUPERVISION", "FELONY ARRAIGNMENT", "DUI ARRAIGNMENT") ~ "Arraignment",
                                hearing_type_original %in% c("SENTENCING", "FELONY SENTENCING", "SENTENCING ON VIOLATION OF PROBATION",
                                                                     "SENTENCING ON VIOLATION OF PROBATION - FELONY", "SENTENCING ON PETITION TO REVOKE/MODIFY PAROLE",
                                                                     "SENTENCING ON PET TO REV/MOD MANDATORY SUPERVISION", "SENTENCING ON PETITION TO MODIFY/REVOKE PRCS") ~ "Sentencing",  
                                T ~ "Other_Unknown"),
...
# San Mateo
- Everything is Join (what does this mean?)
...
book_df <- book_df %>%
  mutate(sex = case_when(sex_original %in% c("U", "") ~ "Other_Unknown",
        ...
        #employment_status = case_when(employment_status_original == "UNEMPLOYED" ~ "Unemployed",
                                 #      employment_status_original %in% c("FULL TIME", "PART TIME", "SELF EMPLOYED") ~ "Unemployed",
                                  #     employment_status_original %in% c("SEASONAL", "CASUAL", "RETIRED") ~ "Other"))
...
book_df%>%
  filter(grepl("Other_VC_146011B2", join_var))%>%
...
court_df <- fread("Historical Data Submissions/San Mateo/41-COURT-01OF03-JAN2016-SEP2019.csv", na.strings=c("NULL",""))

# original_field <- names(court_df)

court_df_1 <- fread("Historical Data Submissions/San Mateo/41-COURT-01OF03-OCT2019-DEC2019.csv", na.strings=c("NULL",""))

court_df <- bind_rows(court_df, court_df_1) %>% distinct()

rm(court_df_1)

names(court_df) <- c("court_docket_id", "court_case_id", "file_date", "court_case_type", "court_party_id", "cii", "fbi", "local_id", 
...
court_df <- court_df %>%
  mutate_at(vars(dob, file_date), as_date)
...
names(court_sentence) <- c("court_docket_id", "court_case_id", "file_date", "court_case_type", "court_case_status_original", "court_case_status_date", "book_num", "arrest_date", "arrest_control", "arrest_agency", "offense_id", "charge_id", "charge_num", "offense_date", "court_charge_original", "court_charge_level_original", "court_charge_description", "plea_type_original", "plea_date", "disposition_type_original", "disposition_date", "final_date", "sentence_type_original", "sentence_date",  "probation_type", "term_years_probation", "term_months_probation", "term_days_probation", "jail_type", "term_years_jail", "term_months_jail", "term_days_jail")
...
court_sentence <- court_sentence %>%
  mutate(disposition_type = case_when(disposition_type_original %in% c("Dismissal: Negotiated Plea", "Dismissal: Insufficient Evidence", "Dismissal: Interest of Justice",
                                                                       "Dismissal: Motion of the People", "Dismissal: Lack of Prosecution", "Dismissal",
                                                                       "No Finding", "Dismissal: Other Dismissal", "Dismissal: Insufficient Grounds to HTA",
                                                                       "Dismissal: After Drug Court", "Dismissal: Civil Compromise", "Dismissal: After Military Diversion",
                                                                       "Dismissal: No Reason Available", "Dismissal: PC1100 -Insufficient Evid. -Witness for Co-Defend", "Dismissal: PC995",
                                                                       "Dismissal: Prop 64", "Dismissal: PC1385 PC-Furtherance of Justice", 
                                                                       "Dismissal: After Deferred Entry of Judgment", "Dismissal: Other Dismissal",
                                                                       "Dismissal: After Diversion", "Dismissal: PC1377/1378-Case Compromise/Rest of Satis. Made", "Dismissal: Post Sentence",
                                                                       "Dismissal: PC1381/1381.5/1382-Delay; Not Timely", "Dismissal: PC995-Accusation Set Aside",
                                                                       "Dismissal: PC871-Court Found Insufficient Cause", "Dismissal: 1203.4") ~ "Dismissed", 
                                      
                         #disposition_type_original %in% c("") ~ "Dropped",
                         
                         disposition_type_original %in% c("Pled Nolo Contendere", "Pled Nolo Contendere Lesser Included Offense") ~ "Nolo_Contendere",
                         
                         disposition_type_original %in% c("Admitted", "Found True", "Pled Guilty", "Jury verdict of guilty", "Sentenced: Plea of Guilty/Nolo Contendere", 
                                                          "Court trial found guilty", "Found True", "Found Guilty", 	"Found Guilty Lesser Included Offense") ~ "Gulity",
                         
                         disposition_type_original %in% c("DOJ - Found Not Guilty", "Denied", "Dismissal: Insufficient Grounds to HTA", 
                                                          "Jury verdict of not guilty", "Found Not True", "Court trial found not guilty") ~ "Not_Guilty",
                         
                         disposition_type_original %in% c("Held to Answer", "Certified to Superior Court: PN859", "Stricken", "Held to Answer",
                                                           "Certified to Superior Court", "Not Held to Answer", "No Finding", "Extradited", "Consolidated", 
                                                          "Veteran's Treatment/Military Diversion", "NULL", "Mistrial", "Certified to Juvenile Court",
                                                          "Certified to Superior Court: PG859") ~ "Other"),

         

        plea_type = case_when(plea_type_original %in% c("No Contest / Nolo Contendere") ~ "Nolo_Contendere",
                               plea_type_original %in% c("Admit", "Guilty") ~ "Guilty",
        ...
        sentence_type = case_when(sentence_type_original %in% c("Prison Sentence") ~ "Prison",
                               sentence_type_original %in% c("Sentenced  PC1170(h) - Straight", "Mandatory Supervision Violation Sentence", "Probation Violation Sentence") ~ "Jail",
                               sentence_type_original %in% c("Probation Sentence", "Sentenced Prop 36") ~ "Probation",
                               sentence_type_original %in% c("Sentenced", "DIVERSION", "na_missing", "Sentenced PC1170(h) - Split", "Sentence Modified", "Drug Court Sentence"
                                                              ,"DUI Court Sentence") ~ "Other"),

         court_case_status = case_when(court_case_status_original %in% c("Active") ~ "Active",
                                court_case_status_original %in% c("Closed") ~ "Inactive"))
                                # court_case_status_original %in% c() ~ "Other"))
...
court_sentence <- left_join(court_sentence, charge_hier, by = "join_var")  

court_sentence<-court_sentence %>%
 mutate(enhancement = if_else(join_var %in% c("Other_VC_14601B2", "Other_VC_146011B2","Other_VC_146012D2","Other_VC_146015D2", "Other_VC_23578", "Other_VC_231035", "Other_VC_23540",                                              "Other_PC_117012C1", "Other_PC_120225A","Other_PC_1170H3","Other_PC_1170H", "Other_PC_120227A", "Other_PC_120221", "Other_PC_667A1", 
                              "Other_PC_667A1", "Other_PC_12022A1", "Other_PC_12022B1", "Other_PC_1203E4", "Other_VC_23577", "Other_PC_1202253C", "Other_PC_290018BZ", 
                              "Other_PC_290018BZ", "Other_PC_1202253B","Other_PC_6676D", "Other_PC_1202253D", "Other_PC_1203K", "Other_HS_113702A",    
                              "Other_PC_120307A11","Other_HS_11370A","Other_PC_6665", "Other_PC_6675B", "Other_PC_6675C21", "M_VC_23578", "M_VC_231035",  
                              "F_PC_117012C1", "F_PC_120225A", "F_PC_1170H3", "F_PC_1170H", "F_PC_120227A", "F_PC_120221", "F_PC_667A1", "F_PC_667A1", "F_PC_12022A1", 
                              "F_PC_12022B1",   "F_VC_23577", "F_PC_1202253C", "F_PC_290018BZ", "F_PC_290018BZ", "F_PC_1202253B", "F_PC_6676D", "F_PC_1202253D",    
                              "F_PC_1203K", "F_HS_113702A", "F_PC_120307A11", "F_HS_11370A", "F_PC_6675C21", "M_VC_231035A"), 1, 0), 
                        hierarchy = if_else(is.na(hierarchy) & enhancement == 1, 1e6, hierarchy),
       enhancement=if_else(court_charge_level_original %in% c("E"), 1, 0), hierarchy = if_else(is.na(hierarchy) & enhancement == 1, 1e6, hierarchy)) %>%
  mutate(hierarchy = if_else(court_charge_level == "I" & is.na(hierarchy), 1e6, hierarchy))
...
#Flag Construction
court_sentence<- court_sentence %>%
    mutate(court_misd_flag=if_else(court_charge_level_original=="M", 1, 0),
           court_felony_flag =if_else(court_charge_level_original=="F", 1, 0),
           court_qualifier_flag=if_else(court_charge_level_original %in% c("E","P"), 1, 0))
...
court_hearing_1 <- fread("Historical Data Submissions/San Mateo/41-COURT-03OF03-OCT2019-DEC2019.csv", na.strings=c("NULL","")) %>%
  mutate_at(vars(contains("date"), DtFile), mdy)


court_hearing <- bind_rows(court_hearing, court_hearing_1) %>% distinct()

rm(court_hearing_1)

names(court_hearing) <- c("court_docket_id", "court_case_id", "file_date", "court_case_type", "setting_id", "hearing_date", "hearing_type_loc", "court_loc", "hearing_type_code", "hearing_type_original", "hearing_result", "hearing_date_next", "fta", "fta_warrant")


court_hearing<-court_hearing %>%
  mutate(hearing_type = case_when(hearing_type_original %in% c("Arraignment","Arraignment on Information","Warrant-Arraignment","Domestic Violence Complaint Arraignment",
                                                              "Further Arraignment", "Arraignment - Felony Fugitive","Arraignment FTA","Arraignment on Information after Preliminary Hearing",
                                                              "Arraignment Parole Violation") ~ "Arraignment",
                                hearing_type_original %in% c("Sentencing", "Probation Report and Sentencing", "Sentence Hearing", 
                                                                     "Prob Report and Sentencing - Proof of Firearm Relinquishment", 
                                                                        "Proof of Firearm and Ammo Relinquishment and Sentencing", "Restitution Memo and Sentencing", 
                                                                        "Sentencing on Violation of Probation", "Surrender for execution of sentence",
                                                                        "Probation Report and Sentencing (DUI)") ~ "Sentencing",
                                hearing_type_original %in% c("Jury Trial", "Pretrial Conference","Warrant-Failed to Appear","To Trail","Jury Trail","Preliminary Hearing",
                                                                     "To Set", "Warrant-Arraignment", "Warrant-Arraignment","Warrant-Arraignment","Warrant-Arraignment","Attorney and Plea",
                                                                     "PC 1538.5 motion","Domestic Violence Pretrial","Further Proceedings",	"Continued Jury Trial",	
                                                                     "Motion to Continue Trial Date", "Motion hearings","Motion to Continue","Disposition and To Set","Pathways Intake",
                                                                     "Further O.R. Report","Disposition","Bail Motion","Drug Court Status Hearing - Predisposition", "Reset",
                                                                     "Order for Return of Prisoner","Pretrial and To Set","Penal Code section 995 motion","Warrant - Failure to comply",
                                                                     "Diversion Violation Arraignment","ID of Counsel","Military Diversion Review","Veteran Treatment Court Review",
                                                                     "Marsden motion", "Review", "Warrant-Probation Violation", "Civil Compromise",
                                                                     "Drug Court Decision on Suitability for Drug Court Diversion","Diversion Violation Conference",
                                                                     "Probation Violation Conference","Motion to Consolidate","Proof of Drivers License",
                                                                     "Receipt of Doctors Report","Bail Bond Motion","Military Diversion Intake","Modification of Sentence", 
                                                                     "Motion to Dismiss","AB208 Diversion Violation Arr-Reinstate Criminal Proceeding", 
                                                                     "Continued Preliminary Hearing", "Subpoenaed Records Status and Further Proceedings",
                                                                     "Probation Violation To Trail", "Extradition Review", "AB 208 Diversion Violation Conference",	"Pitchess Motion",
                                                                     "Superior Court Review and To Set", "Motion to Compel", "Bail Review hearing", 	"Status of:",
                                                                     "Hearing on Demurrer", "Entry of Plea", "Diversion Violation to Trail", "Court Trial", "Status of Appeal",
                                                                     "Humphreys Hearing", "Probation Violation Arraignment", "Receipt of Subpoenaed Records", "Substitution of Attorney",	
                                                                     "Petition to Extend Commitment", "Status Hearing", "AB 208 Diversion Violation to Trail",	"Veteran Treatment Court Intake",
                                                                     "Drug Court Treatment Plan", "Motion - PC1203.4", "Drug Court Status Hearing - Post Disposition",
                                                                     "Faretta Motion","Restoration of Competency Certification - PC 1372",
                                                                     "DEJ Violation Arr and Reinstate Criminal Proceedings", "Status Conference",  "Identification Hearing", 
                                                                     "Plea Hearings","AB 208 Diversion Violation Arraignment","Bail Bond Surrender","Failure to Comply",
                                                                     "Proof of Completion","Restitution Memo","Motion to Vacate","Receipt of Placement Report", 
                                                                     "PC 1170.95 Petitions for Resentencing","AB 208 Diversion Violation Hearing","MDUI Court Review","Motion to Quash",
                                                                     "Other hearing","Proof of Enrollment","Serna Motion (Right to Speedy Trial)","Diversion hearing",
                                                                     "Order to Show Cause Hearing","Proof of Firearm and Ammunition Relinquishment", "Probation Violation Hearing",
                                                                     "Appointment of Doctor(s)","Mental Health Diversion Eligibility","Doctors Report Referral",
                                                                     "Domestic Violence Probation Violation Pretrial","Bridges Review Intake",
                                                                     "Hearing Re: Outpatient Status Pursuant to PC 1606","Military Diversion Determination",
                                                                     "DEJ Violation Arraignment","O.R. Review","Bridges Review","Motion - 17(b) PC",
                                                                     "Motion to Strike Prior Conviction(s)","Motion to Withdraw as Attorney","Domestic Violence Progress Report",
                                                                     "To Set Probation Violation","Further Court Trial","In Camera Hearing","Mental Competence Hearing",
                                                                     "Restitution Hearing", "Disposition or To Set Probation Violation","Hearing Re: Cert of Restoration of Competency PC 1372",
                                                                     "Order to Show Cause - Attorney","Pathways Review","Conference","Placement Report Referral",
                                                                     "Resentencing/modification hearing","Court Trial on Prior Convictions","Pre-Court Trial", 
                                                                     "Mental Competency Progress Report PC1370(b)(1)","Proof of Restitution","DEJ Violation Hearing",
                                                                     "Mental Health Diversion Suitability Hearing","Petition for or Status of Commitment Regarding Competency",
                                                                     "Receipt of Medical Records","Certificate of Rehabilitation and Pardon","Court Trial Pursuant to PC 1026-1027",
                                                                     "DEJ Violation Pretrial","Ruling on Submitted Matter","Order to Show Cause - Witness",	"Restitution Court Status Review",
                                                                     "To Set Mental Health Diversion Suitability Hearing", "Evidentiary Hearing","Examination of Source of Bail",	"To Set Bail",
                                                                     "Jury Trial PC 1367-68",	"Jury Trial Pursuant to PC 1026-1027","Referral to courthouse clinic for mental health evaluation",
                                                                     "Mandatory Supervision to Trail",	"Order to Show Cause Re: Contempt","Petition for Release of Commitment - PC 1026.2",
                                                                     "Petition of Writ for Habeas Corpus","PRCS To Trail","Return on Appeal","HIV Test Results",
                                                                     "Mandatory Supervision Violation Arraignment","Mandatory Supervision Violation Conference",
                                                                     "Mandatory Supervision Violation Hearing","Motion for a new trial","Pretrial on Probation Violation",
                                                                     "Proof of Firearms Ammo Relinquishment and Restitution Report","Return on Appeal - Remittitur Issued",
                                                                     "Veteran Treatment Court Probation Violation","Jury Trial - Prior Convictions","Lifting of no contact order",
                                                                     "Mental Health Diversion Review","Motion for Modification","OSC hearing - Predisposition hearing",
                                                                     "Resentencing Pursuant to PC 1170.18","Return after PC 1203.03 90-day Diagnostic","Status of DA case filing",
                                                                     "Warrant-Mandatory Supervision Violation","Discovery hearing","Franklin Hearing",
                                                                     "Further Proceedings on Probation Violation","Motion to Sanction","Motion to Strike","Parole Revocation Conference",
                                                                     "Petition Hearing","Proof of Correction","Resentencing Pursuant to Prop 64","Veterans Treatment Court Graduation",
                                                                     "Superior Court Review", "Dispo/Confirm", "Mental Competency Progress Report PC1370(b)(1)"
                                                                      ) ~ "Other"))

#Flag Construction
court_hearing<- court_hearing %>%
    mutate(court_fta_flag=if_else(fta_warrant=="Warrant", 1, 0),
           court_fta_nowar_flag =if_else(fta=="FTA", 1, 0))
```


```{r}
#Court Hearing Diagnostics
court_hearing%>%
    count(hearing_type,hearing_type_code_description, sort=TRUE)
...
Jail Data
```{r}
# Jail Bookings Data Import -- Yuba, 2015-01-1 to 2019-11-25 
  ## book_df contains mostly unique rows of bookings with a couple hundred exceptions in which bookings are listed multiple times, sometimes with more than one person attached to them
  ## booking dates go from 1999-06-25 to 2019-11-25
  ## release dates go from 2015-01-01 to 2019-06-30

####

  ## Check why there are 378 duplicate book_num values
      ### many seem to have release date/type information in one row and none in other
      ### one instance of book_num being shared between two people/bookings (4 rows)
      ### many have charge code of "Other"
      ### decide whether to handle this in collapse or in reading data in stage
      ### there is overlap between the two files we are reading in, sometimes additional info.
      ### provided in the first quarterly data not present in historical data for same booking
      ### fixed by distinct-ing by book_num and using only row from quarterly data submission
      ### book_num might be unique to charge instead of booking, needs investigation
  ## local_id is a five digit numeric code that is unique to each individual
  ## why not more individuals with no release date? rated capacity 426, 428 with no release date


# Jail Data

## book_df contains mostly unique rows of bookings with a couple hundred exceptions in which bookings are listed multiple times, sometimes with more than one person attached to them
  ## booking dates go from 1999-06-25 to 2020-7-16
  ## release dates go from 2015-01-01 to 2020-7-16

setwd("G:/CrimJustice/PretrialAssessmentPilot")

# June 30 2020 submission is cumulative, will use only that one
book_df <- fread("Historical Data Submissions/Yuba/P2 Submission-June 30/pretrial.csv", colClasses = "character")
...
# remove duplicated booking numbers (three rows)
book_df <- book_df %>%
...
# Standardize Factors--See pg. xxx for more details.
  ## No book_type provided
  ## All juvenile release_type values filtered out
  ## release_type data very messy (91% re-coded successfully)
book_df <- book_df %>%
  mutate(sex = case_when(sex_original %in% c("M") ~ "Male",
                         sex_original %in% c("F") ~ "Female",
                         T ~ "Other_Unknown"),
...
book_df %>%
  add_count(local_id, book_date) %>%
  filter(n > 1) %>%
  arrange(local_id) %>%
  select(c(last_name, book_date_time, arrest_charge_code, arrest_charge, arrest_charge_level, release_date, release_type, release_type_original)) 
# above shows that people are being booked on same day (at different times) for different charges, or in at least some cases for a real charge and an "other" charge, and given unique book_nums each time

# Email SO contact to find out about people showing multiple bookings on same days, multiple book_ids
# some of them have one actual charge and one "other" charge
...
book_df %>%
  ...
  filter(n == 2) %>%
  arrange(book_num) %>%
  # select(-c(release_type, release_type_original, release_date)) %>%
  filter(join_var != "F_Other_") %>%
  distinct()
...
length(is.na(book_df$book_num))

    book_df %>% distinct(book_num)
    book_df %>% add_count(book_num) %>% 
      filter(n>1) %>%
      arrange(book_num)



    book_df %>% distinct(book_num, local_id) # n = 29,055

    book_df %>% distinct(book_num, book_date, local_id) # n = 29,055
    
    length(unique(book_df$local_id))
    
    book_df %>% distinct(local_id, na.rm = T)

    book_df %>% 
      add_count(book_num, local_id, book_date_time) %>% 
      filter(n>1) %>%
      arrange(book_num, last_name)

# Release type boondoggle
...
mean(is.na(book_df$local_id))
sort(table(book_df$local_id, useNA = "ifany"), decreasing = TRUE)[1:150]

length(unique(book_df$local_id))

book_df[book_df$cii == "A33707628", ]
...
length(which(is.na(book_df$release_date)))

book_df[is.na(book_df$release_date), ][order(book_df$book_date), ]

book_df %>%
  filter(status == "A")
  
  filter(book_date > as_date("2019-12-31")) %>%
  arrange(book_date)

table(book_df$release_date[book_df$status == "A"], useNA = "ifany")
...
Court Data
```{r}
# Court Case Data Import -- Yuba, 2015-01-01 to 2019-12-31
  ## court_charge contains one row per disposition including VOPs stemming from original case
  ## court_hearing contains one row per hearing event (check this) on each court case and includes information about each hearing. 
  ## court_fta_war contains one hearing at which fta occurred and warrant was issued per row
  ## court_sentence contains one row per sentence event component in which the sentence event is confinement
  ## court_sent_prob contains one row per sentence event component in which the sentence event is supervision (e.g. probation) 
  ## court_person contains one row per case/docket id
  ## Not used: court_status, court_sent_dl_cond and court_fine
...
# Court Filing Charges this is old code they sent a new cummulative data set
  ## court_charge contains one row per disposition including VOPs stemming from original case
  ## VOPs and juvenile charges filtered out removing about 2,800 rows
...
# Court Filing Charges --New cummulative data submitted 7/29/2020 
  ## dates are in strage formats they have ymd_hms, but time stamp useless becasue all of the times are zeros
  ## Final date ranges from 6/5/1987 to 7/28/2020
  ## Plea date ranges from 2/21/1997 to 7/28/2020
court_charge <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_Charges.xlsx")
...
# Date Reformatting--See pg. xxx for more details.
  ## Filter out observations where dispo date is after final date
court_charge <- court_charge %>%
  mutate_at(vars(plea_date, disposition_date, final_date), ymd)

# Standardize Factors--See pg. xxx for more details.
  ## juvenile charges filtered out
  ## dispo dates after final_date filtered out (these are actually VOPs stemming from an original case)
court_charge <- court_charge %>%
  filter(!(disposition_type_original %in% c("Admitted Juvenile",
                                            "Juvenile Wardship"))) %>%
  filter(!(disposition_date > final_date)) %>%
...
# Court Hearings
  ## court_hearing contains one row per hearing event (check this) on each court case and includes information about each hearing. 
  ## hearing_outcome contains FTA information ("Failure to Appear")

# court_hearing <- lapply(c("Yuba/Yuba.xlsx", "Yuba SQL Results 1-30-2020.xlsx"),
#                        function(x) {
#                          read_excel(paste0("Historical Data Submissions/Yuba/",
#                                            x),
...
#   distinct()
 ...
 ##hearing dates range from 12/18/1996 to 9/17/2021 (these dont have hearing outcme but are probably calendared events)
court_hearing <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_Hearings.xlsx")


# Variable name standardization--See pg. xxx for more details.
names(court_hearing) <- c("court_docket_id", "court_case_id", "hearing_date", "hearing_type_original", "hearing_outcome")


# Date Reformatting--See pg. xxx for more details.
court_hearing <- court_hearing %>%
  mutate_at(vars(hearing_date), mdy)
...
# Standardize Factors--See pg. xxx for more details.
  ## filter out all juvenile-related hearings
court_hearing <- court_hearing %>%
...
# Court FTAs
  ## court_fta_war contains one hearing at which fta occurred and warrant was issued per row
  ## warrants include FTA warrants, arrest warrants, others. see fta_warrant_type variable.
  ## "NULL"s changed to NA  
  ## all have value 1 for fta_hearing and fta_warrant
  ## fta_hearing renamed fta_flag
# court_fta_war <- lapply(c("Yuba/Yuba.xlsx", "Yuba SQL Results 1-30-2020.xlsx"),
#                         function(x) {
#                           read_excel(paste0("Historical Data Submissions/Yuba/", x),
...
# Court FTA
  ## this file has zero observations needs to be resent
court_fta <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_PTSWarrant.xlsx")

# replace above with cumulative data rec'd June 2020
court_fta_war <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/ODY-330218 CAYUBAPROD - JCC Pretrial Data Extract Script Request.xlsx", sheet = 1, na="NULL", col_types="text")

# Variable name standardization--See pg. xxx for more details.
names(court_fta_war) <- c("court_docket_id", "court_case_id", "fta_flag", "fta_date", "fta_warrant", "fta_warrant_date", "fta_warrant_type")
...
court_fta_war <- court_fta_war %>%
  mutate(fta_flag = as.numeric(fta_flag))
...
# Court Sentences to Confinement 
  ## court_sentence contains one row per sentence event component in which the sentence event is confinement
  ## type of confinement sentence taken from sentence_location column, perhaps imperfectly
...
## this file is missing all observation but has the same variables as the older file above 
# court_sentence <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_SentConf.xlsx")

# cumulative table rec'd June 2020:
court_sentence <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/ODY-330218 CAYUBAPROD - JCC Pretrial Data Extract Script Request.xlsx", sheet = 2, na="NULL", col_types="text")


# Variable name standardization--See pg. xxx for more details.
names(court_sentence) <- c("court_docket_id", "court_case_id", "sentence_date", "sentence_type_original", "sentence_location", "sentence_term", "sentence_term_value")
...
# Standardize Factors--See pg. xxx for more details.
court_sentence <- court_sentence %>%
  mutate(sentence_type = case_when(grepl("Jail", sentence_location, ignore.case = TRUE) ~ "Jail", 
                                   grepl("Prison", sentence_location, ignore.case = TRUE) ~ "Prison",
                                   T ~ "Other"))


# Court Sentences to Supervision
  ## court_sent_prob contains one row per sentence event component in which the sentence event is supervision (e.g. probation) 
  ## all given sentence_type "Probation", some are Court Summary Probation (Unsupervised) and some are Formal Probation
...
####for some reason this new data set is totally empty####
# court_sent_prob <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_SentProb.xlsx")
######################################################

# below is cumulative file rec'd June 2020:
court_sent_prob <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/ODY-330218 CAYUBAPROD - JCC Pretrial Data Extract Script Request.xlsx", sheet = 3, na="NULL", col_types="text")
                       

# Variable name standardization--See pg. xxx for more details.
names(court_sent_prob) <- c("court_docket_id", "court_case_id","sentence_date", "sentence_type_original", "sent_prob_location", "sentence_term_value", "sentence_term")
...
# Standardize Factors--See pg. xxx for more details.
court_sent_prob <- court_sent_prob %>%
  mutate(sentence_type = case_when(sentence_type_original %in% c("Court (Summary/Unsupervised)",
                                                                 "Formal (Supervised)") ~
                                     "Probation"))
...
# Court person
  ## court_person contains one row per case/docket id
  ## file_dates from 5/19/2016 through 6/30/2020  
court_person <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/Yuba_TechElements.xlsx")


# Variable name standardization--See pg. xxx for more details. 
names(court_person) <- c( "court_docket_id", "court_case_id", "last_name", "first_name", "middle_name", "file_date", "dob", "cii", "cdl_id", "fbi", "sex_original", "race_original", "cdl_state", "assessment_date")


# Date Reformatting--See pg. xxx for more details.
court_person <- court_person %>%
  mutate_at (vars (assessment_date), convertToDate) %>%
  mutate_at (vars(file_date, dob), mdy)
...
court_dispo <- court_charge %>%
  mutate(disposition_type = ordered(disposition_type, levels = c("Other", "Dismissed", 
                                                                 "Not_Guilty", "Nolo_Contendere", 
                                                                 "Guilty"))) %>%
  group_by(court_case_id) %>%
  ## Charges that have not been disposed are removed (Cannot find any evidence this was actually done)
  mutate(hierarchy = if_else(is.na(hierarchy), 1e6, as.numeric(hierarchy))) %>%
  group_by(court_case_id) %>%
...
# Court FTAs are collapsed to each case. 
  ## court_fta_war is used instead of court_hearing following Noah Lehman's example w/ Santa Barbara 
  ## (NL: "due to the very small number of ftas that do not have a warrant issued along with it")
  ## CHECK WHETHER value "Failure to Appear" in court_hearing$hearing_outcome corresponds with case appearing in court_fta_war
court_fta_war <- court_fta_war %>%
...
# court_hearing collapsed to only leave earliest arraignment 
court_hearing <- court_hearing %>%
...
# Court charges (which includes dispos) are filtered for plea events and collapsed to court case id. 
...
# Court sentences are combined and collapsed to court case id. 
  ## Currently whatever is in the top row of sentence type is filtered. E.g. if the first row is "Jail" and the second is "Prison", "Jail" will be the selected row. However, confinement is always given priority over any type of supervision because court_sentence contains sentences to confinment and and court_sent_prob contains sentences to supervision.  
court_sent <- bind_rows(court_sentence, court_sent_prob) %>%
  arrange(sentence_date) %>%
  distinct(court_case_id, .keep_all = T) %>%
  select(court_case_id, court_docket_id, sentence_date, sentence_type_original, sentence_type)

# All data are joined.
## court_person is one row per case so it will go first in join
## there are some 740 cases that appear in court_person but do not have an arraignment listed in court_hearing, most common hearing types for these cases in court_hearing are "PreTrial Hearing" and "Case Pull w/Criminal" but these do not seem like arraignments so they will be ignored
## since there are cases that show up in court_hearing but not in court_person, we will start our join with the cases with arraignments in court_hearing
## there are a lot of court_case_ids in court_person and court_case_ids in court_charge that do not overlap with the other table
## we are starting our collapse with court_dispo (derived from court_charge) because otherwise we'd have individuals with no charge information included

court_df <- reduce(list(court_dispo, court_hearing, court_person, court_plea, court_fta_war, court_sent), left_join, by = "court_case_id") %>%
  select(-contains(".y")) %>%
  select(-contains(".x.x")) %>%
  select(-court_docket_id.x) %>% # court_docket_id appears in all tables incl leftmost
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .))) %>%
  mutate(final_disposition_date = if_else(disposition_date <= sentence_date|is.na(sentence_date), 
                                          disposition_date, sentence_date), 
...
```
 ...
 Court Diagnostics
```{r eval=FALSE, include=FALSE}
court_charge[court_charge$court_case_id %in% check_ids, ]
               
check_ids <- anti_join(court_person, court_dispo)["court_case_id"]

check_dockets <- anti_join(court_person, court_dispo)["court_docket_id"]

court_charge[anti_join(court_dispo, court_person)
...
summary(as.integer(unlist(check_dockets)))

572718-612457

court_charge[court_charge$court_docket_id %in% check_dockets, ]


court_hearing %>%
  distinct(court_case_id)

no_arraignment_cases <- court_person$court_case_id[!(court_person$court_case_id %in% court_hearing$court_case_id)]

anti_join(court_person, court_hearing)

table(court_hearing[court_hearing$court_case_id %in% no_arraignment_cases, c("hearing_type_original", "hearing_type")])
...
length(which(is.na(court_charge$final_date)))

court_charge %>%
  filter(disposition_type %in% c("Dismissed")) %>% # 3,679
  # filter(!(disposition_date > final_date)) %>%
  filter(is.na(final_date)) # 143
...
# Probation data are long rather than wide and need to be cleaned up.  
  ## sheet 1 (release level data )comes from Tyler probation, sheet 2 (assesment level data) comes from the assessment data that is   not integrated into Tyler.
  ## In sheet 1 if ref_status== ("Pretrial - Rec. Override", "Pretrial - Recommendation") then there should mostly be a match in the assessment data
  ## what are /r's at the end of scores are junk characters that Lisa Lit could not filter out.
  ## how are court date reminders done : Everyone on pretrial supervision is supposed to be enrolled in the Automated Reminder System, in addition to calls from the supervising PO. For this initial data draw, I was unable to query the ARS table. A ticket has been submitted to Tyler to get access to that table for future reports. 
  ## only 98 assessments out of 180 probation IDs had a match in the assessment score table.  Is it becasue any people released to supervision not assessed?  This is becasue sheet 1 contains releases that did not have a coresponding assessmnet, usually cases closed for neutral reason. So this result is to be expected.


# Yuba Assessment Data Import
  ## assess_df2 contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The tool questions are widened into columns.
  ## column `Filenum` is identical to `File Num` (court_case_id) so it will be dropped
  ## column `Ref Type consists of only one value, "Pre-Trial", so it will be dropped as well
  ## Group of people whose court case IDs start with "PT" seem to have less data about them in this table
  ## "AA" and "PA" release recommendations/overrides -- what do these mean?
  ## assessment_date originally called "release recommendation date", but is actually assessment date (and re-named accordingly)
  ## 
assess_df <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/200701 Pretrial Probation Yuba County Jan 1-June 30 2020.xlsx") %>%
    distinct()

names_df <- tibble(original_field = names(assess_df))    

names(assess_df) <- c("pretrial_id", "last_name", "first_name", "middle_name", "sex_original", "race_original", "zip_code", "court_case_id", "cdl_id", "cii", "fbi", "court_case_id_duplicate", "ref_type_useless", "ref_status_date", "ref_status_original", "ref_status_reason_original")
...
assess_df <- assess_df %>%
  mutate(ref_status = ref_status_original,
         ref_status_reason = ref_status_reason_original) %>%
  group_by(ref_status) %>%
  mutate(rn = row_number()) %>%
  pivot_wider(names_from = ref_status, 
              values_from = c(ref_status_reason, ref_status_date)) %>%
  ungroup() %>%
  filter(pretrial_id != "Total") %>%
  mutate(pretrial_termination_outcome = case_when(!is.na(`ref_status_reason_Pretrial - Closed Successful`) ~ "Successful",
                                                  !is.na(`ref_status_reason_Pretrial - Closed Unsuccessful`) ~ "Unsuccessful",
                                                  !is.na(`ref_status_reason_Pretrial - Closed Neutral`) ~ "Ineligible")) %>%
  unite(col = "pretrial_termination_reason_original",
        c("ref_status_reason_Pretrial - Closed Neutral",
          "ref_status_reason_Pretrial - Closed Successful",
          "ref_status_reason_Pretrial - Closed Unsuccessful"),
        na.rm = TRUE) %>%
  mutate(pretrial_termination_reason_original = if_else(pretrial_termination_reason_original == "",
                                                        NA_character_,
                                                        pretrial_termination_reason_original)) %>%
  mutate_at(c("ref_status_date_Pretrial - Closed Neutral",
          "ref_status_date_Pretrial - Closed Successful",
          "ref_status_date_Pretrial - Closed Unsuccessful"), 
          as.character) %>%
  unite(col = "pretrial_termination_date",
        c("ref_status_date_Pretrial - Closed Neutral",
          "ref_status_date_Pretrial - Closed Successful",
          "ref_status_date_Pretrial - Closed Unsuccessful"),
        na.rm = TRUE) %>%
  unite(col = "release_recommendation_original",
        c("ref_status_reason_Pretrial - Recommendation",
          "ref_status_reason_Pretrial - Rec. Override"),
        na.rm = TRUE) %>%
  mutate_at(c("ref_status_date_Pretrial - Recommendation",
              "ref_status_date_Pretrial - Rec. Override"),
            as.character) %>%
  unite(col = "assessment_date",
        c("ref_status_date_Pretrial - Recommendation",
          "ref_status_date_Pretrial - Rec. Override"),
        na.rm = TRUE) %>% 
  rename(release_decision_original = "ref_status_reason_Pretrial - Judge's Decision",
         release_decision_date = "ref_status_date_Pretrial - Judge's Decision",
         supervision_start_date = "ref_status_date_Pretrial - Supervision") %>%
  select(-c(court_case_id_duplicate, ref_type_useless, rn, `ref_status_reason_Pretrial - Supervision`, ref_status_reason_NA, ref_status_date_NA))

assess_df <- assess_df %>%
    mutate(pretrial_termination_date = na_if(pretrial_termination_date, ""),
           assessment_date = na_if(assessment_date, "")) %>%
    mutate_at(vars(pretrial_termination_date, assessment_date), ymd) %>%
    mutate_at(vars(release_decision_date, supervision_start_date), as_date)

# New reasons for termination/unsuccessful will need to be added to below code for later data

assess_df <- assess_df %>%
  mutate(release_recommendation = case_when(release_recommendation_original %in% 
                                              c("AA Release w/Conditions", 
                                                "PA Release w/Conditions") ~ "Monitor",
                                            release_recommendation_original %in% 
                                              c("Detain") ~ "Detain"),
         release_decision = case_when(release_decision_original %in% 
                                        c("Release w/No Conditions") ~ "OR",
                                      release_decision_original %in% 
                                        c("Release w/Conditions") ~ "Monitor",
                                      release_decision_original %in% 
                                        c("Detain") ~ "Detain",
                                      release_decision_original %in%
                                        c("Other") ~ "Other_Unknown"),
         pretrial_termination_reason = 
           case_when(pretrial_termination_outcome == "Other_Unknown" ~ "Not_Tracked",
                     pretrial_termination_reason_original == "Absconder" ~ "Abscond",
                     pretrial_termination_outcome == "Successful" ~ "Case_Disposed"),
         sex = case_when(sex_original %in% c("F") ~ "Female",
                         sex_original %in% c("M") ~ "Male",
                         sex_original %in% c("U") ~ "Other_Unknown"),
         race = case_when(race_original %in% c("Hispanic") ~ "Hispanic",
                          race_original %in% c("White") ~ "White",
                          race_original %in% c("Black") ~ "Black",
                          T ~ "Other_Unknown"),
...
#### 
## assess_df2 contains one row per risk assessment event/risk assessment question/pretrial terms and conditions. The tool questions are widened into columns.
## Eliminate carriage return characters ("\r") from last three columns of table

assess_df2 <- read_excel("G:/CrimJustice/PretrialAssessmentPilot/Historical Data Submissions/Yuba/P2 Submission-June 30/200701 Pretrial Probation Yuba County Jan 1-June 30 2020.xlsx", sheet = 2) %>%
  filter(`Probation ID` != "Total") %>%
  mutate_all(~ gsub("\\\r$", "", .))

names_df2 <- tibble(original_field = names(assess_df2))    


names(assess_df2) <- c("pretrial_id", "last_name", "first_name", "middle_name", "court_case_id", "cii", "fbi", "oras_tool_response_1", "oras_tool_response_2", "oras_tool_response_3", "oras_tool_response_4", "oras_tool_response_5", "oras_tool_response_6", "oras_tool_response_7", "oras_risk_score", "current_offense_severity", "prior_offense_severity")

names_df2$field <- names(assess_df2)
...
# Make ORAS responses, total scores, and offense severity modifiers numeric
assess_df2 <- assess_df2 %>%
  mutate_at(vars(contains("oras_tool_response"), oras_risk_score, contains("offense_severity")),
            ~ as.numeric(.)) %>%
  mutate(risk_level = case_when(oras_risk_score %in% c(0,1,2) ~"Low",
                                oras_risk_score %in% c(3, 4,5) ~ "Medium",
                                oras_risk_score %in% c(6, 7, 8, 9) ~ "High"))



# there are 6 people who appear in both tables but do not have matching court_case_ids
# assess_df2 %>%
#   anti_join(assess_df, by = "court_case_id") %>%
#   arrange(pretrial_id)
# assess_df %>%
#   filter(pretrial_id %in% c("2581676", "1504334", "1499498", "2547951", "2550594", 
#                                        "1504593", "2581681")) %>%
#   arrange(pretrial_id)


## check how many people have an assessment date but do not have an assessment score
## everyone with a score has an assessment date

## Probation Assessment data Flags

## need to finish these

## no detail on em monitoring recommended release condition or drug test release condition
...
assess_df <- assess_df %>% 
  mutate(rec_release_flag = if_else(release_recommendation %in% c("OR", "Monitor"), 1, 0),
         ...
         # condition_em_flag = if_else(grepl("ELECTRONIC MONITOR|GPS|SCRAM", toupper(release_conditions_original))| grepl("SCRAM", toupper(pretrial_release_type_original)), 1, 0),
         ...
         # condition_drug_test_flag = if_else(grepl("SCRAM|TEST", toupper(release_conditions_original))| grepl("SCRAM|TEST", toupper(pretrial_release_type_original)), 1, 0),
         # condition_no_contact_flag = if_else(grepl("NO CONTACT|136", toupper(release_conditions_original)), 1, 0),
         # court_date_reminder_flag = if_else(court_date_reminder_original == "CourtAppearance"| pretrial_release_type_original %in% c("PTS with court reminders", "OR-with court reminders"), 1, 0),
...
# collapse rows so that recommendations and terminations appear on the same one for same probation_id/court_case_id
# need a way to make the NANANANA values actually show up as NA
suppressWarnings(assess_df <- assess_df %>%
...
# 
# To do next time: semi_join to figure out how to combine assess_df2 (99 rows) with assess_df (411 rows)
assess_df <- assess_df %>%
  left_join(assess_df2, by = "pretrial_id") %>%
  select(-contains(".y")) %>%
  rename_at(vars(contains(".x")), funs(gsub("\\.x", "", .)))

assess_df <- assess_df %>%
  mutate(assess_flag = if_else(!is.na(as.character(assessment_date)) &
                                 !is.na(oras_risk_score), 1, 0))
...
assess_table <- assess_df %>%
  ...
  # Create county, tool-name, dob, charge_level.
  mutate(county = "Yuba",
         tool_name = "oras",
         dob = NA,
         charge_level = NA,
  ...
  select(county, release_recommendation, release_decision, assessment_date, oras_risk_score, tool_name, assessment_date, dob, sex, race,  charge_level, risk_level, monitor_level, pretrial_termination_reason)

# Save file  
fwrite(assess_table, file = "G:/CrimJustice/PretrialAssessmentPilot/Assessment Output Tables/Yuba Assessment Output 10-2020.csv")
...
# need to re-do names_df by hand to reflect original variables we want displayed, code below doesn't work bc of issues with columns we created/destroyed with pivot_wider and unite above
...
names_df <- names_df %>%
  mutate(
    modified_field = case_when(
      ...
      field == "ref_status_original" ~ "release_decision, pretrial_termination_reason, release_recommendation, pretrial_termination_outcome, pretrial_violation_flag, rec_release_flag, decision_release_flag",
      field == "ref_status_reason_original" ~ "release_decision, pretrial_termination_reason, release_recommendation, pretrial_termination_outcome, pretrial_violation_flag, rec_release_flag, decision_release_flag",
      ...
      field == "oras_risk_score" ~ "assess_flag")) %>%
...
flag_df <- flag_df %>%
  ...
  modified_field == "pretrial_violation_flag" & modified_value == "1" ~ "If pretrial termination outcome is Unsuccessful",
  T ~ value)) %>%
  # filter(modified_field == "pretrial_ineligible_reason") %>%
  group_by(modified_field, value) %>%
  mutate_at(vars(original_field, field), funs(paste(., collapse = ", "))) %>%
...
write.xlsx(list(factor_df, flag_df, date_df), file = "G:/CrimJustice/PretrialAssessmentPilot/Code Books/Assessment Code Book/Yuba Assessment Code Book 9-20.xlsx", asTable = T, tableStyle = "TableStyleMedium1", colWidths = "auto", sheetName = c("Factors", "Flags", "Dates"))
...
assess_df[assess_df$pretrial_id == "1512041", ]


assess_df2 

sapply(assess_df2$pretrial_id,
       function(x) {
         assess_df$pretrial_termination_date[assess_df$pretrial_id == x]
       })
...
names(prob_df)
prob_df3 <- prob_df %>%
  filter (`Ref Status` %in% c( "Pretrial - Rec. Override", "Pretrial - Recommendation" ))

prob_df3 %>%
  distinct(`Probation ID`)%>%
  semi_join(prob_df2, by = c("Probation ID"))


write.csv (table(assess_df[c( "Ref Status Reason", "Ref Status")]), file = "Yuba_Questions.csv") 


table(assess_df["Ref Type"])


# Variable name standardization--See pg. xxx for more details.
names(assess_df) <- c("pretrial_id", "cii", "court_case_id", "court_docket_id", "fbi", "local_id", "cdl_id", "first_name", "last_name", "dob", "sex_original", "race_original", "hispanic_flag", "tool_name", "assessment_date_time", "zip_code", "release_recommendation_original", "recommendation_comments", "release_authorization_original", "release_type_original", "release_date", "release_conditions_original", "pretrial_violation_flag", "pretrial_violation_date",  "court_date_reminder_original", "other_pretrial_service_original", "pretrial_termination_outcome_original", "pretrial_termination_date", "open_date", "assignment_date", "arrest_date", "oras_tool_response_1", "oras_tool_response_2", "oras_tool_response_3", "oras_tool_response_4", "oras_tool_response_5", "oras_tool_response_6", "oras_tool_response_7", "oras_risk_score", "current_offense_severity", "prior_offense_severity")
...
assess_df <- assess_df %>%
  mutate(tool_name = case_when(tool_name == "Virginia Pretrial Risk Assessment_Old" ~ "vprai",
                               ...
                               T ~ "NULL")) %>%
  mutate(vprai_risk_score = if_else(tool_name == "vprai", vprair_risk_score, NA_character_),
         vprair_risk_score = if_else(tool_name == "vprair", vprair_risk_score, NA_character_))
...
table(assess_df$`Ref Status Reason_Pretrial - Closed Successful`, useNA = "ifany")

any(apply(assess_df[c("Ref Status Reason_Pretrial - Closed Neutral", 
                      "Ref Status Reason_Pretrial - Closed Successful",
                      "Ref Status Reason_Pretrial - Closed Unsuccessful")],
          MARGIN = 1,
          function(x) length(which(is.na(x))) != 2))



mean(assess_df$Filenum == assess_df[c("Filenum", "File Num")], na.rm = TRUE)

nrow(unique(assess_df["Probation ID"]))

which(!is.na(assess_df["Ref Status Reason_Pretrial - Closed Successful"]))

prob_df %>%
  count (`Ref Status`,`Ref Status Reason`)

summary(prob_df$`Ref Status Date`)
...
#Create an excel workbook with all data table. Make sure to include the cii table first. 
write.xlsx(list(doj_cii, doj_book, doj_court, doj_assess), file = "G:/CrimJustice/PretrialAssessmentPilot/CIIs for DOJ/October 2020/Yuba P2 CIIs and Demographics for DOJ.xlsx", sheetName = c("ciis", "booking", "court", "assessment"))
```
...
write collapsed tables
```{r}
fwrite(book_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Yuba Booking Collapse.csv")
fwrite(court_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Yuba Court Collapse.csv")
fwrite(assess_df, "G:/CrimJustice/PretrialAssessmentPilot/Agency Collapse CSV Files/Yuba Assessment Collapse.csv")
