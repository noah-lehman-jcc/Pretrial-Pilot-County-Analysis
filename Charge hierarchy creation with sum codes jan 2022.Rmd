...
doj_hierarchy_2022sec <- doj_hierarchy_2022sec %>%
  ...
  distinct(charge, charge_code, charge_level, .keep_all = T) %>%
  unite(join_var, charge_level, charge_code, charge, remove = F) %>%
  select(join_var, master_charge_description, charge_level, charge_code, charge, bcs_code, summary_code, possible_sentence) %>%
  mutate(summary_code = as.numeric(summary_code),
         bcs_code = as.numeric(bcs_code))

doj_hierarchy_2022 <- doj_hierarchy_2022lei %>%
  left_join(doj_hierarchy_2022sec %>% select(-bcs_code, -master_charge_description), by = c("join_var", "charge_level", "charge_code", "charge")) 
```
...
write.xlsx(new_codes, file = "G:/CrimJustice/PretrialAssessmentPilot/Charge Definitions and Hierarchy/New 2022 codes.xlsx", asTable = T, tableStyle = "TableStyleMedium1")
```
