...
eval_df %>%
  ...
  count(filed_flag, court_misd_flag, court_felony_flag)
  count(release_type_original)
  
full_df %>%
  filter(county == "Alameda") %>%
  count(filed_flag, court_misd_flag, court_felony_flag, arrest_dis)
...
df %>%
  filter(county == "Alameda") %>%
  count(is.na(assessment_date), is.na(vprair_risk_score))
...
drop_df2 <- drop_df %>%
  ...
  mutate(`Validation Type` = case_when(`Released Pretrial` >= 1000 ~ "General + Bias",
         ...
         County = if_else(County == "Nevada", "Nevada/Sierra", County)) %>%
  arrange(County) %>%
  ungroup() %>%
  mutate(`Validation Type` = if_else(County == "Santa Barbara" & `Tool Name` == "VPRAI", "No longer in use", `Validation Type`)) %>%
  select(County, `Tool Name`, everything())#%>%
  # rename(`Tool\nName` = `Tool Name`) %>%
  # rename(`Assessed\nBookings` = `Assessed Bookings`) %>%
  # rename(`Pretrial\nComplete` = `Pretrial Complete`) %>%
  # rename(`Released\nPretrial` = `Released Pretrial`) 
  # select(-`Tool Name`) %>%
...
ggplot(risk_df %>% distinct(vprair_risk_score, Total)%>% mutate(percent = Total/sum(Total))) +
  ...
  # ylim(0,100) +
  # facet_wrap(~name) +
  labs(x = "Risk Category", y = "Percent", title = "Alameda Distribution of Assessments by VPRAIR Risk Category")  
...
VPRAI--With Interactions at score 6 XXXXXXXX there is no VPRAI score 6, should have done this at score 5
\newline
```{r results='asis'}
m1 <- glm(family = binomial(),fta_flag ~ vprai_risk_score * race + release_to_pretrial_end, data = race_df %>% mutate(across(contains("score"), ~.x-6)))
m2 <- glm(family = binomial(),recid_arrested_flag ~ vprai_risk_score * race + release_to_pretrial_end, data = race_df %>% mutate(across(contains("score"), ~.x-6)))
m3 <- glm(family = binomial(),recid_filed_flag ~ vprai_risk_score * race + release_to_pretrial_end, data = race_df %>% mutate(across(contains("score"), ~.x-6)))
m4 <- glm(family = binomial(),recid_convicted_flag ~ vprai_risk_score * race + release_to_pretrial_end, data = race_df %>% mutate(across(contains("score"), ~.x-6)))
m5 <- glm(family = binomial(),recid_arrest_violent_psa_flag ~ vprai_risk_score * race + release_to_pretrial_end, data =
            race_df %>% mutate(across(contains("score"), ~.x-6)))
m6 <- glm(family = binomial(), any_pretrial_failure_flag ~ vprai_risk_score * race + release_to_pretrial_end, data = race_df %>% mutate(across(contains("score"), ~.x-6)))
...
ggplot(risk_df %>% distinct(vprair_risk_score, Total) %>% 
  ...
  geom_text(vjust = -.2, aes(label = str_c(round(percent*100), "%")), size = 2.6, position = position_dodge(width = .9)) +
  theme_bw() +   scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(x = "Risk Category", y = "Percent", title = "Alameda Distribution of Assessments by VPRAIR Risk Category", fill = "Gender")  
